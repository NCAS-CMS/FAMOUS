...
SUBROUTINE GC_ISEND (MSG, LEN, RECI, ISTAT, RARR, SARR)
!     ******************************************************************
!     * Purpose:
!     *
...
INTEGER (KIND=GC_INT_KIND) :: MSG, LEN, RECI, ISTAT, RARR(LEN),   &
                              SARR(LEN)
...
IF (RECI  /=  ME) THEN  ! If I'm not sending to myself
   CALL MPL_BSEND(SARR, GC__ISIZE*LEN, MPL_BYTE, RECI, MSG,       &
