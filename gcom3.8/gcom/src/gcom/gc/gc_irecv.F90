...
SUBROUTINE GC_IRECV (MSG, LEN, SEND, ISTAT, RARR, SARR)
!     ******************************************************************
!     * Purpose:
!     *
...
INTEGER (KIND=GC_INT_KIND) :: MSG, LEN, SEND, ISTAT, RARR(LEN),   &
                              SARR(LEN)
...
ELSE ! Receiving from another processor
  IF (SEND  ==  GC__ANY) THEN
     CALL MPL_RECV(RARR, GC__ISIZE*LEN, MPL_BYTE,                 &
          MPL_ANY_SOURCE,                                         &
          MSG, GC__MY_MPI_COMM_WORLD, STATUS, ISTAT)
  ELSE
     CALL MPL_RECV(RARR, GC__ISIZE*LEN, MPL_BYTE, SEND,           &
          MSG,GC__MY_MPI_COMM_WORLD, STATUS, ISTAT)
  ENDIF
ENDIF ! IF (SEND  ==  ME)
#endif
