...
do
  ...
  then
    ...
    echo -e \
"\nqsmncompile($SECONDS): ***\t  ERROR: $PREOBJDIR/$dir not found."
    if [ "$BUILDSECT" = "true" ]
    then
...
echo -e `gen_sed_string "@objects@" "6" "$OBJS1 $OBJS2"`          >> $SEDSC
echo -e `gen_sed_string "@preobjects@" "6" "$PREOBJS1 $PREOBJS2"` >> $SEDSC
