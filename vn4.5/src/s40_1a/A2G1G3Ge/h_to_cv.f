...
C
C*LL
CLL   SUBROUTINE H_TO_CV
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE FIELDS ON
CLL   ARAKAWA B GRID MASS POINTS TO ARAKAWA C GRID V POINTS.
CLL
...
C                          ! Mediterranean
     ...
     &,data_cv(imt,jmtm1)   ! out data on C grid y velocity points.
C
...
C
      do j=J_1,J_jmtm1 
        do i=1,imt-1
          data_cv(i,j) = ( data_h(i+1,j) + data_h(i+1,j+1) ) * 0.5
        end do
        data_cv(imt,j) = ( data_h(1,j) + data_h(1,j+1) ) * 0.5
      end do
C
      return
      end
