...
C
C*LL
CLL   SUBROUTINE UV_TO_CU
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE X COMPONENT
CLL   FIELDS ON ARAKAWA B GRID VELOCITY POINTS TO ARAKAWA C GRID.
CLL
...
CLL
CLL   WRITTEN BY J.F.THOMSON (07/05/93)
CLL
CLL  MODEL            MODIFICATION HISTORY:
CLL VERSION  DATE
CLL 3.4Generalise for slab model. J.Thomson
!     3.5    16.01.95   Remove *IF dependency. R.Hill
!     4.3    01.02.97   Correct indices on data_cu/data_uv. R.Hill
!     4.5    03/09/98   For CALL SWAPBOUNDS change all *IF DEF,MPP
!                       to be *IF DEF,MPP,AND,-DEF,SLAB since the
!                       slab model can now run with mpp selected.
!                       C. D. Hewitt
CLL
...
C
     ...
     &,imt            ! in number of points in each row.
      real
     & data_uv(imt,jmtm1) ! in  data on B grid.
     &,data_cu(imt,jmtm1) ! out data on C grid.
C
...
C Interpolate velocity field.
C
      do j=J_1,J_jmtm2
        do i=1,imt
          data_cu(i,j)   = ( data_uv(i,j) + data_uv(i,j+1) ) * 0.5
        end do
      end do
      do i=1,imt
        data_cu(i,J_jmtm1) = data_uv(i,J_jmtm1)
      end do
C
      return
      end
