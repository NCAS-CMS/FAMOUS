...
C
C*LL
CLL   SUBROUTINE UV_TO_CV
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE Y COMPONENT
...
CLL
CLL   IT ADHERES TO THE STANDARDS OF DOCUMENTATION PAPER 4, VERSION 1.
CLL
...
C
     ...
     &,data_cv(imt,jmtm1) ! out data on C grid.
C
...
C
      do j=J_1,J_jmtm1
        do i=1,imt-1
          data_cv(i,j) = ( data_uv(i,j) + data_uv(i+1,j) ) * 0.5
        end do
        data_cv(imt,j) = ( data_uv(imt,j) + data_uv(1,j) ) * 0.5
      end do
C
      return
      end
