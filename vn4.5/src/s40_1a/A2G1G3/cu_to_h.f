...
C
C*LL
CLL   SUBROUTINE CU_TO_H
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE FIELDS ON
CLL   ARAKAWA C GRID U POINTS TO ARAKAWA B GRID MASS POINTS.
CLL
...
C
     ...
     &,data_cu(imt,jmtm1) ! in  data on C grid x velocity points.
C
...
C
       do j=j_2,j_jmt
        do i=2,imt
          data_h(i,j) = ( data_cu(i-1,j-1) + data_cu(i,j-1) ) * 0.5
        end do
        data_h(1,j) = ( data_cu(imt,j-1) + data_cu(1,j-1) ) * 0.5
      end do
      do i=1,imt
        data_h(i,1)   = data_h(i,2)
      end do
      IF (L_OCYCLIC) THEN
C
