...
C
C*LL
CLL   SUBROUTINE H_TO_CU
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE FIELDS ON
CLL   ARAKAWA B GRID MASS POINTS TO ARAKAWA C GRID U POINTS.
CLL
...
C                          ! Mediterranean
     ...
     &,imt            ! in number of points in each mass row.
      real
     & data_h(imt,jmt)    ! in  data on mass grid.
     &,data_cu(imt,jmtm1) ! out data on C grid x velocity points.
C
...
C
      do j=J_1,J_jmtm1
        do i=1,imtm1
          data_cu(i,j) = ( data_h(i,j+1) + data_h(i+1,j+1) ) * 0.5
        end do
        data_cu(imt,j) = ( data_h(imt,j+1) + data_h(1,j+1) ) * 0.5
      end do
C
      return
      end
