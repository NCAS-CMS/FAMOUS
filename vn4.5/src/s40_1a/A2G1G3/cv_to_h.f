...
C
C*LL
CLL   SUBROUTINE CV_TO_H
CLL   -------------------
CLL
CLL   DYNAMIC SEA ICE MODEL SUBROUTINE TO INTERPOLATE FIELDS ON
CLL   ARAKAWA C GRID V POINTS TO ARAKAWA B GRID MASS POINTS.
CLL
...
!
     ...
     &,data_cv(imt,jmtm1) ! in  data on C grid y velocity points.
C
...
C
       do j = j_2, j_jmtm1
        do i=2,imt
          data_h(i,j)  = ( data_cv(i-1,j-1) + data_cv(i-1,j) ) * 0.5
        end do
      end do
      do i=2,imt
       IF (JST.EQ.1) THEN
          data_h(i,J_1) = data_cv(i-1,J_1)
       ENDIF
       IF (JFIN.EQ.JMT_global) THEN
          data_h(i,j_jmt)  = data_cv(i-1,J_jmt-1)
       ENDIF
      end do
      do j=2,jmtm1
        data_h(1,j)  = ( data_cv(imt,j-1) + data_cv(imt,j) ) * 0.5
      end do
      data_h(1,1)    = data_cv(imt,1)
      data_h(1,jmt)  = data_cv(imt,jmtm1)
       IF (L_OCYCLIC) THEN
C
C Make cyclic if necessary.
C
      do j=J_1,J_jmt 
        data_h(1,j)  = data_h(imt-1,j)
