...
!   4.4    9/12/97  Change gridbox mean canopy water from prognostic
!                   to diagnostic.                    R.A.Betts
!
...
!                                 !    soil layer (kg/m2/s)
     &  FRAC(LAND,NTYPE),         ! IN Tile fractions
...
!                                 !    (kg/m2/s)
     ...
     &  SNOW_FRAC(LAND),          ! IN Fraction of snow cover.
     &  SNOW_SUBLIMATION(LAND),   ! IN Sublimation of snow (kg/m2/s)
     &  SOIL_SURF_HTF(LAND),      ! IN Net downward surface heat flux
!                                 !    (W/m2)  - snow-free land.
     &  SNOW_SURF_HTF(LAND),      ! IN Net downward surface heat flux
!                                 !    (W/m2)  - snow.
     &  TSTAR_SNOW(LAND),         ! IN Snow surface temperature (K)
...
!                                 !       of saturation
     ...
     &  TSNOW(LAND)               ! INOUT Snow layer temperature (K)
!----------------------------------------------------------------------
! OUT Variables
!----------------------------------------------------------------------
      REAL
...
!
!----------------------------------------------------------------------
! Local variable requiring inclusion in STASH in a future release
! (so will become intent OUT when fixed)
!----------------------------------------------------------------------
      REAL
     &  CAN_WCNT_GB(LAND)         ! Gridbox canopy water content
!                                 ! (Kg/m2)
...
!----------------------------------------------------------------------
       REAL
     &  CAN_CPY(LAND),
     &  CAN_WCNT(LAND), ! Ex-prognostic is now diagnostic CAN_WCNT_GB
     &  CANOPY_EVAPORATION(LAND),
     &  INFIL_FAC(LAND),
     &  LAYER_DEPTH(SM_LEVELS),
     &  ROOTD(LAND),
     &  SOILB,
     &  SOIL_EVAPORATION(LAND),
     &  SURF_HT_FLUX(LAND),
     &  TSTAR(LAND),
     &  VFRAC(LAND)
...
!
      CALL HYDROL(
     &     LICE_PTS,LICE_INDEX,SOIL_PTS,NTYPE,SOIL_INDEX,
     &     TILE_PTS,TILE_INDEX,LAND,SM_LEVELS,
     &     B_EXP,CAN_CPY_TILE,
     &     CONV_RAIN,CONV_SNOW,CAN_EVAP_TILE,
     &     EXT,HCAP,HCON,LS_RAIN,LS_SNOW,
     &     SATCON,SATHH,SNOW_SUBLIMATION,
     &     SOIL_SURF_HTF,SNOW_SURF_HTF,TSTAR_SNOW,
     &     FRAC,SNOW_FRAC,TIMESTEP,VSAT,VWILT,
     &     CAN_WCNT_TILE,
     &     HF_SNOW_MELT,
     &     STF_HF_SNOW_MELT,RGRAIN,L_SNOW_ALBEDO,SMCL,
     &     STHF,STHU,SNODEP,T_DEEP_SOIL,TSNOW,
