...
!LL  System task:
!LL
!LL  Documentation: Unified Model Documentation Paper No 26: Eq 26.29.
!LL
...
!-----------------------------------------------------------------------
        IF (TFOC .LE. (TM - 15.0))  THEN
!       Low temperatures, cloud water all frozen------------------------
          ROCWWIL(I) = 0.0
!
        ELSE IF (TFOC .LT. (TM - 5.0))  THEN
!       Intermediate temperatures---------------------------------------
          ROCWWIL(I) = (TFOC - TM + 15.0) * (TFOC - TM + 15.0) / 150.0
!
        ELSE IF (TFOC .LT. TM)  THEN
!       Intermediate temperatures---------------------------------------
          ROCWWIL(I) = 1.0 - ((TFOC - TM) * (TFOC - TM) / 75.0)
!
        ELSE
