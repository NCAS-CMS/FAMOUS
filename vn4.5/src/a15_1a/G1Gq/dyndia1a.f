...
C    DEFINE LOCAL VARIABLES
     ...
     *, WORK2(U_FIELD)        ! Work array
     *, WORK3(U_FIELD)        ! Work array
     *, WORK4(U_FIELD)        ! Work array
     *, COEFF1(U_FIELD)       ! Rotation coeffs
     *, COEFF2(U_FIELD)       ! Rotation coeffs
C
C R IS GAS CONSTANT FOR DRY AIR
C CP IS SPECIFIC HEAT OF DRY AIR AT CONSTANT PRESSURE
C PREF IS REFERENCE SURFACE PRESSURE
...
CL------------------Calculate rotation coeffs if required -------------
      IF(QVCOMP_P.OR.QUCOMP_P.OR.QMAX_WIND_PRESSURE) THEN
        IF(ROTATE_UV.OR.ROTATE_MAX_UV) THEN
          K=0
          DO I=1,U_ROWS
            DO J=1,ROW_LENGTH
              K=K+1
C Calculate relative to NW corner of LAM grid
              WORK1(K)=NMOST_LAT - 
     &          (I-1-NS_Halo+datastart(2)-0.5)*NS_SPACE ! Equatorial Phi
              WORK2(K)=WMOST_LONG+ 
     &          (J-1-EW_Halo+datastart(1)-0.5)*EW_SPACE
              IF(WORK2(K).GT.360.0) THEN
                WORK2(K)=WORK2(K)-360.0
              ENDIF
            ENDDO
          ENDDO
C  WORK3 holds true PHI and WORK4 holds true LAMBDA
          CALL EQTOLL(WORK1,WORK2,WORK3,WORK4,PHI_POLE,LAMBDA_POLE,
     *    U_FIELD)
          DO I=1,U_FIELD
            IF(WORK4(I).GT.180.0) WORK4(I)=WORK4(I)-360.0
          ENDDO
          CALL W_COEFF(COEFF1,COEFF2,WORK4,WORK2,PHI_POLE,LAMBDA_POLE,
     *    U_FIELD)
        ENDIF
      ENDIF
...
C-----------------------------------------------------------------------
      IF(QUCOMP_P.OR.QVCOMP_P.OR.QZ_P) THEN 
        DO I=1,P_FIELD
          PHI_STAR(I)=OROG(I)*G
        ENDDO
CL------------------Test to see if winds are to be rotated-------------
        IF(ROTATE_UV) THEN
          TEST=.TRUE.
          IF(UCOMP_P_LEVS.EQ.VCOMP_P_LEVS) THEN
            DO K=1,UCOMP_P_LEVS
              IF(UCOMP_PRESS(K).NE.VCOMP_PRESS(K)) TEST=.FALSE.
            ENDDO
          ELSE
            TEST=.FALSE.
          ENDIF
          IF(TEST) THEN
            DO K=1,UCOMP_P_LEVS
              DO I=FIRST_FLD_PT,LAST_U_FLD_PT
                PZ(I)=UCOMP_PRESS(K)*100.0
              ENDDO
              CALL V_INT(PUV,PZ,U,WORK1,U_FIELD,P_LEVELS,
     &        UCOMP_MAX_WIND,MAX_WIND_PRESSURE,.FALSE.
     &        ,FIRST_FLD_PT,LAST_U_FLD_PT)
              CALL V_INT(PUV,PZ,V,WORK2,U_FIELD,P_LEVELS,
     &        VCOMP_MAX_WIND,MAX_WIND_PRESSURE,.FALSE.
     &        ,FIRST_FLD_PT,LAST_U_FLD_PT)
              CALL W_EQTOLL(COEFF1,COEFF2,WORK1(FIRST_FLD_PT)
     &          ,WORK2(FIRST_FLD_PT),UCOMP_P(FIRST_FLD_PT,K)
     &          ,VCOMP_P(FIRST_FLD_PT,K),U_FLD_VALID,U_FLD_VALID)
            ENDDO  ! Levels loop
          ELSE  ! Test is false UCOMP_LEVS and VCOMP_LEVS are different
            IF(QUCOMP_P) THEN
              DO K=1,UCOMP_P_LEVS
                DO I=FIRST_FLD_PT,LAST_U_FLD_PT
                  PZ(I)=UCOMP_PRESS(K)*100.0
     ...
     &          ,FIRST_FLD_PT,LAST_U_FLD_PT)
                CALL W_EQTOLL(COEFF1,COEFF2,WORK1(FIRST_FLD_PT)
     &            ,WORK2(FIRST_FLD_PT),UCOMP_P(FIRST_FLD_PT,K),WORK3
     &            ,U_FLD_VALID,U_FLD_VALID)
              ENDDO  ! Levels loop
            ENDIF
            IF(QVCOMP_P) THEN
              DO K=1,VCOMP_P_LEVS
                DO I=FIRST_FLD_PT,LAST_U_FLD_PT
                  PZ(I)=VCOMP_PRESS(K)*100.0
                ENDDO
                CALL V_INT(PUV,PZ,U,WORK1,U_FIELD,P_LEVELS,
     &          UCOMP_MAX_WIND,MAX_WIND_PRESSURE,.FALSE.
     &          ,FIRST_FLD_PT,LAST_U_FLD_PT)
     ...
     &          ,FIRST_FLD_PT,LAST_U_FLD_PT)
                CALL W_EQTOLL(COEFF1,COEFF2,WORK1(FIRST_FLD_PT)
     &            ,WORK2(FIRST_FLD_PT),WORK3,VCOMP_P(FIRST_FLD_PT,K)
     &            ,U_FLD_VALID,U_FLD_VALID)
              ENDDO  ! Levels loop
            ENDIF
          ENDIF  ! End of TEST IF block
C
        ELSE     ! Winds not to be rotated--------------------------
C
...
CL------------------Calculate the maximum wind-------------------------
     ...
     *     WORK1,WORK2,MAX_WIND_PRESSURE)
          CALL W_EQTOLL(COEFF1,COEFF2,WORK1(FIRST_FLD_PT)
     &      ,WORK2(FIRST_FLD_PT),UCOMP_MAX_WIND(FIRST_FLD_PT)
     &      ,VCOMP_MAX_WIND(FIRST_FLD_PT),U_FLD_VALID,U_FLD_VALID)
        ELSE  !  Do not rotate
          CALL WINDMAX(
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
