...
C DEFINE LOCAL SCALARS
     ...
     &  SCALAR
      ...
      REAL SUM_N(P_LEVELS),SUM_S(P_LEVELS)
...
C CALCULATE AVERAGE OF DV/D(PHI), STORE IN WORK3.
        DO I=ROW_LENGTH+2,U_FIELD
          WORK3(I) = WORK2(I) + WORK2(I-1)
        END DO
...
C CALCULATE DIVERGENCE AT POLES BY SUMMING DV/D(LAT) AROUND POLAR
C CIRCLE AND AVERAGING.
     ...
     &    /GLOBAL_ROW_LENGTH)
        ...
        IF (at_top_of_LPG) THEN
          DO I=TOP_ROW_START+EW_Halo,START_POINT_NO_HALO-1-EW_Halo
            SUM_N_ARRAY(I-TOP_ROW_START,K)=
     &        -(V(I,K)*DELTA_P(I))*(COS_U_LATITUDE(I)*SCALAR)
          ENDDO
        ENDIF
        IF (at_base_of_LPG) THEN
          DO I=U_BOT_ROW_START+EW_Halo,LAST_U_FLD_PT-EW_Halo
            SUM_S_ARRAY(I-U_BOT_ROW_START,K)=
     &        (V(I,K)*SCALAR)*(DELTA_P(I)*COS_U_LATITUDE(I))
          ENDDO
        ENDIF
 ...
 100  CONTINUE
     ...
     &    EW_Halo,P_LEVELS,SUM_N_ARRAY,GC_ROW_GROUP,info,SUM_N)
      ENDIF
      IF (at_base_of_LPG) THEN
        CALL GCG_RVECSUMR(ROW_LENGTH,ROW_LENGTH-2*EW_Halo,
     &    EW_Halo,P_LEVELS,SUM_S_ARRAY,GC_ROW_GROUP,info,SUM_S)
      ENDIF

      DO K=1,P_LEVELS
C DIVERGENCE AT POLES
        IF (at_top_of_LPG) THEN
          ETADOT(START_POINT_NO_HALO+EW_Halo-1,K) = SUM_N(K)
        ENDIF
        IF (at_base_of_LPG) THEN
          ETADOT(END_P_POINT_NO_HALO-EW_Halo+1,K) = SUM_S(K)
        ENDIF
...
C IF GLOBAL PERFORM SECTION 2 OVER POLES AS WELL AS OTHER POINTS.
C ONLY ONE POLAR POINT NEEDS TO BE DONE.
      START = START_POINT_NO_HALO+EW_Halo
      END = END_P_POINT_NO_HALO-EW_Halo
C At poles, extra point holds polar values
      IF (at_top_of_LPG)  START = START-1
      IF (at_base_of_LPG) END = END+1
...
C MPP: Above, polar values stored in START and END points
        IF (at_top_of_LPG) THEN
          DO I=TOP_ROW_START+EW_Halo,START-1
            ETADOT(I,K)=ETADOT(START,K)
          ENDDO
        ENDIF
        IF (at_base_of_LPG) THEN
          DO I=END+1,LAST_P_FLD_PT-EW_Halo
            ETADOT(I,K)=ETADOT(END,K)
          ENDDO
        ENDIF
 230  CONTINUE
