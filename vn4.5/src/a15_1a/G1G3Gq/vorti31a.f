...
C Calculate d(theta)/d(phi).
 ...
 120    continue
        ...
        dtheta_dlongitude(i)=dtheta_dlongitude(i+1)

        i=END_P_POINT_NO_HALO
        dtheta_dlongitude(i)=dtheta_dlongitude(i-1)

C Calculate vorticity3.

        do 140 j=START_POINT_NO_HALO,END_P_POINT_NO_HALO
...
C   Zero vorticity3 at the poles.
       do i=1,row_length
       if (at_top_of_LPG) vorticity3(TOP_ROW_START+i)=0.0
       if (at_base_of_LPG)vorticity3(P_BOT_ROW_START+i)=0.0
       end do
