...
C
C First row of southern hemisphere relative to global grid
      S_HEM_TOP=(GLSIZE-1)/2+1
C Loop over all local rows, convert to global grid and check if
C in Southern hemisphere
      DO I=FIRST_ROW,U_LAST_ROW+1
        GLOBAL_ROW_NO=I+FIRST_GLOBAL_ROW_NUMBER-1-NS_Halo
        IF (GLOBAL_ROW_NO.GE.S_HEM_TOP) THEN
          DO K=(I-1)*ROW_LENGTH+1,I*ROW_LENGTH
            HORIZ_SHEAR(K)=-HORIZ_SHEAR(K)
          ENDDO
        ENDIF
      ENDDO
C=======================================================================
