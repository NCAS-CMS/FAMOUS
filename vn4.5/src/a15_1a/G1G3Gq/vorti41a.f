...
C Calculate vorticity4 at poles by summing d(theta)/d(lat) around polar
C circle and averaging.
     ...
     &      theta_on_press(TOP_ROW_START+FIRST_ROW_PT-1+ROW_LENGTH),
     ...
     &                      2.*omega*sum_n
        ...
        if (at_base_of_LPG) then
          ...
          CALL GCG_RVECSUMR(
     &      ROW_LENGTH-2*EW_Halo,ROW_LENGTH-2*EW_Halo,1,1,
     &      theta_on_press(P_BOT_ROW_START-row_length+LAST_ROW_PT-1),
