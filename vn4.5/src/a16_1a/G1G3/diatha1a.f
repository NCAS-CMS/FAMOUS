...
C
     ...
     *  -T_P(I+ROW_LENGTH))*LAT_SI_OVER_TWO_A
      ...
      ENDDO
C-----------------------------------------------------------------------
C         Recalculate terms for end of each row in the Global model
C-----------------------------------------------------------------------
      DO I=ROW_LENGTH,U_FIELD,ROW_LENGTH
        TERM1=U_P(I)*(T_P(I+1-ROW_LENGTH)+T_P(I+1)-T_P(I+ROW_LENGTH)
     *  -T_P(I))*SEC_U_LATITUDE(I)*LONG_SI_OVER_TWO_A
C
        TERM2=V_P(I)*(T_P(I)+T_P(I+1-ROW_LENGTH)-T_P(I+1)
     *  -T_P(I+ROW_LENGTH))*LAT_SI_OVER_TWO_A
        TH_UV(I)=-(TERM1+TERM2)
      ENDDO
