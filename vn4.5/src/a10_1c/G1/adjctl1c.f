...
CLL   4.4   11/08/97  Remove extra swapbound by transferring it from U
CLL                   and V to RECIP_RS*DELTAP_UV array. Need to
CLL                   initialise variables such as V_MEAN to zero
CLL                   removed. T3E version of P_EXNER calculation
CLL                   reworked for efficiency gains.
CLL                   A. Dickinson
!LL   4.5    21/08/98  Comment out cdir$ cache_bypass directives due
!LL                    to t3e hardware error with new compiler.
!LL                    S.D.Mullerworth
!LL   4.5    28/10/98  Corrected error in loop bounds for non-T3E code
...
C    DEFINE LOCAL ARRAYS: 6 ARE REQUIRED IF TIME SMOOTHING
     ...
     *                                   ! MODEL SURFACE.
     *,RECIP_RS_DELTAP_UV(U_FIELD,P_LEVELS) ! 1./RS*DELTAP AT UV POINTS
...
CL    IF LIMITED AREA SET U,V AT END OF ROW EQUAL TO U,V 1 GRID-LENGTH
CL    TO THE LEFT.

      IF (at_right_of_LPG) THEN  ! only do this at the RHS of the LPG
        DO K=1,P_LEVELS
          DO I=FIRST_FLD_PT-1+LAST_ROW_PT , LAST_U_FLD_PT , ROW_LENGTH
!           Loop over the right-hand column of the field
            U(I,K) = U(I-1,K)
            V(I,K) = V(I-1,K)
          ENDDO
        ENDDO
      ENDIF ! if this processor is at the RHS of the LPG
...
! Initialise RS_DELTAP prior to calling P_TO_UV
! cdir$ cache_bypass rs_deltap
...
! loop over "local" points - not including top and bottom halos
          DO 320 I= FIRST_FLD_PT,LAST_U_FLD_PT
            ...
            RECIP_RS_DELTAP_UV(I,K)=RECIP_RS_DELTAP
 320      CONTINUE
...
! loop over all points, including valid halos
          DO I = 1,U_FIELD
            U_MEAN(I,K)=  U(I,K) * RECIP_ADJUSTMENT_STEPS
            V_MEAN(I,K)=  V(I,K) * RECIP_ADJUSTMENT_STEPS
          ENDDO
        ENDDO
...
! loop over "local" points - not including top and bottom halos
          DO 620 I= FIRST_VALID_PT,LAST_U_VALID_PT
            U(I,K) = U(I,K)*RECIP_RS_DELTAP_UV(I,K)
            V(I,K) = V(I,K)*RECIP_RS_DELTAP_UV(I,K)*SEC_U_LATITUDE(I)
 620      CONTINUE
...
! loop over all points, including valid halos
            ...
            P_EXNER(I,K)=((AKH(K)+BKH(K)*PSTAR(I))*RECIP_PREF)**KAPPA
            END DO
