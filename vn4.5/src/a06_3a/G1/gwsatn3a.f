...
!  4.0   04/09/95   Mod to prevent floating point exception caused
!                   by divide by zero especially when run at      
!                   32 bit precision. (J.Clifton)                 
!                   Remove line of redundant code. (N.Farnon)     
!
!  4.1   27/03/96   Mod to ensure that x,y components of the      
!                   critical stress vector are correctly defined  
!                   (S.Webster)                                   
!
...
!----------------------------------------------------------------
      ...
      DO I=1,POINTS
        ...
        ELSE IF( H_JUMP(I) ) THEN
          X_STRESS(I,KL) = S_X_STRESS(I)/3.0
          Y_STRESS(I,KL) = S_Y_STRESS(I)/3.0
          H_O_L=H_O_LEV(I)
