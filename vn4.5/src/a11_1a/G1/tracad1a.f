...
C LOOP OVER ALL POINTS.
          ...
          DO I=START_P_UPDATE+1,END_P_UPDATE-1
     ...
     &              ABS_COURANT(I+II))
          ...
          B2(START_P_UPDATE)=0.
          B2(END_P_UPDATE)=0.
          B_SWITCH(START_P_UPDATE) = 0.
          B_SWITCH(END_P_UPDATE) = 0.
C For Limited Area Model, set W & E edge values of B2 to zero
C since these may involve wraparound values of flux_delta_t etc.
C Because B2 is on u_grid, also need to zero first interior E points.
      if (atleft) then
      do i = START_P_UPDATE,END_P_UPDATE,row_length
        B2(i+EW_Halo) = 0.0
      end do
      end if
      if (atright) then
      do i = START_P_UPDATE,END_P_UPDATE,row_length
        B2(i+row_length-EW_Halo-1) = 0.0
        B2(i+row_length-EW_Halo-2) = 0.0
      end do
      end if
...
C ADD INCREMENTS TO FIELD
        J1 = 1
        J2 = ROW_LENGTH-2
          if (atleft) J1 = J1+EW_Halo
          if (atright) J2 = J2-EW_Halo
CFPP$ NODEPCHK
          DO 150 J=START_P_UPDATE,END_P_UPDATE,ROW_LENGTH
            DO I = J1,J2
              FIELD(J+I) = FIELD(J+I)+FIELD_INC(J+I)*
     &                     RS_SQUARED_DELTAP_RECIP(J+I)
            END DO
 150      CONTINUE
...
! For limited area or MPP just make sure have sensible value
      COURANT_MW(FIRST_VALID_PT) = V_MEAN(FIRST_VALID_PT)*
...
C ADD INCREMENTS TO FIELD
      ...
      J2 = ROW_LENGTH-2
          ...
          if (atright) J2 = J2-EW_Halo
CFPP$ NODEPCHK
      DO 270 J = START_POINT_NO_HALO,END_P_POINT_NO_HALO,ROW_LENGTH
        DO I = J1,J2
          FIELD(J+I) = FIELD(J+I)+FIELD_INC(J+I)*SEC_P_LATITUDE(J+I)/
     &                 RS_SQUARED_DELTAP(J+I)
        END DO
 270  CONTINUE
