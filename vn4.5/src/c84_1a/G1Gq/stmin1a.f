...
CLL  Routine: STMIN ----------------------------------------------------
CLL
CLL  Purpose: Computes the point-by-point minimum in time of a field
CLL           by comparing the field at the current time with the
CLL           minimum so far (STASH TEMPORAL service routine)
CLL
...
C
      SUBROUTINE STMIN(fieldin,result,size,masking,amdi)
C
      IMPLICIT NONE
C
      INTEGER size             ! IN size of fieldin and result.
      REAL fieldin(size)       ! IN input field
      REAL result(size)        ! OUT output field (minimum)
...
CL     result to amdi, else set result to minimum of fieldin and result
CL
      IF (masking) THEN
        DO i=1,size
      ...
      ELSE
CL
CL 1.2 loop over array size, set result to minimum of fieldin and result
