...
C ******************************COPYRIGHT******************************
C
CLL Subroutine SET_PSEUDO_LIST -----------------------------------------
CLL
CLL Purpose : To set up a list of pseudo levels at which a diagnostic
CLL           is required, using information in the STASH list.
CLL
CLL Service routine
CLL
CLL Written by D Robinson 7/10/92
CLL Copy of Subroutine SET_LEVELS_LIST (Deck SETLST1) taken and
CLL adapted for pseudo levels.
CLL
CLL  Model            Modification history from model version 3.0:
CLL version  Date
CLL 3.1    12/1/93   : More error checking . Dave Robinson
CLL   3.2    13/07/93 Changed CHARACTER*(*) to CHARACTER*(80) for
CLL                   portability.  Author Tracey Smith.
CLL
CLL Programming Standard : Unified Model Documentation paper number 3
CLL                      : Version no 4, dated 5/2/92
CLL
CLL Logical components covered : D3
CLL
CLL System task : P0
CLL
...
C*L Arguments
     ...
     &      STASH_PSEUDO_LEVELS,NUM_STASH_PSEUDO,ICODE,CMESSAGE)
      ...
      INTEGER
     &   N_LEVELS            ! IN Number of possible pseudo levels
     &,  LEN_STLIST          ! IN Dimension of STLIST
     &,  STLIST(LEN_STLIST)  ! IN STASH list
     &,  NUM_STASH_PSEUDO    ! IN Dimension for STASH_PSEUDO_LEVELS
     &,  STASH_PSEUDO_LEVELS(NUM_STASH_PSEUDO+1,*) ! IN Pseudo levels
...
C*L   Workspace Usage :- None

C*L   External Subroutines called :- None
...
C  Local variables
      ...
      INTEGER
     &      JLEV       ! Loop counter over levels
     &,     LEVEL_NO   ! Level no in pseudo list
     &,     LIST_NO    ! Pseudo level list number

C* ---------------------------------------------------------------------

CL Initialise pseudo levels list to false

      DO JLEV=1,N_LEVELS
        PSEUDO_LIST(JLEV)= .FALSE.
      END DO

CL Get pseudo list number

      LIST_NO = STLIST(ST_PSEUDO_IN)

CL Check that Pseudo list number is valid (should be GE 0)

      IF (LIST_NO.LT.0) THEN

C       Illegal control data
...
C  Set logical array list to identify pseudo levels required.

      IF (LIST_NO.GT.0) THEN

        DO JLEV=2,STASH_PSEUDO_LEVELS(1,LIST_NO)+1
          LEVEL_NO = STASH_PSEUDO_LEVELS(JLEV,LIST_NO)
          IF (LEVEL_NO.GE.1 .AND. LEVEL_NO.LE.N_LEVELS) THEN

C           Level is within range
            PSEUDO_LIST(LEVEL_NO) =.TRUE.

          ELSE

C           Level is out of range
            ICODE=2
