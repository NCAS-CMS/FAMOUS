...
CLL  Routine: STZONM ---------------------------------------------------
CLL
CLL  Purpose: Calculate weighted zonal mean within a region specified
...
!LL                  SPATIAL.
!LL                  Significantly rewritten for MPP mode - zonal
...
!LL                  subdomain area which will not otherwise
!LL                  receive the result of the zonal mean.  P.Burton
...
CLL
CLL  Logical components covered: D713
CLL
CLL  Project task: D7
CLL
...
C
      SUBROUTINE STZONM(fieldin,vx,vy,st_grid,gr,lwrap,lmasswt,
...
C
     ...
     &    fieldout(ystart:yend),                ! OUT output field
...
! we're interested only in the end segment
            ...
            local_send_size_x=local_data_bot_right_x-Offx
...
! all other processors
     ...
     &        global_xstart
          ...
          IF (pos_in_zonal_array .LT. 1) THEN
...
! the correct position in the array we add the global row length
            pos_in_zonal_array=pos_in_zonal_array+glsize(1)
          ENDIF
...
!         zonal mean value for this row to be returned.
              ...
              IF (proc_x .EQ. proc_top_left_x) THEN
