...
CLL -----------------------------------------------------------
CLL Stash routine
CLL purpose: Generate extra data for the timeseries.
CLL This extra data provides information about what processing was done
CLL to produce the timeseries. This information will hopefully be of som
CLL use to users doing further processing of the timeseries data.
CLL This deck contains two subroutines
CLL (1) EXTRA_TS_INFO : which generates the codes and sets up the space
CLL                   : for the extra data.
CLL
CLL (2) EXTRA_MAKE_VECTOR: which computes the long/latt ht domain info
CLL                   : and puts that into the correct place in the
CLL                   : extra data
CLL Routines are  called by stmulspa1.
CLL
CLL To some extent this routine has much in common with the
CLL multi_spatial routine but as it has a different function
CLL viz generate info on timeseries rather than generating a single time
CLL for the timeseries it is coded separately.
CLL when modifying multi_spatial be sure also to modify this routine and
CLL vice versa
CLL
CLL 16/3/92 Written by Simon Tett
CLL
CLL  Model            Modification history from model version 3.0:
CLL version  date
CLL
CLL Programming Standard: UM DOC Paper3, Verion 4 (05/02/92)
CLL
CLL System Component Covered: D711
CLL
CLL System Task:C4
CLL
...
C order is lat, long, 2nd lat, 2nd long, first level, 2nd level
...
CL routine to put an integer (or other single word) into a real variable
CL    through hidden equivalencing via argument passing
      real array_out
      real data_in
      array_out=data_in
      RETURN
