...
CLL  Routine: STGLOM ---------------------------------------------------
CLL
CLL  Purpose: Calculate weighted global mean within a region specified
CLL           by a lower left hand and upper right hand corner.
CLL           Multiple level fields.
...
CLL
CLL  Logical components covered: D716
CLL
CLL  Project task: D7
CLL
...
C
      SUBROUTINE STGLOM(fieldin,vx,vy,vz,st_grid,gr,lwrap,lmasswt,
...
C
     ...
     &    index_lev(zsize),          ! IN offset for each horiz lev
...
C Local variables
C
        INTEGER i,ii,j,k,kk ! ARRAY INDICES FOR VARIABLE
        integer level ! value of level
        ...
        DO kk=1,zsize
          ...
          DO i=xstart,xend
            ...
            DO j=ystart,yend
              ...
              IF (mask(ii,j)) THEN
                IF (.NOT. lmasswt) THEN
                  local_sum_array_bot(i,j)=local_sum_array_bot(i,j)+
     &              pstar_weight(ii,j)*area_weight(ii,j)
                  local_sum_array_top(i,j)=local_sum_array_top(i,j)+
     &              fieldin(ii,j,k)*pstar_weight(ii,j)*
     &              area_weight(ii,j)
                ELSE
                  local_sum_array_bot(i,j)=local_sum_array_bot(i,j)-
     &              (delta_ak(level)+delta_bk(level)*
     &              pstar_weight(ii,j))*area_weight(ii,j)
                  local_sum_array_top(i,j)=local_sum_array_top(i,j)-
     &              fieldin(ii,j,k)*
     &              (delta_ak(level)+delta_bk(level)*
      ...
      IF (mype .EQ. sum_pe) THEN
        ...
        DO i=global_xstart,global_xend
          DO j=global_ystart,global_yend
            SUMGTOP=SUMGTOP+global_sum_array_top(i,j)
            SUMGBOT=SUMGBOT+global_sum_array_bot(i,j)
          ENDDO
        ENDDO

        IF (SUMGBOT .EQ. 0.0) THEN
          fieldout=rmdi
        ELSE
