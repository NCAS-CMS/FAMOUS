...
CLL VERSION  DATE
CLL   4.2   1/11/96   New deck version based on CONVEC2A with HADCM2
...
CLL   4.3   12.5.97   Correction to argument list for consistency with
CLL                   a change to the glue routine at 4.2
!LL  4.4  Oct 97    Add halo mask to stop redundant calculations
!LL                                               Alan Dickinson
!LL  4.5  20/02/98  Remove redundant code. A. Dickinson               
!LL       23/02/98  Call comdecks. D. Robinson.
CLL
...
C
C     Dummy diagnostics expected by 2A glue routine but not calculated
C     by this version of convection
C
      REAL ICCBPxCCA(NP_FIELD)    ! dummy
      REAL ICCTPxCCA(NP_FIELD)    ! dummy
      REAL GBMCCWP(NP_FIELD)      ! dummy
      REAL GBMCCW(NP_FIELD,NLEV)  ! dummy
C
C----------------------------------------------------------------------
...
C
C----------------------------------------------------------------------
C NOTE THAT THE BELOW WORK SPACE ALLOCATION ASSUMES THAT LEVPAR>=11
C----------------------------------------------------------------------
C
