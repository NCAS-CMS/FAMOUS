...
!!!
!!!
!!!  Programming standard: UM Documentation Paper No 4
!!!
...
!                                in common deck C_GAMMA
     &,NTML(P_FIELD)          ! IN Number of model layers in turbulent
!                                  mixing layer.
...
!                                     K-1 to level K.
     &, GRAD_T_ADJ(P_FIELD)      ! IN Temperature gradient adjustment
!                                     for non-local mixing in unstable
!                                     turbulent boundary layer.
     &, GRAD_Q_ADJ(P_FIELD)      ! IN Humidity gradient adjustment
!                                     for non-local mixing in unstable
!                                     turbulent boundary layer.
...
!
!         !-------------------------------------------------------------
!         !  Add gradient adjustment terms to fluxes within the
!         !  mixing layer.
!         !-------------------------------------------------------------
          IF (K .LE. NTML(L) ) THEN
            FTL(L,K) = FTL(L,K)
     &                 + RHOKHZ(L,K) * GRAD_T_ADJ(L)
            FQW(L,K) = FQW(L,K)
     &                 + RHOKHZ(L,K) * GRAD_Q_ADJ(L)
