...
!     TEMPORARY FLUXES
      REAL
     &     FLUX_UP(3,NPD_PROFILE,0:NPD_LAYER)
     &   , FLUX_DOWN(3,NPD_PROFILE,0:NPD_LAYER)
...
!     THUS THE FLUXES IMPINGING ON THE LAYER ARE HELD.
      I=N_CLOUD_TOP-1
      DO L=1, N_PROFILE
         FLUX_DOWN(1,L, I)=V11(L, I)*FLUX_TOTAL(L, 2*I+2)
         FLUX_DOWN(2,L, I)=V21(L, I)*FLUX_TOTAL(L, 2*I+2)
         FLUX_DOWN(3,L, I)=V31(L, I)*FLUX_TOTAL(L, 2*I+2)
      ENDDO
!
...
!           PROPAGTE THE FLUX ACROSS THE LAYER.
         ...
         DO L=1, N_PROFILE
            FLUX_PROPAG_1(L)=T(L, I)*FLUX_DOWN(1,L, I-1)
            FLUX_PROPAG_2(L)=T_STRAT(L, I)*FLUX_DOWN(2,L, I-1)
            FLUX_PROPAG_3(L)=T_CONV(L, I)*FLUX_DOWN(3,L, I-1)
         END DO

         DO L=1, N_PROFILE
            FLUX_PROPAG_1(L) =  FLUX_PROPAG_1(L)
     &         +S_DOWN(L, I)
            FLUX_PROPAG_2(L) =  FLUX_PROPAG_2(L)
     &         +S_DOWN_STRAT(L, I)
         END DO
         DO L=1, N_PROFILE
            FLUX_PROPAG_3(L) =  FLUX_PROPAG_3(L)
     &         +S_DOWN_CONV(L, I)
         END DO
!
!           TRANSFER ACROSS THE INTERFACE.
         ...
         DO L=1,N_PROFILE
            FLUX_DOWN(1,L, I)=V11(L, I)*FLUX_PROPAG_1(L)
            FLUX_DOWN(2,L, I)=V21(L, I)*FLUX_PROPAG_1(L)
            FLUX_DOWN(3,L, I)=V31(L, I)*FLUX_PROPAG_1(L)
         END DO

         DO L=1,N_PROFILE
            FLUX_DOWN(1,L,I) =  FLUX_DOWN(1,L,I)
     &           +V12(L, I)*FLUX_PROPAG_2(L)
            FLUX_DOWN(2,L,I) =  FLUX_DOWN(2,L,I)
     &           +V22(L, I)*FLUX_PROPAG_2(L)
            FLUX_DOWN(3,L,I) =  FLUX_DOWN(3,L,I)
...
!        PROPAGTE THE FLUX THROUGH THE LAYER.
         FLUX_DOWN(1,L, N_LAYER)
     &      =T(L, N_LAYER)*FLUX_DOWN(1,L, N_LAYER-1)
         FLUX_DOWN(2,L, N_LAYER)
     &      =T_STRAT(L, N_LAYER)*FLUX_DOWN(2,L, N_LAYER-1)
         FLUX_DOWN(3,L, N_LAYER)
     &      =T_CONV(L, N_LAYER)*FLUX_DOWN(3,L, N_LAYER-1)
      END DO
      DO L=1,N_PROFILE
         FLUX_DOWN(1,L, N_LAYER)=FLUX_DOWN(1,L, N_LAYER)
     &      +S_DOWN(L, N_LAYER)
         FLUX_DOWN(2,L, N_LAYER)=FLUX_DOWN(2,L, N_LAYER)
     &      +S_DOWN_STRAT(L, N_LAYER)
         FLUX_DOWN(3,L, N_LAYER)=FLUX_DOWN(3,L, N_LAYER)
...
!        REFLECT FROM THE SURFACE.
         FLUX_UP(1,L, N_LAYER)
     &      =ALBEDO_SURFACE_DIFF(L)*FLUX_DOWN(1,L, N_LAYER)
         FLUX_UP(2,L, N_LAYER)
     &      =ALBEDO_SURFACE_DIFF(L)*FLUX_DOWN(2,L, N_LAYER)
         FLUX_UP(3,L, N_LAYER)
     &      =ALBEDO_SURFACE_DIFF(L)*FLUX_DOWN(3,L, N_LAYER)
      END DO

      DO L=1,N_PROFILE
         FLUX_UP(1,L, N_LAYER)= FLUX_UP(1,L, N_LAYER)
     &      +SOURCE_GROUND_FREE(L)
         FLUX_UP(2,L, N_LAYER)= FLUX_UP(2,L, N_LAYER)
     &      +SOURCE_GROUND_STRAT(L)
         FLUX_UP(3,L, N_LAYER)= FLUX_UP(3,L, N_LAYER)
...
!        PROPAGATE ACROSS THE BOTTOM LAYER.
         FLUX_PROPAG_1(L)
     &      =T(L, N_LAYER)*FLUX_UP(1,L, N_LAYER)
         FLUX_PROPAG_2(L)
     &      =T_STRAT(L, N_LAYER)*FLUX_UP(2,L, N_LAYER)
         FLUX_PROPAG_3(L)
     &      =T_CONV(L, N_LAYER)*FLUX_UP(3,L, N_LAYER)
      END DO
     ...
     &      +S_UP_STRAT(L, N_LAYER)
      ...
      DO L=1,N_PROFILE
         FLUX_PROPAG_1(L)=FLUX_PROPAG_1(L)
     &        +R(L, N_LAYER)*FLUX_DOWN(1,L, N_LAYER-1)
         FLUX_PROPAG_2(L)=FLUX_PROPAG_2(L)
     &        +R_STRAT(L, N_LAYER)*FLUX_DOWN(2,L, N_LAYER-1)
         FLUX_PROPAG_3(L)=FLUX_PROPAG_3(L)
     &        +R_CONV(L, N_LAYER)*FLUX_DOWN(3,L, N_LAYER-1)
      END DO
...
!
         DO L=1, N_PROFILE
            FLUX_UP(1,L, I)=U11(L, I)*FLUX_PROPAG_1(L)
            FLUX_UP(2,L, I)=U21(L, I)*FLUX_PROPAG_1(L)
            FLUX_UP(3,L, I)=U31(L, I)*FLUX_PROPAG_1(L)
         END DO

         DO L=1, N_PROFILE
            FLUX_UP(1,L,I)= FLUX_UP(1,L,I)
     &         +U12(L, I)*FLUX_PROPAG_2(L)
            FLUX_UP(2,L,I)= FLUX_UP(2,L,I)
     &         +U22(L, I)*FLUX_PROPAG_2(L)
            FLUX_UP(3,L,I)= FLUX_UP(3,L,I)
...
!
         END DO
         DO L=1,N_PROFILE
            FLUX_PROPAG_1(L)=T(L, I)*FLUX_UP(1,L, I)
            FLUX_PROPAG_2(L)=T_STRAT(L, I)*FLUX_UP(2,L, I)
            FLUX_PROPAG_3(L)=T_CONV(L, I)*FLUX_UP(3,L, I)
         END DO
     ...
     &           +S_UP_CONV(L, I)
         ...
         DO L=1,N_PROFILE
            FLUX_PROPAG_1(L) = FLUX_PROPAG_1(L)
     &         +R(L, I)*FLUX_DOWN(1,L, I-1)
            FLUX_PROPAG_2(L) = FLUX_PROPAG_2(L)
     &           +R_STRAT(L, I)*FLUX_DOWN(2,L, I-1)
            FLUX_PROPAG_3(L) = FLUX_PROPAG_3(L)
     &           +R_CONV(L, I)*FLUX_DOWN(3,L, I-1)
!
         ENDDO
      ENDDO
!
...
!     ASSIGN THE TOTAL FLUXES ON THE INTERMEDIATE CLOUDY LAYERS.
      DO I=N_CLOUD_TOP, N_LAYER
         DO L=1, N_PROFILE
            FLUX_TOTAL(L, 2*I+1)=FLUX_UP(1,L, I)+FLUX_UP(2,L, I)
     &         +FLUX_UP(3,L, I)
            FLUX_TOTAL(L, 2*I+2)=FLUX_DOWN(1,L, I)+FLUX_DOWN(2,L, I)
     &         +FLUX_DOWN(3,L, I)
         ENDDO
      ENDDO
!
