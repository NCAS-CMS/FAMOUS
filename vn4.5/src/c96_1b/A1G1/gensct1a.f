...
! 4.5      15/04/98  Use local_land_field for fields on land points.
!                    D. Robinson.
!
...
C     Types of items for d1_type
     ...
     &  ADDR_INFO(D1_LIST_LEN)   ! IN: addressing info about field
      ...
      REAL
     &  GLOBAL_FIELD(GLOBAL_SIZE) ! IN:  field to scatter
...
! Local variables
      ...
      INTEGER
     &  grid_type ! grid type of field being scattered
...
! Surface (land points only) fields
     ...
     &  (grid_type .EQ. ppx_atm_compressed) THEN
        ...
        IF (mype .EQ. SCATTER_PE) THEN
          ...
          CALL FROM_LAND_POINTS(buf_expand,GLOBAL_FIELD,atmos_landmask,
...
! part of the field into the array buf_expand_local
     ...
     &                     lasize(1) , lasize(2),
     &                     glsize(1) , glsize(2),
     &                     SCATTER_PE,GC_ALL_PROC_GROUP,info)

! Fill the halo area of the local field

        CALL SWAPBOUNDS(buf_expand_local,lasize(1),lasize(2),
     &                  Offx,Offy,1)
...
! Atmosphere Lateral boundary fields
     ...
     &  (grid_type .EQ. ppx_atm_rim) THEN
        ...
        CALL SCATTER_ATMOS_LBCS(GLOBAL_FIELD,global_LENRIMDATA_A,
     &                          LOCAL_FIELD,LENRIMDATA_A,
     &                          SCATTER_PE,ICODE,CMESSAGE)
...
! Ocean Lateral boundary fields
     ...
     &  (grid_type .EQ. ppx_ocn_rim) THEN
        ...
        CALL SCATTER_OCEAN_LBCS(GLOBAL_FIELD,global_LENRIMDATA_O,
...
!       fields
     ...
     &    grid_type,SCATTER_PE,ICODE,CMESSAGE)
