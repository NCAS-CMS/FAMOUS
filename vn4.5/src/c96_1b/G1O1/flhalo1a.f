...
C ******************************COPYRIGHT******************************
!+ Parallel UM: Fills halos with number copied from inside the array
!
! Subroutine interface:
      SUBROUTINE FILL_HALOS(FIELD,P_FIELD,ROW_LENGTH,N_LEVS,FLD_TYPE)

      IMPLICIT NONE
!
! Description:
! This routine fills the halo areas of the local array with "sensible"
! numbers, to stop any references to undefined array elements
!
! Method:
! "Sensible numbers" are obtained by copying data one row/column
! in from the halo. In the case of the southern halo, data is
! copied from two rows up. On a field on a U grid, if the processor
! is at the base of the LPG, both the halo area, and the last line
! of data are set to "sensible" numbers (because U data has been
! stored on a P grid, so there is an extra row of unset data)
!
...
!  version
!    4.1    12/7/95   New DECK created for the Parallel Unified
!                     Model. P.Burton
!
...
!                                     ! sure we get some proper data
      DO K=1,N_LEVS
        IF ((FLD_TYPE .EQ. fld_type_u) .AND. atbase) THEN
          J_end=Offy+1
        ELSE
          J_end=Offy
        ENDIF
...
! Now West halo
      I_start=1
      I_end=P_FIELD-ROW_LENGTH+1
      DO K=1,N_LEVS
         DO I=I_start,I_end,ROW_LENGTH
           DO J=I, I+(Offx-1)
             FIELD(J,K)=FIELD(I+Offx,K)
           ENDDO
         ENDDO
       ENDDO


! and finally East halo
       I_start=ROW_LENGTH
       I_end=P_FIELD
       DO K=1,N_LEVS
         DO I=I_start,I_end,ROW_LENGTH
