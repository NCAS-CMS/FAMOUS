...
CLL VERSION  DATE
CLL   3.1   10/02/93  Insert missing brackets in Non-CRAY code
!     4.0   5/5/95    Removed erroneous resetting of U_MAX_OLD_SOUTH
!                     in calculation of FILTER_WAVE_NUMBER  P.Burton
CLL   4.0  1/8/95 REMOVE L_HALF_TIMESTEP_TOP
!     4.1   09/05/96  Changed structure of code to allow MPP code to be
!                     inserted.  P.Burton
!LL 4.2      16/08/96 Added TYPLDPT variables.
!LL                   Made FILTER_WAVE_NUMBER arrays globally sized.
!LL                   Removed use of filter_wave_common.
!LL                                                        P.Burton
!LL   4.2   17/10/96  Changed indexing to send/receive maps for MPP
!LL                   code, required for GCOM v1.1    P.Burton
!LL   4.3   05/02/97  Print warning if filtering entire area
!LL                   P.Burton
...
! Am I receiving it?
              IF (proc_dest .EQ. mype) THEN  ! Is this my row to fft?
                ...
                filt_level(fft_rows(fld_type),fld_type) = K
