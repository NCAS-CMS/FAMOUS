...
C DEFINE LOCAL VARIABLES
     ...
     *, RECIP_G
      INTEGER np,sp  ! points in field refering to poles
      REAL
     &  MU_NORTH_POLE(P_LEVELS)   ! MU at North Pole
     &, MU_SOUTH_POLE(P_LEVELS)   ! MU at South Pole
...
CL   END IF
CL---------------------------------------------------------------------
! Set up array of MU values at poles for use in section 3.5
! put into north_pole_mu(level) and south_pole_mu(level)
      IF (LWHITBROM) THEN
! North Pole first
        IF (MY_PROC_ID .EQ. NP_PROC_ID) THEN
        np=TOP_ROW_START+FIRST_ROW_PT-1
        DO K=1,P_LEVELS
          MU_NORTH_POLE(K)=(U(np,K)*U(np,K)+V(np,K)*V(np,K))/
     &                      RS(np,K)*RECIP_G
        ENDDO
        ENDIF
        IF (at_top_of_LPG) THEN
! Send this array to everyone on top processor row
          CALL GCG_RBCAST(123,P_LEVELS,NP_PROC_ID,
     &                    GC_ROW_GROUP,info,MU_NORTH_POLE)
        ENDIF

! And now the South Pole
        IF (MY_PROC_ID .EQ. SP_PROC_ID) THEN
        sp=U_BOT_ROW_START+LAST_ROW_PT-1
        DO K=1,P_LEVELS
          MU_SOUTH_POLE(K)=(U(sp,K)*U(sp,K)+V(sp,K)*V(sp,K))/
     &                      RS(sp+ROW_LENGTH,K)*RECIP_G
        ENDDO
        ENDIF
        IF (at_base_of_LPG) THEN
! Send this array to everyone on bottom processor row
          CALL GCG_RBCAST(321,P_LEVELS,SP_PROC_ID,
     &                    GC_ROW_GROUP,info,MU_SOUTH_POLE)
        ENDIF
      ENDIF ! IF (LWHITBROM)
