...
C ******************************COPYRIGHT******************************
C
CLL   SUBROUTINE DIAG10_B ------------------------------------------
CLL
CLL  PURPOSE: Calculate diagnostics from section 10 after call to
CLL           THETL_QT.
CLL
...
CLL 3.4 10/08/94 Correct all calculations of temperature R A Stratton
CLL              (T=theta*exner instead of T=theta/exner)
CLL              Also multiply some fields by 1.e-6 to avoid problems
CLL              with values close to missing data value.
CLL     11/10/94 Further corrections to moist static energy calculation
CLL              plus correction to calling of COPYDIAG_3D. R A Stratton
!   4.2 25/04/95 Multiply many of the output field by 1.0e-6 to avoid
!                problems with partial sums reaching max values of
!                > 1.e9. R A Stratton.
...
CLL
CLL   System components covered : D3112
CLL
CLL   Project task: P1
CLL
...
C*L  ARGUMENTS:------------------------------------------------------
      ...
      SUBROUTINE DIAG10_B(
     &                    U_ADJ,V_ADJ,QT,THETAL,P_EXNER,PSTAR,U,V,
     &                    SEC_U_LATITUDE,AKH,BKH,
     &                    ROW_LENGTH,P_LEVELS,Q_LEVELS,P_FIELD,
     &                    U_FIELD,FIRST_POINT,LAST_POINT,
...
C INPUT DATA
     ...
     &, THETAL(P_FIELD,P_LEVELS)!IN PRIMARY MODEL ARRAY FOR THETAL FIELD
     &, PSTAR(P_FIELD)              !IN PRIMARY MODEL ARRAY FOR PSTAR
     &, U(U_FIELD,P_LEVELS)         !IN PRIMARY MODEL ARRAY FOR U FIELD
     &, V(U_FIELD,P_LEVELS)         !IN PRIMARY MODEL ARRAY FOR V FIELD
     &, U_ADJ(U_FIELD,P_LEVELS)     !IN MEAN U OVER ADJUSTENT STEPS
     &, V_ADJ(U_FIELD,P_LEVELS)     !IN MEAN V OVER ADJUSTENT STEPS.
     &, QT(P_FIELD,Q_LEVELS)    !IN PRIMARY MODEL ARRAY FOR TOTAL WATER
...
C DEFINE LOCAL ARRAYS: 1 is REQUIRED.
      REAL
     &  VELOCITY(P_FIELD*P_LEVELS) ! WORK-SPACE FOR INTERPOLATED WIND
     &                             ! FIELD.
...
C DEFINE LOCAL VARIABLES
     ...
     &, RECIP_G
     &, PKP1,PK            !  Pressure at half levels k+1 and k
     &, P_EXNER_FULL       !  Exner pressure at full model level
     &, TEMPL_I,TEMPL_IP1  !  Temperatures at points/rows i and i+1
     &, FACTOR             ! Field normalised by this factor

      INTEGER
     &  I,K,K1,LEVEL
     &  ,FIRST_U,FIRST_P    ! first point for copydiag u and p grids
     &  ,LAST_U,LAST_P      ! Last point for copydiag u and p grids
...
C     P_EXNERC contains statement function P_EXNER_C
...
C     Comdeck C_DG10_2 initilaises local variables defined in C_DG10_1
...
C --------------------------------------------------------------------
CL SECTION 1.1 ITEM 215 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * U
C --------------------------------------------------------------------
...
C MINUS SIGN SETS DELTA P TO POSITIVE VALUE.
      IF (L_UADJ_U_DP) THEN
        DO 110 K=1,P_LEVELS
          K1 = (K-1)*U_FIELD
          DO I=FIRST_U,LAST_U
            FIELD(K1+I) = -U(I,K)*U_ADJ(I,K)*EARTH_RADIUS_INVERSE*FACTOR
          END DO
 110    CONTINUE

        CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_U_DP),FIELD,FIRST_U,
     &                    LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                    STLIST(1,INDEX_UADJ_U_DP),LEN_STLIST,
     &                    STASH_LEVELS,
     &                    NUM_STASH_LEVELS+1,
     &                    im_ident,10,215,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 1.2 ITEM 217 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * V
C --------------------------------------------------------------------
...
C MINUS SIGN SETS DELTA P TO POSITIVE VALUE.
      IF (L_UADJ_V_DP) THEN
        DO 120 K=1,P_LEVELS
          K1 = (K-1)*U_FIELD
          DO I=FIRST_U,LAST_U
            FIELD(K1+I) = -V(I,K)*U_ADJ(I,K)*EARTH_RADIUS_INVERSE*FACTOR
          END DO
 120    CONTINUE

        CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_V_DP),FIELD,FIRST_U,
     &                    LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                    STLIST(1,INDEX_UADJ_V_DP),LEN_STLIST,
     &                    STASH_LEVELS,
     &                    NUM_STASH_LEVELS+1,
     &                    im_ident,10,217,
! COMDECK ARGPPX     
...
C CHECK TO SEE IF ANY U DIAGNOSTICS REQUESTED WHICH NEED U_ADJ TO
C BE INTERPOLATED.

      IF(L_UADJ_TL_DP.OR.L_UADJ_QT_DP.OR.
     &   L_UADJ_GEOPOT_DP.OR.L_UADJ_ENERGY_DP) THEN

C --------------------------------------------------------------------
CL SECTION 1.3 REMOVE RADIUS OF EARTH FROM U FIELD AND
CL             INTERPOLATE TO C-GRID U POINTS.
C --------------------------------------------------------------------
...
C --------------------------------------------------------------------
CL SECTION 1.4 ITEM 211 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * LIQUID WATER TEMPERATURE.
C --------------------------------------------------------------------

        IF (L_UADJ_TL_DP) THEN
          DO 140 K=1,P_LEVELS
            K1 = (K-1)*P_FIELD
     ...
     &        (P_EXNER(I,K+1),P_EXNER(I,K),PKP1,PK,KAPPA)
              TEMPL_I = THETAL(I,K) * P_EXNER_FULL

              PKP1 = AKH(K+1) + BKH(K+1)*PSTAR(I+1)
              PK   = AKH(K)   + BKH(K)  *PSTAR(I+1)
              P_EXNER_FULL = P_EXNER_C
     &        (P_EXNER(I+1,K+1),P_EXNER(I+1,K),PKP1,PK,KAPPA)
              TEMPL_IP1 = THETAL(I+1,K) * P_EXNER_FULL

              FIELD(K1+I) = VELOCITY(K1+I) * 0.5 * (TEMPL_I + TEMPL_IP1)
     &                      *FACTOR
...
! Set last point of field (halo) to a valid number
 ...
 140      CONTINUE
          ...
          CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_TL_DP),FIELD,FIRST_P,
     &                      LAST_P,P_FIELD,ROW_LENGTH,P_LEVELS,
     &                      STLIST(1,INDEX_UADJ_TL_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                      im_ident,10,211,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 1.5 ITEM 213 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * TOTAL WATER.
C --------------------------------------------------------------------

        IF (L_UADJ_QT_DP) THEN
          DO 150 K=1,Q_LEVELS
            K1 = (K-1)*P_FIELD
            DO I=FIRST_P,LAST_P-1
              FIELD(K1+I) = VELOCITY(K1+I)*.5*(QT(I,K)+QT(I+1,K))
            END DO
! Set last point of field (halo) to a valid number
            FIELD(K1+LAST_P)=FIELD(K1+LAST_P-1)
 150      CONTINUE

          CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_QT_DP),FIELD,FIRST_P,
     &                      LAST_P,P_FIELD,ROW_LENGTH,Q_LEVELS,
     &                      STLIST(1,INDEX_UADJ_QT_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                      im_ident,10,213,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 1.6 ITEM 219 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * GEOPOTENTIAL
C --------------------------------------------------------------------

        IF (L_UADJ_GEOPOT_DP) THEN
            DO 160 K=1,P_LEVELS
              K1 = (K-1)*P_FIELD
            DO I=FIRST_P,LAST_P-1
                FIELD(K1+I) = VELOCITY(K1+I)*.5*FACTOR*
     &             (STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1)+
     &              STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I))
              END DO
! Set last point of field (halo) to a valid number
            FIELD(K1+LAST_P)=FIELD(K1+LAST_P-1)
 160        CONTINUE

            CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_GEOPOT_DP),FIELD,
     &                       FIRST_P,
     &                       LAST_P,P_FIELD,ROW_LENGTH,P_LEVELS,
     &                       STLIST(1,INDEX_UADJ_GEOPOT_DP),LEN_STLIST,
     &                       STASH_LEVELS,
     &                       NUM_STASH_LEVELS+1,
     &                       im_ident,10,219,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 1.7 ITEM 221 MEAN PRESSURE WEIGHTED U OVER ADJUSTMENT STEPS
CL                      * MOIST STATIC ENERGY.
C --------------------------------------------------------------------

        IF (L_UADJ_ENERGY_DP) THEN
            DO 170 K=1,Q_LEVELS
              K1 = (K-1)*P_FIELD
              DO I=FIRST_P,LAST_P-1
     ...
     &        (P_EXNER(I+1,K+1),P_EXNER(I+1,K),PKP1,PK,KAPPA)
              TEMPL_IP1 = THETAL(I+1,K) * P_EXNER_FULL
                ...
                FIELD(K1+I) = VELOCITY(K1+I)*.5*FACTOR*
     &             (STASHWORK(K1+LOC_GEOPOTENTIAL+I-1)+
     &              STASHWORK(K1+LOC_GEOPOTENTIAL+I)+CP*
     &                                 (TEMPL_I + TEMPL_IP1)
     &                                 +LC*(QT(I,K)+QT(I+1,K)))
...
! Set last point of field (halo) to a valid number
 ...
 175        CONTINUE
            ...
            CALL COPYDIAG_3D (STASHWORK(LOC_UADJ_ENERGY_DP),FIELD,
     &                      FIRST_P,
     &                      LAST_P,P_FIELD,ROW_LENGTH,P_LEVELS,
     &                      STLIST(1,INDEX_UADJ_ENERGY_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                      im_ident,10,221,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 2.1 ITEM 216 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * U
C --------------------------------------------------------------------
...
C MINUS SIGN SETS DELTA P TO POSITIVE VALUE.
      IF (L_VADJ_U_DP) THEN
        DO 210 K=1,P_LEVELS
          K1 = (K-1)*U_FIELD
          DO I=FIRST_U,LAST_U
            FIELD(K1+I)= -U(I,K)*V_ADJ(I,K)*SEC_U_LATITUDE(I)
     &                               *EARTH_RADIUS_INVERSE*FACTOR
          END DO
 210    CONTINUE

        CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_U_DP),FIELD,FIRST_U,
     &                    LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                    STLIST(1,INDEX_VADJ_U_DP),LEN_STLIST,
     &                    STASH_LEVELS,
     &                    NUM_STASH_LEVELS+1,
     &                    im_ident,10,216,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 2.2 ITEM 218 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * V
C --------------------------------------------------------------------
...
C MINUS SIGN SETS DELTA P TO POSITIVE VALUE.
      IF (L_VADJ_V_DP) THEN
        DO 220 K=1,P_LEVELS
          K1 = (K-1)*U_FIELD
          DO I=FIRST_U,LAST_U
            FIELD(K1+I)= -V(I,K)*V_ADJ(I,K)*SEC_U_LATITUDE(I)
     &                                *EARTH_RADIUS_INVERSE*FACTOR
          END DO
 220    CONTINUE

        CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_V_DP),FIELD,FIRST_U,
     &                    LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                    STLIST(1,INDEX_VADJ_V_DP),LEN_STLIST,
     &                    STASH_LEVELS,
     &                    NUM_STASH_LEVELS+1,
     &                    im_ident,10,218,
! COMDECK ARGPPX     
...
C CHECK TO SEE IF ANY V DIAGNOSTICS REQUESTED WHICH NEED V_ADJ TO
C BE INTERPOLATED.

      IF(L_VADJ_TL_DP.OR.L_VADJ_QT_DP.OR.
     &   L_VADJ_GEOPOT_DP.OR.L_VADJ_ENERGY_DP) THEN

C --------------------------------------------------------------------
CL SECTION 2.3 REMOVE RADIUS OF EARTH * COSINE OF LATITUDE FROM V FIELD
CL             AND INTERPOLATE TO C-GRID V POINTS.
C --------------------------------------------------------------------
...
C MINUS SIGN SETS DELTA P TO POSITIVE VALUE.
     ...
     &                                V_ADJ(I-1,K)*SEC_U_LATITUDE(I-1))
     &                                *EARTH_RADIUS_INVERSE
          END DO
...
C --------------------------------------------------------------------
CL SECTION 2.4 ITEM 212 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * LIQUID WATER TEMPERATURE.
C --------------------------------------------------------------------

        IF (L_VADJ_TL_DP) THEN
          DO 240 K=1,P_LEVELS
            K1 = (K-1)*U_FIELD
            DO I=FIRST_U,LAST_U
     ...
     &         PKP1,PK,KAPPA)
              TEMPL_IP1 = THETAL(I+ROW_LENGTH,K) * P_EXNER_FULL
 ...
 240      CONTINUE
          ...
          CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_TL_DP),FIELD,FIRST_U,
     &                      LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                      STLIST(1,INDEX_VADJ_TL_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                      im_ident,10,212,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 2.5 ITEM 214 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * TOTAL WATER.
C --------------------------------------------------------------------

        IF (L_VADJ_QT_DP) THEN
          DO 250 K=1,Q_LEVELS
            K1 = (K-1)*U_FIELD
            DO I=FIRST_U,LAST_U
              FIELD(K1+I) = VELOCITY(K1+I)*.5*
     &                                 (QT(I,K)+QT(I+ROW_LENGTH,K))
            END DO
 250      CONTINUE

          CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_QT_DP),FIELD,FIRST_U,
     &                      LAST_U,U_FIELD,ROW_LENGTH,Q_LEVELS,
     &                      STLIST(1,INDEX_VADJ_QT_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                         im_ident,10,214,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 2.6 ITEM 220 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * GEOPOTENTIAL HEIGHT
C --------------------------------------------------------------------

        IF (L_VADJ_GEOPOT_DP) THEN
            DO 260 K=1,P_LEVELS
              K1 = (K-1)*U_FIELD
              DO I=FIRST_U,LAST_U
                FIELD(K1+I) = VELOCITY(K1+I)*.5*FACTOR*
     &          (STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1)+
     &         STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1+ROW_LENGTH))
              END DO
 260        CONTINUE

            CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_GEOPOT_DP),FIELD,
     &                      FIRST_U,
     &                      LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                      STLIST(1,INDEX_VADJ_GEOPOT_DP),LEN_STLIST,
     &                      STASH_LEVELS,
     &                      NUM_STASH_LEVELS+1,
     &                      im_ident,10,220,
! COMDECK ARGPPX     
...
C --------------------------------------------------------------------
CL SECTION 2.7 ITEM 222 MEAN PRESSURE WEIGHTED V OVER ADJUSTMENT STEPS
CL                      * MOIST STATIC ENERGY.
C --------------------------------------------------------------------

        IF (L_VADJ_ENERGY_DP) THEN
            DO 270 K=1,Q_LEVELS
              K1 = (K-1)*U_FIELD
              DO I=FIRST_U,LAST_U
     ...
     &                 (STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1)+
     &          STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1+ROW_LENGTH)
     &                               +CP*(TEMPL_I + TEMPL_IP1)
     &                               +LC*(QT(I,K)+QT(I+ROW_LENGTH,K)))

              END DO
 270        CONTINUE
            DO 275 K= Q_LEVELS+1,P_LEVELS
              K1 = (K-1)*U_FIELD
              DO I=FIRST_U,LAST_U
     ...
     &                  (STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1)+
     &          STASHWORK((K-1)*P_FIELD+LOC_GEOPOTENTIAL+I-1+ROW_LENGTH)
     &                               + CP * (TEMPL_I + TEMPL_I+1) )

              END DO
 275        CONTINUE

            CALL COPYDIAG_3D (STASHWORK(LOC_VADJ_ENERGY_DP),FIELD,
     &                       FIRST_U,
     &                       LAST_U,U_FIELD,ROW_LENGTH,P_LEVELS,
     &                       STLIST(1,INDEX_VADJ_ENERGY_DP),LEN_STLIST,
     &                       STASH_LEVELS,
     &                       NUM_STASH_LEVELS+1,
     &                       im_ident,10,222,
! COMDECK ARGPPX     
