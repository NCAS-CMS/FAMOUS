...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE CHK_LOOK---------------------------------------
CLL
CLL  Written by A. Dickinson
CLL
CLL  Purpose: Cross checks pointers in PP LOOKUP records with
...
CLL   3.3  22/07/93    Applies a different consistency check for
CLL                    boundary files because the value stored in
CLL                    FIXHD(161) is half the correct value. This is a
CLL                    temporary fix. A permanent fix will follow in
CLL                    version 3.3.
CLL                    Author: D.M.Goddard
CLL   3.4  24/12/93    Removes fix included at version 3.3 as GENINTF1
CLL                    now sets FIXHD(161) correctly for boundary files
CLL                    Author D.M.Goddard
!LL   4.1  20/05/96    Remove check on length of data for MPP code
!LL                    P.Burton
!     4.1  18/06/96    Changes to cope with changes in STASH addressing
!                      Author D.M. Goddard.
!     4.2  10/05/96    Bypass checks in cumf, pumf and ieee
!                      Author Bob Carruthers
!     4.3  12/03/97    Bypass checks in convpp
!                      Author D.M.Goddard 
!     4.3  20/03/97    Bypass checks in camdump
!                      Author A Brady
!  4.4  15/09/97  Add helpful message.  RTHBarnes
!  4.4  14/10/97  Skip check for Pre 3.4 Boundary Datasets. D. Robinson
CLL
...
CLL------------------------------------------------------------
C*L Arguments:-------------------------------------------------
      SUBROUTINE CHK_LOOK(FIXHD,LOOKUP,LEN1_LOOKUP,     ! Intent (In)
     &                    LEN_DATA,                     !
! COMDECK ARGPPX     
! Description:
...
! End of comdeck
     ...
     *,LEN1_LOOKUP     !IN First dimension for LOOKUP
...
C Local variables:---------------------------------------------
      INTEGER
     * K       ! Loop count
     *,LEN_D     ! Cumulative length of data
     *,N1
C--------------------------------------------------------------
...
! CHK_LOOK falls over with Boundary Datasets if pre-3.4
      IF (FIXHD(5).EQ.5 .and. FIXHD(12).LT.304) THEN
