...
C ******************************COPYRIGHT******************************
C
CLL  Subroutine ABORT_IO-----------------------------------------------
CLL
CLL  Purpose:  Prints out message and stops execution of program.
CLL            Called if ICODE .NE. 0
CLL
...
CLL   4.4    15/10/97 Added code to print the error messages
CLL                   to stderr as well as the unit 6
CLL                     Author: Bob Carruthers, Cray Research
CLL   4.5    08/07/98 Print only the leading non-blank 
CLL                   characters in 'cmessage'
CLL                     Author: Bob Carruthers, Cray Research
CLL
CLL  Logical component number: E5
CLL
CLL  External Documentation: None
CLL
CLLEND
C*L  Arguments:--------------------------------------------------------
      ...
      INTEGER
     * ICODE   !IN Code returned by UM routines
     *,NFT     !IN Unit no being processed

      CHARACTER*80
     * STRING  !IN Subroutine name and position
...
CL Internal structure: None
      ...
      WRITE(6,'('' Error detected in subroutine '',A)')STRING
      IF(NFT.NE.0)THEN
      WRITE(6,'('' while doing I/O on unit'',I3)')NFT
      ENDIF
      WRITE(6,'(1X,A80)')CMESSAGE(1:get_char_len(cmessage))
      WRITE(6,'('' ICODE='',I6)')ICODE
