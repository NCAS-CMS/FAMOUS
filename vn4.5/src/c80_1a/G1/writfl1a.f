...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE WRITFLDS---------------------------------------
CLL
CLL  Purpose:
CLL          Buffers out NUMBER_OF_FIELDS fields from DATA block on unit
CLL          NFTOUT. 32-bit and 64-bit real numbers and integer/logical
CLL          data types are handled. The I/O starts at field number
CLL          POSITION, where POSITION is the number of the PP header
CLL          pointing to the 1st field to be written. The code uses SETP
CLL          to position the file pointer. The output file must therefor
CLL          be unblocked, ie use assign ... -su ... in the script.
CLL
CLL AD, DR, TJ  <- programmer of some or all of previous code or changes
CLL
CLL  Model            Modification history from model version 3.0:
CLL version  Date
CLL   3.1  19/02/93  Use FIXHD(12) not FIXHD(1) as Version no in P21BITS
CLL   3.3  25/11/93  Use PR_LFLD to print logical fields. Skip DIAG81
CLL                  diagnostics for observation files. Skip field
CLL                  summaries for boundary data. D. Robinson.
CLL   3.3  08/12/93  Extra argument - first dimension of lookup table.
CLL                  Remove hard-wired value of 64. D. Robinson
CLL   4.1  11/05/96  Allowed for Var and OPS files. Author Colin Parrett
CLL   4.1  03/01/96  Relace Char*100 with Char*80 (N Farnon)
...
CLLEND---------------------------------------------------------
C*L Arguments:-------------------------------------------------
      SUBROUTINE WRITFLDS(NFTOUT,NUMBER_OF_FIELDS,      ! Intent (In)
...
! End of comdeck
     &                    ICODE,CMESSAGE)               ! Intent (Out)
      ...
      INTEGER
     * NFTOUT       !IN Unit number for I/O
     *,ICODE            !OUT Return code =0 normal exit; >0 error
     *,NUMBER_OF_FIELDS !IN No of fields to be written
     *,LEN_BUF      !IN Length of I/O buffer
     *,POSITION     !IN Field number from which to begin I/O
     *,FIXHD(*)     !IN Fixed length header
     *,LEN1_LOOKUP  !IN First dimension of lookup table
     *,LOOKUP(LEN1_LOOKUP,*) !IN PP lookup starting at field no 1

      REAL
     * D1(*)        !IN Start address of data to be written out

      CHARACTER*80
     * CMESSAGE     !OUT Message returned if ICODE>0
...
C
      ...
      real buf(((len_buf-1+um_sector_size)/  ! I/O buffer
     2 um_sector_size)*um_sector_size)
cdir$ cache_align buf
C -------------------------------------------------------------
C External subroutines called:---------------------------------
      EXTERNAL PR_LOOK,PR_RFLD,PR_IFLD,IOERROR,PACK21,SETPOS,BUFFIN
     *        ,P21BITS,PR_LFLD
      INTEGER  P21BITS
C*-------------------------------------------------------------
C Local variables:---------------------------------------------
      INTEGER
     * I,J,K            !  Indicies
     *,LEN_IO          ! Length of I/O returned by LENGTH
     *,IPTS            ! No of values to be written to disk
     *,WORD_ADDRESS    ! word address to begin I/O
     ...
     *, kk             ! local value of k for address computing
...
CL 2. Buffer out NUMBER_OF_FIELDS fields of data:
      J=0
      DO 200 K=POSITION,POSITION+NUMBER_OF_FIELDS-1

C Test whether data stored as 32-bit on disk
      IF(MOD((LOOKUP(LBPACK,K)),10).EQ.2) THEN
C Pack 32-bit numbers using P21BITS for exponent (fn of dump release)
...
C Prog lookups in dump before vn3.2:
     4    ((lookup(lbnrec,k).eq.imdi) .and. (fixhd(12).le.301))) then
          word_address=1
          do kk=2, k
            if(mod(lookup(lbpack,kk-1),10).eq.2) then
              l_ipts=(lookup(lblrec,kk-1)+1)/2
            else
              l_ipts=lookup(lblrec,kk-1)
            endif
            word_address=word_address+l_ipts
          end do
          word_address=word_address+fixhd(160)-2
          um_sector_ipts=ipts
...
C Position file pointer
        ...
        if(mod(lookup(lbpack,k),10).eq.2) then
          call pack21(lookup(lblrec,k),d1(j+1),buf,p21bits(fixhd(12)))
        else
!Compiler error removal *DIR$ CACHE_BYPASS BUF,D1
...
C Check for I/O errors
       if(a_io.ne.-1.0.or.len_io.ne.ipts_write) then
...
! End of comdeck
     &             LOOKUP,LOOKUP,LEN1_LOOKUP,K)
        ENDIF
        CALL IOERROR('buffer out of real data',A_IO,LEN_IO,
     *               IPTS)
        ICODE=NINT(A_IO)+1
        CMESSAGE='WRITFLDS:I/O error'
        RETURN
       ENDIF
