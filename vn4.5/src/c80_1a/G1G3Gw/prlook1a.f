...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE PR_LOOK----------------------------------------
CLL
CLL  Purpose: Prints out Kth 64-word PP header
CLL
...
CLL   4.0  12/09/95    Change NPERIODS to LBUSER, LBUSER to LBPLEV
CLL                    as changes in CLOOKADD and PPHEAD1A.
CLL                    (Andrew Brady) 
CLL  4.0  12/10/95  Chg. FORMAT, as last LBUSER is now MODEL_CODE. RTHB
...
! End of comdeck
     &              ICODE,CMESSAGE)
      ...
      WRITE(6,100) K,LOOKUP(LBHR,K),LOOKUP(LBMIN,K),LOOKUP(LBDAT,K),
     *  LOOKUP(LBMON,K),LOOKUP(LBYR,K),LOOKUP(LBDAY,K),LOOKUP(LBHRD,K),
     *  LOOKUP(LBMIND,K),LOOKUP(LBDATD,K),LOOKUP(LBMOND,K),
     &  LOOKUP(LBYRD,K),LOOKUP(LBDAYD,K),PHRASE

  100 FORMAT(' FIELD NO.', I3,
     *  ' VALID AT: ',  2I2.2,'Z ON ',2(I2.2,'/'),I4.4,' DAY',I6,
     *  ' DATA TIME: ', 2I2.2,'Z ON ',2(I2.2,'/'),I4.4,' DAY',I6, 4X,A)

CL Rest of header
      WRITE(6,200) (LOOKUP(I,K),I=13,45),(RLOOKUP(I,K),I=46,64)
  200 FORMAT( '   LBTIM   LBFT LBLREC LBCODE  LBHEM  LBROW  LBNPT',
     *  '  LBEXT LBPACK  LBREL   LBFC  LBCFC LBPROC',
     *  '   LBVC  LBRVC  LBEXP LBBEGIN LBNREC'/
     *  1X, 18I7, / ,'  LBPROJ  LBTYP  LBLEV LBRSVD LBRSVD LBRSVD',
     *  ' LBRSVD LBSRCE DATA_TYPE   NADDR  LBUSER  ITEM_CODE   LBPLEV  
     &    LBUSER  MODEL_CODE',/,1X,9I7,6I10,/,5X,'BRSVD(1)',4X,
     &  'BRSVD(2)',4X,'BRSVD(3)',4X,'BRSVD(4)', 
     *  6X, 'BDATUM', 8X, 'BACC', 8X, 'BLEV', 7X, 'BRLEV', 7X, 'BHLEV',/
     *  1X, 1P, 9E12.4, /,
     *  7X, 'BHRLEV', 7X, 'BPLAT', 7X, 'BPLON', 8X, 'BGOR', 9X, 'BZY',
     *  9X, 'BDY', 9X, 'BZX', 9X, 'BDX', 8X, 'BMDI', 8X, 'BMKS',/,
     *  1X, 1P, 10E12.4)
