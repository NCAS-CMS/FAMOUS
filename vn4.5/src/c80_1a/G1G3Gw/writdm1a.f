...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE WRITDUMP---------------------------------------
CLL
CLL AD, TJ      <- programmer of some or all of previous code or changes
CLL
...
CLL   3.2  24/07/93  CHECK ON THE ERROR STATUS FROM BUFFOUT
CLL   3.2  25/05/93  Skip DIAG81 diagnostics for obs files. D. Robinson
CLL   3.3  08/04/94  Check that BUFLEN is long enough
CLL   3.3  22/11/93  Prevent dynamic allocation of zero dimension for
CLL                  BUF. (Possible for obs files) Call PR_LFLD to print
CLL                  logical fields. D Robinson.
...
CLL  Project task: F3
CLL
CLL  Purpose: Writes out model dump on unit NFTOUT and checks model
...
C -------------------------------------------------------------
C Local arrays:------------------------------------------------
      real buf(((buflen+um_sector_size)/um_sector_size)*um_sector_size)
cdir$ cache_align buf
C -------------------------------------------------------------
C*L External subroutines called:-------------------------------
      EXTERNAL IOERROR,POSERROR,WRITHEAD,PR_LOOK,PR_IFLD,PR_RFLD
     *,PACK21,EXPAND21,BUFFOUT,P21BITS,PR_LFLD
      INTEGER  P21BITS
...
C Local variables:---------------------------------------------
     ...
     *                     ! buffered in
     ...
     &, local_len  ! length of local field from buffout
     &, address    ! address of field in local D1 array
     &, word_address     ! disk address of the record
     &, real_start_block ! real start address and number of words moved
     &, l_ipts           ! record length during index search
     &, um_sector_ipts   ! number fo words to write, rounded up
     &, ipts_write       ! number of words actually write from disk
     &, disk_address                    ! Current rounded disk address
     &, number_of_data_words_on_disk    ! Number of data words on disk
...
! Select the relevant decomposition type for this dump
     ...
     &  CALL CHANGE_DECOMPOSITION(decomp_standard_ocean,ICODE)
      ...
      ELSE  ! unsupported decomposition type
        WRITE(6,*) 'WRITEDUMP : Error - Only atmosphere and ocean ',
...
c--reset the disk addresses and lengths for well-formed I/O
...
CL    conversion from 64-bit to 32-bit numbers
...
C If new format Dumpfile, we must reset the start address
     ...
     2     ((lookup(lbnrec,1).eq.imdi) .and. (fixhd(12).le.301))) then
        CMESSAGE='WRITDUMP: Addressing conflict'
        ICODE=1
...
C Loop over number of fields in data blocks
      address=1
      object_index=1
      level=1
       DO 200 K=1,FIXHD(152)
        ...
        IF (LOOKUP(LBLREC,K) .GT. 0) THEN  ! if this isnt a zero
!                                          ! length field
C Is the buffer length long enough for this field.
        IF(LOOKUP(LBLREC,K).GT.BUFLEN) THEN
          ICODE=100
          CMESSAGE='WRITDUMP : Field length longer than buffer'
          WRITE(6,*) 'WRITDUMP :Field length longer than buffer, abort',
     &                LOOKUP(LBLREC,K),'>',BUFLEN
          RETURN
        END IF
! Data packing to 32 bits is moved down to write_multi. We only want
! to compress data after the global data has been gathered
...
CL  Compute word address in file from which to begin I/O
...
C Dump and ancillary files
            ...
            if(k.gt.1) then
              do i=2, k
                if(mod(lookup(lbpack,i-1),10).eq.2) then
...
C Position file pointer
          call setpos(nftout, word_address, icode)


C Write data out from buffer
C Check that data_type is valid no: 1 to 3 or -1 to -3
     ...
     &      THEN
        ...
        ENDIF
          ...
          call write_multi(nftout,d1(address),um_sector_ipts,
     &                     len_io,local_len,a,
     ...
     &                     CMESSAGE)
          ...
          if((a.ne.-1.0).or.(len_io.ne.ipts_write)) then

            WRITE(6,*)'ERROR WRITING DUMP ON UNIT ',NFTOUT
            ICODE=3
            CMESSAGE='WRITDUMP: BAD BUFFOUT OF DATA'
            CALL IOERROR('BUFFER OUT FROM WRITDUMP',A,LEN_IO,IPTS)
            RETURN
          END IF
        ELSE
          IF (FIXHD(5).LT.6.OR. FIXHD(5).GT.9) THEN !Not AC/VarOb/Cx/Cov
          CALL PR_LOOK(
! COMDECK ARGPPX     
! Description:
!   Argument comdeck for passing arrays which hold the
...
! End of comdeck
     &                 LOOKUP,LOOKUP,LEN1_LOOKUP,K)
          ENDIF
          ICODE=3
          CMESSAGE='WRITDUMP:  Invalid code in LOOKUP(DATA_TYPE,K)'
          RETURN
        END IF

      ENDIF ! IF this field is non-zero length

       START_BLOCK=START_BLOCK+LOOKUP(LBLREC,K)
          real_start_block=real_start_block+ipts
...
! Reset to original decomposition type
      ...
      IF (mype .EQ.0 ) THEN
      WRITE(6,'('' '')')
      WRITE(6,'('' MODEL DUMP SUCCESSFULLY WRITTEN -'',I9,
     *'' WORDS TO UNIT'',I3)')START_BLOCK,NFTOUT
       if(real_start_block.ne.start_block) then
         write(6,'(/'' Number of Words Written to Disk was '',i9)')
     2    real_start_block
       endif
      ENDIF ! if mype .EQ. 0
...
CLL  SUBROUTINE WRITDUMP---------------------------------------
CLL
CLL  Purpose : Writes an obs file.
CLL
CLL  Code mostly copied from original WRITDUMP
CLL
CLL  Model            Modification history from model version 4.3:
CLL version  Date
CLL   4.3  19/3/97   New routine introduced                P.Burton
CLL
CLL  Programming standard: Unified Model Documentation Paper No 3
CLL                        Version No 1 15/1/90
CLL
...
C
C*L Arguments:-------------------------------------------------
      SUBROUTINE WRITDUMP(NFTOUT,FIXHD,LEN_FIXHD,
     &                    INTHD,LEN_INTHD,
     &                    REALHD,LEN_REALHD,
     &                    LEVDEPC,LEN1_LEVDEPC,LEN2_LEVDEPC,
     &                    ROWDEPC,LEN1_ROWDEPC,LEN2_ROWDEPC,
     ...
     &                    CFI2,LEN_CFI2,
     &                    CFI3,LEN_CFI3,
     &                    LOOKUP,LEN1_LOOKUP,LEN2_LOOKUP,BUFLEN,
     &                    LEN_DATA,D1,
! COMDECK ARGPPX     
! Description:
!   Argument comdeck for passing arrays which hold the
...
C Check for error in file pointers
       real_start_block=start_block
       if(start_block.eq.fixhd(160)) then
C If new format Dumpfile, we must reset the start address
         if((lookup(lbnrec,1).eq.0) .or.
C Ocean ACOBS Files (?)
...
C Check that data_type is valid no: 1 to 3 or -1 to -3
     ...
     +     THEN
          ...
          CALL BUFFOUT_shmem(NFTOUT,BUF(1),IPTS,LEN_IO,A)
