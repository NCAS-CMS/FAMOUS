...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE PR_IFLD----------------------------------------
CLL
CLL  Purpose: Prints out selected values from integer data
...
CLL  3.2   28/07/93     STOP WRITING OF FIELDS WHEN HEADERS ARE NOT SET.
CLL
...
C Print out max and min values of field
      F_MIN=ID1(1)
      F_MAX=ID1(1)
      POS_MAX=1
      POS_MIN=1
      DO 500 J=1,N_ROWS*N_COLS
      IF(ID1(J).GT.F_MAX)THEN
      F_MAX=ID1(J)
      POS_MAX=J
      ENDIF
      IF(ID1(J).LT.F_MIN)THEN
      F_MIN=ID1(J)
      POS_MIN=J
      ENDIF
500   CONTINUE

      WRITE(6,'('' MINIMUM='',I8,'' POSITION='',I8,
     *'' MAXIMUM='',I8,'' POSITION='',I8)')
