...
! Subroutine Interface
      SUBROUTINE SCATTER_ATMOS_LBCS(FULL_LBC,FULL_LENRIMDATA,
...
!
! Description:
! Scatters atmosphere LBCs to relevant processors at the
! grid boundaries
!
! Method:
...
! is sent from SCATTER_PE's FULL_LBC array to that
! processors PART_LBC array.
...
!    4.2    23/10/96   Use GC_SETOPT to use GETs under GCOM_shmem
!                      P.Burton
...
! Subroutine Arguments:
     ...
     &, SCATTER_PE        ! IN processor to scatter data from
     ...
     &  CMESSAGE          ! OUT error message
      ...
      REAL
     &  FULL_LBC(FULL_LENRIMDATA)  ! IN full LBC (only on SCATTER_PE)
...
! CNTLATM contains L_LSPICE_BDY to say whether LBC file contains QCF
! ----------------------- Comdeck: CNTLATM  ----------------------------
...
!             Northern or Southern boundary
     ...
     &                 2*P_LEVELS*N_U_ROWS*N_U_POINTS
            if (L_LSPICE_BDY) then
              data_size=data_size+Q_LEVELS*N_P_ROWS*N_P_POINTS
            end if
!           the size of the data to be sent to processor iproc

!           Check the buffer is big enough
            IF (FULL_LENRIMDATA .LT.
     &          data_size) THEN
              WRITE(6,*) 'ERROR Buffer not big enough in SCATTER_LBCS'
...
!      2.2.2 Pack all the data for this boundary into the buf3 array

              CALL GC_SETOPT(GC_SHM_DIR,GC_SHM_GET,info)  ! scatter
              info=GC_NONE

            IF (mype .EQ. SCATTER_PE) THEN  ! I'm scattering data
...
!             --- PSTAR ---
     ...
     &                   global_START_POINT+N_P_POINTS-1
                  buf3(buf_pt)=FULL_LBC(global_PSTAR_START-1+
     &                                 global_P_bound_off+
     &                                 POINT+
...
!             --- U and V winds ---
     ...
     &                     global_START_POINT+N_U_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_U_START-1+
     &                                   global_U_bound_off+
     &                                   POINT+
     &                                   (ROW-1)*global_U_ROW_LEN+
     &                                   (LEVEL-1)*global_U_LENRIM)
                    buf_pt=buf_pt+1
                    buf3(buf_pt)=FULL_LBC(global_V_START-1+
     &                                   global_U_bound_off+
     &                                   POINT+
     &                                   (ROW-1)*global_U_ROW_LEN+
     &                                   (LEVEL-1)*global_U_LENRIM)
...
!             -- Theta ---
              ...
              DO LEVEL=1,P_LEVELS
                DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                  DO POINT=global_START_POINT,
     &                     global_START_POINT+N_P_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_THETA_START-1+
     &                                   global_P_bound_off+
     &                                   POINT+
     &                                   (ROW-1)*global_P_ROW_LEN+
...
!           --- Q ---
              ...
              DO LEVEL=1,Q_LEVELS
                DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                  DO POINT=global_START_POINT,
     &                     global_START_POINT+N_P_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_Q_START-1+
     &                                   global_P_bound_off+
     &                                   POINT+
     &                                   (ROW-1)*global_P_ROW_LEN+
     &                                   (LEVEL-1)*global_P_LENRIM)
              ...
              DO TVAR=1,TR_VARS
                DO LEVEL=1,TR_LEVELS
                  DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                    DO POINT=global_START_POINT,
     &                       global_START_POINT+N_P_POINTS-1
                      buf3(buf_pt)=FULL_LBC(global_TR_START-1+
     &                            global_P_bound_off+
     &                            POINT+
     &                            (ROW-1)*global_P_ROW_LEN+
     &                            (LEVEL-1)*global_P_LENRIM+
...
!           --- QCF ---
              ...
              DO LEVEL=1,Q_LEVELS
                DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                  DO POINT=global_START_POINT,
     &                     global_START_POINT+N_P_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_QCF_START-1+
     &                                   global_P_bound_off+
     &                                   POINT+
     &                                   (ROW-1)*global_P_ROW_LEN+
     &                                   (LEVEL-1)*global_P_LENRIM)
...
!      2.2.3 Send buf3 array to processor iproc into array local_buf

              CALL GC_RSEND(iproc+1000*bound_type,data_size,iproc,info,
     &                      local_buf,buf3)

            ENDIF ! IF I'm doing the scatter
...
!      2.2.4 Receive our part of the LBC info local_buf

              CALL GC_RRECV(iproc+1000*bound_type,data_size,SCATTER_PE,
     &                      info,local_buf,buf3)


!      2.2.5 Unpack local_buf into our PART_LBC array
