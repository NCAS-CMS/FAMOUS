...
! Subroutine arguments:
     ...
     &, SCATTER_PE      ! IN:  PE on which GLOBAL_FIELD resides
      ...
      REAL
     &  GLOBAL_FIELD(GLOBAL_SIZE,LEVELS) ! IN : field to scatter
...
! Local variables
     ...
     &, send_map(7,MAXPROC)  ! send map
     &, receive_map(7,1)     ! receive map
...
!--------------------------------------------------------------------
     ...
     &      g_datastart(2,iproc)
      ...
      DO k=1,LEVELS
        ...
        IF (fld_type .EQ. fld_type_p) THEN
          CALL GCG_RALLTOALLE(
     &      GLOBAL_FIELD(1,k),send_map,n_mess_to_send,glsize(2),
     &      LOCAL_FIELD(1,k), receive_map,1,lasize(2),
     &      GC_ALL_PROC_GROUP,flag,info)
        ELSE
          CALL GCG_RALLTOALLE(
     &      GLOBAL_FIELD(1,k),send_map,n_mess_to_send,glsize(2)-1,
     &      LOCAL_FIELD(1,k), receive_map,1,lasize(2),
     &      GC_ALL_PROC_GROUP,flag,info)
        ENDIF
