...
!  version
!    4.1    24/1/96   New DECK created for the Parallel Unified
!                     Model. P.Burton
!    4.2    17/10/96  Modify send/receive maps and change args to
!                     alltoall for GCOM/GCG v1.1     P.Burton
!    4.3    07/02/97  Correct INTENT of data arrays
...
! Subroutine Arguments:
     ...
     &, SCATTER_PE       ! IN processor to scatter global field from
...
! Local variables
     ...
     &, old_SCATTER_PE        ! value on last call
...
! 1.0 Find the type of field (P or U) being done
        ...
        ELSEIF (GLOBAL_ROWS .EQ. glsize(2)-1) THEN
          fld_type=fld_type_u
        ELSE
          WRITE(6,*) 'Bad field type in SCATTER_FIELD'
          info=-1
          GOTO 9999
        ENDIF
...
!           One less row at the bottom of a U field
     ...
     &      LOCAL_ROWS-2*Offy
        ...
        receive_map(R_ELEMENT_LENGTH,1) = LOCAL_ROW_LEN-2*Offx
