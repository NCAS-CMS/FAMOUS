...
C ******************************COPYRIGHT******************************
!+ Passes out ocean LBCs to processors at boundaries
!
! Subroutine Interface
      SUBROUTINE SCATTER_OCEAN_LBCS(FULL_LBC,FULL_LENRIMDATA,
...
!
! Description:
! Scatters ocean LBCs to relevant processors at the
! grid boundaries
!
! Method:
...
! P_LEVELS, Q_LEVELS and RIMWIDTHO which we require
C*L================ COMDECK TYPSIZE ===========================
C   Description:
...
! Set up some sizes and addresses for the global data
...
! Set up some addresses for variable types
        ...
        IF (L_OBDY_TRACER) THEN
          ...
          offset = local_P_LENRIM * KM_UI * NT_UI
        ELSE
          local_TRACER_START = -1
          offset = 0
        ENDIF
...
! Set up some addresses for grid location
        ...
        IF ( L_OBDY_SOUTH) THEN
          ...
          offset_p = offset_p + local_ROW_LENGTH * RIMWIDTHO
          offset_u = offset_u + local_U_ROW_LENGTH * RIMWIDTHO
        ELSE
...
!             --- TRACERS ---
     ...
     &                   global_START_POINT+N_P_POINTS-1
                      buf3(buf_pt)=FULL_LBC(global_TRACER_START-1+
     &                                 global_P_bound_off+
     &                                 POINT+
              ...
              IF (L_OBDY_STREAM) THEN
               ...
               DO TVAR = 1,2
                DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                  DO POINT=global_START_POINT,
     &                 global_START_POINT+N_P_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_STREAM_START-1+
     &                               global_P_bound_off+
     &                               POINT+
     &                               (ROW-1)*global_P_ROW_LEN+
              ...
              IF (L_OBDY_ICE) THEN
               ...
               DO TVAR=1,3
                DO ROW=global_START_ROW,global_START_ROW+N_P_ROWS-1
                  DO POINT=global_START_POINT,
     &                 global_START_POINT+N_P_POINTS-1
                    buf3(buf_pt)=FULL_LBC(global_ICE_START-1+
     &                               global_P_bound_off+
     &                               POINT+
     &                               (ROW-1)*global_P_ROW_LEN+
     &                               (TVAR-1)*global_P_LENRIM)
