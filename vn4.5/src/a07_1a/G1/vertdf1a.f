...
CLL version  Date
CLL
CLL   3.3   15/11/93  Removal of DIAG07 directive. New arguments to
...
CLL                    (was used to switch on WHENFGT) 
CLL                                    S.J.Swarbrick
!LL   4.4    17/06/97 Only prints out warning if there are no points
!LL                   on all processors                     P.Burton
...
! --------------------- End of comdeck PARVARS ---------------------
      ...
      INTEGER global_points  ! maximum number of points on any PEs
...
C   1 INTEGER INDEX ARRAY
     ...
     *,PSTAR_VD( ROWS*ROW_LENGTH )   ! GATHERED INTERPOLATED PSTAR
     *,VERT_DIF_LAT(ROWS*ROW_LENGTH) ! LAT. DEPENDENT DIFFUSION*TIMESTEP
     *,VERT_DIF_VD(ROWS*ROW_LENGTH)  ! GATHERED NON_ZERO DIFFUSION
     *,U_VD(ROWS*ROW_LENGTH*LEVELS_VD)  !GATHERED U
     *,V_VD(ROWS*ROW_LENGTH*LEVELS_VD)  !GATHERED V
     *,FLUX_U_DG(POINTS_FLUX_U,LEVELS_FLUX) !GATHERED FLUX
     *,FLUX_V_DG(POINTS_FLUX_V,LEVELS_FLUX) !GATHERED FLUX

      INTEGER
     * VERT_INDEX(ROWS*ROW_LENGTH)
...
CL         AND DETERMINE WHICH POINTS DIFFUSION IS APPLIED
C------------------------------------------------------------------
...
C
        ...
        END DO
CL      SET UP INDEX FOR POINTS WITH NON-ZERO DIFFUSION


        POINTS_VD = 0
        II = 1
        DO I=1,U_POINTS
          IF(VERT_DIF_LAT(I).GT.0) THEN
            VERT_INDEX(II)=I
            II=II + 1
            POINTS_VD=POINTS_VD + 1
          END IF
       END DO

      global_points=POINTS_VD

      CALL GC_IMAX(1,nproc,info,global_points)


CL      TEST FOR NO NON-ZERO DIFFUSION
        IF(POINTS_VD.EQ.0) THEN
        IF(global_points.EQ.0) THEN
      WRITE(6,*) ' *************VERT_DIF WARNING*******************'
      WRITE(6,*) ' * NO POINTS WITH NON_ZERO DIFFUSION COEFFICIENT*'
      WRITE(6,*) ' *************VERT_DIF WARNING*******************'
        ENDIF
           IRET = 0
           GOTO 1000
...
C------------------------------------------------------------------
CL    2. GATHER PSTAR,WINDS AND DIFFUSION COEFFICIENT AT POINTS
CL       WHERE NON-ZERO DIFFUSION
C------------------------------------------------------------------

      DO I=1,POINTS_VD
       PSTAR_VD(I)  = PSTAR_UV(VERT_INDEX(I))
       VERT_DIF_VD(I)  = VERT_DIF_LAT(VERT_INDEX(I))
      END DO

CL    LOOP OVER LEVELS

      DO K = LEVEL_START, LEVEL_END
         IK = (K-LEVEL_START)*POINTS_VD
         DO I=1,POINTS_VD
           U_VD(I+IK)=U(START_U+VERT_INDEX(I),K)
           V_VD(I+IK)=V(START_U+VERT_INDEX(I),K)
         END DO
      END DO
      LEVELS_VD=LEVEL_END-LEVEL_START+1

CL  3.  CALL VERT_DIF AND UPDATE WINDS

      CALL VERT_DIF(PSTAR_VD,U_VD,V_VD,LEVELS_VD,POINTS_VD,
     &              AK(LEVEL_START),BK(LEVEL_START),
     &              DELTA_AK(LEVEL_START),DELTA_BK(LEVEL_START),
     &              VERT_DIF_VD,FLUX_U_DG,FLUX_V_DG,
     &              POINTS_FLUX_U,POINTS_FLUX_V,LEVELS_FLUX,
     &              FLUX_UD_ON,FLUX_VD_ON)
      ...
      DO K = LEVEL_START, LEVEL_END
         ...
         DO I=1,POINTS_VD
           U(START_U+VERT_INDEX(I),K)  = U_VD(I+IK)
           V(START_U+VERT_INDEX(I),K)  = V_VD(I+IK)
         END DO
      ...
      IF (FLUX_UD_ON .OR. FLUX_VD_ON) THEN
        ...
        DO K = LEVEL_START,LEVEL_END-1
          ...
          IF (FLUX_UD_ON .AND. U_LIST(K)) THEN
           STASH_U_FLUX(START_U+VERT_INDEX(I),KOUT_U) = FLUX_U_DG(I,IK)
          ENDIF
          IF (FLUX_VD_ON .AND. V_LIST(K)) THEN
           STASH_V_FLUX(START_U+VERT_INDEX(I),KOUT_V) = FLUX_V_DG(I,IK)
          ENDIF
...
C*L  ARGUMENTS:---------------------------------------------------
      SUBROUTINE VERT_DIF
     *  (PSTAR,U,V,LEVELS_VD,POINTS_VD,AK,BK,DELTA_AK,DELTA_BK,
     ...
     *, LEVELS_FLUX        !IN    NUMBER OF LEVELS TO BE UPDATED
      ...
      REAL
     * PSTAR(POINTS_VD)       !IN    PSTAR FIELD
     *,U(POINTS_VD,LEVELS_VD) !INOUT ARRAY FOR U FIELD
     *,V(POINTS_VD,LEVELS_VD) !INOUT ARRAY FOR V FIELD
C            AK,BK  DEFINE HYBRID VERTICAL COORDINATES P=A+BP*,
C       DELTA_AK,DELTA_BK  DEFINE LAYER PRESSURE THICKNESS PD=AD+BDP*,
     ...
     *,DIFFUSION_K(POINTS_VD)  ! LAT. DEPENDENT DIFFUSION*TIMESTEP
