...
C ******************************COPYRIGHT******************************
C
CLL  SUBROUTINE F_TYPE-----------------------------------------------
CLL
CLL  Purpose:  Returns each field code and associated field length from
CLL            the PP header and a count of the number of fields
CLL            of each type.
CLL
...
CLL version  Date
CLL
CLL   3.1  05/02/93    Trap use of user defined PPXREF file
CLL                    Author: A. Dickinson    Reviewer: R. Stratton
CLL 3.4  16/6/94 : Change CHARACTER*(*) to CHARACTER*(80) N.Farnon
CLL   3.5  May 95     Submodels project. Replace call to RDPPXRF by
CLL                   function EXPPXC to extract name of diagnostic
CLL                   item.        S.J.Swarbrick
CLL   4.1  Apr. 96    Rationalise *CALLs     S.J.Swarbrick
CLL   4.4  28/10/97   Change format of printout. D. Robinson.
CLL
...
! End of comdeck
     *TITLE)
      ...
      INTEGER
     * LEN2_LOOKUP             !IN 2nd dimension of LOOKUP
     *,N_TYPES                 !IN No of separate field types in file
     *,LOOKUP(64,LEN2_LOOKUP)  !IN LOOKUP record
     *,PP_NUM(LEN2_LOOKUP)     !OUT No of successive fields with same co
     *,PP_LEN(LEN2_LOOKUP)     !OUT Length of field
     *,PP_STASH(LEN2_LOOKUP)   !OUT PP code of field
     *,PP_TYPE(LEN2_LOOKUP)    !OUT Integer/real/timeseries
     *,PP_POS(LEN2_LOOKUP)     !OUT Pointer to number of PP field
     *,PP_LS(LEN2_LOOKUP)       !OUT Data stored on land or sea pts
     *,FIXHD(*)
...
C External subroutines called:------------------------------------------
      CHARACTER*36 EXPPXC
      EXTERNAL EXPPXC,ABORT_IO
C*----------------------------------------------------------------------
C*L  Local variables:---------------------------------------------------
      INTEGER
     * ICODE      ! Error code
     *,ITEM_CODE  ! STASH item code
...
C Initialise arrays
      ...
      PP_POS(1)=1
        IF(MOD(INT(LOOKUP(21,1)/10),10).EQ.2)THEN
          PP_LS(1)=MOD(INT(LOOKUP(21,1)/100),10)
        ENDIF

      DO K=2,LEN2_LOOKUP
        IF(LOOKUP(42,K).EQ.LOOKUP(42,K-1).AND.
     &     LOOKUP(15,K).EQ.LOOKUP(15,K-1))THEN
        ...
        IF(MOD(INT(LOOKUP(21,K)/10),10).EQ.2)THEN
          PP_LS(N_TYPES)=MOD(INT(LOOKUP(21,K)/100),10)
        ENDIF
        ENDIF
      ENDDO

C Print out details of fields
      WRITE(6,'(''  '',/,'' '',A/)')TITLE
      I=1
      DO K=1,N_TYPES
        PHRASE=' '
...
!         ELSE
!         CALL ABORT_IO('F_TYPE',CMESSAGE,ICODE,1)
!         ENDIF
!       ENDIF
        I=I+PP_NUM(K)
        WRITE(6,'('' '',I4,I5,I8,I4,2I6,2x,A36)')
     &   PP_LS(K),PP_NUM(K),PP_LEN(K)
     *  ,PP_TYPE(K),PP_STASH(K),PP_POS(K),PHRASE
      ENDDO
