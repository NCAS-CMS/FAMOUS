...
C ******************************COPYRIGHT******************************
C
CLL   SUBROUTINE READSTM
CLL     PURPOSE  TO READ A RECORD FROM THE PRE STASH-MASTERS FILE
CLL   AND RETURN THE PPXREF CODES AND NAME OF A GIVEN DIAGNOSTIC
CLL   TESTED UNDER CFT77 ON OS 5.1
CLL
CLL   AUTHOR            M.J.CARTER      DATE 23/01/92
CLL
...
CLL                   Add read of new record for sub-model type.
CLL   3.4    01/11/94 M.Carter. Change to stash-master file format
CLL                   to allow 4 digits for PP field code.
CLL   3.5    13/03/95 Sub-Models Project:
CLL                   Mods which facilitate expansion of PPXREF file to
CLL                   incorporate the entire preSTASHmaster record, and
CLL                   to have "Internal Model no." as an extra dimension
CLL                   in the file (in addition to section,item).
CLL                   S.J.Swarbrick
...
CLL
CLL   LOGICAL COMPONENT R913
CLL
CLL   PROJECT TASK: C4
CLL
CLL   PROGRAMMING STANDARD  UMDP 4
CLL
CLL   EXTERNAL DOCUMENT C4
CLL
CLLEND
      SUBROUTINE READSTM
...
C
 ...
 2010 FORMAT(2X,3(I5,2X),36A1)
      ...
      IF(CODES(ppx_model_number).EQ.-1) GO TO 9999           
...
! 20-digit option code read in as 4x5 digit groups
      READ(NFT,2120,END=3100,ERR=3200)
     &(opcod(ii),ii=1,20),
     &(IMASK(II),II=1,20)
 2120 FORMAT(3X,20(I1),3X,20(I1))
