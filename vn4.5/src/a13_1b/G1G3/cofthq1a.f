...
C ******************************COPYRIGHT******************************
C
CLL   SUBROUTINE COEFF_TH_Q -----------------------------------------
CLL
CLL   PURPOSE:  CALCULATES EFFECTIVE DIFFUSION COEFFICIENTS
CLL              FOR THETAL OR Q IN NS AND EW DIRECTIONS
...
CLL VERSION  DATE
CLL  4.0  03/02/95  NEW SUBROUTINE CALLED BY DIF_CTL
CLL                 AUTHOR: T.DAVIES FR.  REVIEWER: M MAWSON
!     4.1    07/05/96 Added MPP code and TYPFLDPT arguments and fixed
!                     bug in wrap-arounds    P.Burton
CLL
...
C*L   ARGUMENTS:---------------------------------------------------
      SUBROUTINE COEFF_TH_Q
     1                  (DIFFUSION_EW,DIFFUSION_NS,
     2                   PRESSURE,LEVEL,PRESSURE_TEST,AK,BK,
     ...
     *, LEVEL              ! MODEL LEVEL FOR DIFFUSION
...
! End of comdeck TYPFLDPT
      ...
      REAL
     * DIFFUSION_EW(P_FIELD)           !OUT HOLDS EAST-WEST
     *                                 !EFFECTIVE DIFFUSION COEFFICIENT.
     *,DIFFUSION_NS(P_FIELD)           !OUT HOLDS NORTH_SOUTH
     *                                 !EFFECTIVE DIFFUSION COEFFICIENT.

      REAL
     * PRESSURE(P_FIELD,P_LEVELS)      !IN HOLDS 3-D PRESSURE FIELD
     *           ! LEVEL=1 IS SURFACE THEN LEVEL=K IS MODEL LEVEL K-1
     *,DIFFUSION_COEFFICIENT(U_FIELD)  !IN HOLDS EAST-WEST DIFFUSION
     *                                 ! COEFFICIENT.
     *,DIFFUSION_COEFFICIENT2(U_FIELD) !IN HOLDS NORTH-SOUTH DIFFUSION
     *                                 ! COEFFICIENT.
     ...
     *, PRESSURE_TEST      ! PRESSURE ALTITUDE LIMIT FOR SLOPE TEST
...
CL
CL---------------------------------------------------------------------
CL    SECTION 1.    DELTALAMBDA TERMS
CL---------------------------------------------------------------------
      LEVEL_P=LEVEL+1
C----------------------------------------------------------------------
C     LEVEL_P =LEVEL+1 SINCE LEVEL_P=1 IS THE SURFACE
...
C----------------------------------------------------------------------
      ...
      DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO
        DIFFUSION_EW(I) = 0.5*(DIFFUSION_COEFFICIENT(I-ROW_LENGTH)+
     &              DIFFUSION_COEFFICIENT(I))*LONGITUDE_STEP_INVERSE
     &              *LONGITUDE_STEP_INVERSE
      END DO

C----------------------------------------------------------------------
CL    TEST TO SEE IF  DIFFUSION COEFFICIENT SET TO ZERO
C     IF STEEP SLOPE AT PRESSURE > PRESSURE_TEST ONLY
...
C                  IF STEEP SLOPE BELOW CHOSEN LEVEL
C----------------------------------------------------------------------

      DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO-1
        MASK(I)=((PRESSURE(I,LEVEL_P).GT.PRESSURE(I+1,LEVEL_P-1)).OR.
     &           (PRESSURE(I,LEVEL_P).LT.PRESSURE(I+1,LEVEL_P+1)))
      ENDDO


! And zero appropriate points of EW_DIFFUSION
      DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO
        IF (MASK(I)) DIFFUSION_EW(I)=0.0
      ENDDO
...
C----------------------------------------------------------------------
CL    SECTION 2    CALCULATE NS EFFECTIVE DIFFUSION
C----------------------------------------------------------------------

C   CALCULATE DELTAPHI TERMS
C   DELTALAMBDAK*COSLAT/(DELTAPHI)SQUARED

      DO  I= START_POINT_NO_HALO-ROW_LENGTH+1,END_P_POINT_NO_HALO
       DIFFUSION_NS(I)=0.5*(DIFFUSION_COEFFICIENT2(I)*COS_U_LATITUDE(I)
     &             +DIFFUSION_COEFFICIENT2(I-1)*COS_U_LATITUDE(I-1))*
...
C     RECALCULATE END-POINTS
      ...
      DIFFUSION_NS(START_POINT_NO_HALO-ROW_LENGTH)=
     &  DIFFUSION_NS(START_POINT_NO_HALO-ROW_LENGTH+1)
...
C
      IF(PRESSURE_LEVEL.GT.PRESSURE_TEST)THEN
       ...
       IF((PRESSURE(I+ROW_LENGTH,LEVEL_P).GT.PRESSURE(I,LEVEL_P-1)).OR.
     &    (PRESSURE(I+ROW_LENGTH,LEVEL_P).LT.
     &         PRESSURE(I,LEVEL_P+1)))THEN
          DIFFUSION_NS(I)=0.0
        ENDIF
       END DO
