...
CLL VERSION  DATE
!     4.1    30/06/95  Minor change to P_TO_UV for MPP version and
!                      added TYPFLDPT arguments
!                      P.Burton
!LL 4.2      16/08/96 Added TYPLDPT variables.
!LL                   Made FILTER_WAVE_NUMBER_U_ROWS globally sized.
!LL                                                        P.Burton
...
!       LAST WAVE NUMBER NOT TO BE CHOPPED
     ...
     *,RS_FUNCTIONS(P_FIELD,P_LEVELS) !IN RS*RS*DELTA P
...
C REAL SCALARS
      REAL
     *  SCALAR1(LAST_P_VALID_PT-FIRST_VALID_PT+1),
     *  SCALAR2(LAST_P_VALID_PT-FIRST_VALID_PT+1),
     *  SCALAR3
...
C COUNT VARIABLES FOR DO LOOPS ETC.
     ...
     *               ! ROUTINE.
     *, n_input      ! no. of inputs for sqrt_v function
...
CL    CALCULATE RS*DELTA P FROM RS*RS*DELTA P HELD IN RS_FUNCTIONS AND
CL    DELTA P.
        DO I=FIRST_VALID_PT,LAST_P_VALID_PT         
          SCALAR1(I-FIRST_VALID_PT+1)
     &              = DELTA_AK(K) + DELTA_BK(K)*PSTAR(I)  
          SCALAR2(I-FIRST_VALID_PT+1)  
     &              = RS_FUNCTIONS(I,K)/SCALAR1(I-FIRST_VALID_PT+1)     
        END DO  

        n_input=  LAST_P_VALID_PT-FIRST_VALID_PT+1
        DO I=1,n_input
          scalar2(I)=sqrt(scalar2(I))
        END DO

        DO I=FIRST_VALID_PT,LAST_P_VALID_PT
          WORK(I) = SCALAR1(I-FIRST_VALID_PT+1) * 
     &              SCALAR2(I-FIRST_VALID_PT+1)  
        END DO  
