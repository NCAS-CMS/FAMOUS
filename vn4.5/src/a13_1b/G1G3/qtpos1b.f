...
!   4.2   28/10/96  New deck for HADCM2-specific section A13_1B,
!                   as QTPOS1A but with inconsistent 'old' type
!                   of polar weights.  T.Johns
!    4.3    24/04/97  Improved error trapping for MPP code
!                     Set QTFIX to safe values         P.Burton
!    4.4    22/09/97  Correct missetting of local variables  T Johns
...
!                                ! but using integers
     ...
     &, LATITUDE_STEP_INVERSE    ! Computed 1/(DELTA_LAMBDA)
     &, COS_P_LAT_NP             ! COS(latitude) at North Pole
     &, COS_P_LAT_SP             ! COS(latitude) at South Pole
     &, SEC_P_LAT_NP             ! SEC(latitude) at North Pole
     &, SEC_P_LAT_SP             ! SEC(latitude) at South Pole
! See note below concerning rotated global grids
...
! Recalculate LATITUDE_STEP_INVERSE to avoid changing argument list
! (HADCM2-specific code, needs *CALL C_PI)

      LATITUDE_STEP_INVERSE=RECIP_PI_OVER_180 *
     &           REAL((GLOBAL_P_FIELD/GLOBAL_ROW_LENGTH)-1) / REAL(180)
      ...
      ELSE ! global correction scheme
        I_start=GLOBAL_ROW_LENGTH
        I_end=GLOBAL_P_FIELD+1-GLOBAL_ROW_LENGTH
        IF (at_top_of_LPG) THEN
          I_start_local=TOP_ROW_START+LAST_ROW_PT-1
        ELSE
          I_start_local=FIRST_FLD_PT
        ENDIF
        IF (at_base_of_LPG) THEN
          I_end_local=P_BOT_ROW_START+FIRST_ROW_PT-1
        ELSE
          I_end_local=LAST_P_FLD_PT
        ENDIF

        IF (at_top_of_LPG .AND. at_right_of_LPG) THEN
          COS_P_LAT_NP=COS_P_LATITUDE(1)
          SEC_P_LAT_NP=SEC_P_LATITUDE(1)
          COS_P_LATITUDE(TOP_ROW_START+LAST_ROW_PT-1)=
     &      .125*GLOBAL_ROW_LENGTH/LATITUDE_STEP_INVERSE
          SEC_P_LATITUDE(TOP_ROW_START+LAST_ROW_PT-1)=
     &      1.0/COS_P_LATITUDE(TOP_ROW_START+LAST_ROW_PT-1)
        ENDIF

        IF (at_base_of_LPG .AND. at_left_of_LPG) THEN
       COS_P_LAT_SP=COS_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)
       SEC_P_LAT_SP=SEC_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)
          COS_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)=
     &      .125*GLOBAL_ROW_LENGTH/LATITUDE_STEP_INVERSE
          SEC_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)=
     &      1.0/COS_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)
        ENDIF
...
! Now remove the mass weighting, and calculate QT_FIX diagnostic
     ...
     &              SEC_P_LATITUDE(I)
      ...
      IF (.NOT. L_QT_POS_LOCAL) THEN

! Set all points at poles equal to last(NP)/first(SP) point
! Set up arrays containing NP values
          IF (at_top_of_LPG .AND. at_right_of_LPG) THEN
            DO K=1,Q_LEVELS
              NP_VALUE(K,1)=QT(I_start_local,K)
              NP_VALUE(K,2)=QTFIX(I_start_local,K)
            ENDDO
          ELSE
            DO K=1,Q_LEVELS
              NP_VALUE(K,1)=0.0
...
! Distribute the NP value over processors on polar row
          IF (at_top_of_LPG) THEN
            CALL GCG_RSUM(Q_LEVELS,GC_ROW_GROUP,info,NP_VALUE(1,1))
            DO K=1,Q_LEVELS
            ...
            ENDDO
            IF (SF_QTFIX) THEN
              CALL GCG_RSUM(Q_LEVELS,GC_ROW_GROUP,info,NP_VALUE(1,2))
              DO K=1,Q_LEVELS
                DO I=TOP_ROW_START,TOP_ROW_START+ROW_LENGTH-1
                  QTFIX(I,K)=NP_VALUE(K,2)
                ENDDO
              ENDDO
            ENDIF
          ENDIF

! Set up arrays containing SP values
          IF (at_base_of_LPG .AND. at_left_of_LPG) THEN
            DO K=1,Q_LEVELS
              SP_VALUE(K,1)=QT(I_end_local,K)
              SP_VALUE(K,2)=QTFIX(I_end_local,K)
            ENDDO
          ELSE
            DO K=1,Q_LEVELS
              SP_VALUE(K,1)=0.0
...
! Distribute the SP value over processors on polar row
          IF (at_base_of_LPG) THEN
            CALL GCG_RSUM(Q_LEVELS,GC_ROW_GROUP,info,SP_VALUE(1,1))
            DO K=1,Q_LEVELS
            ...
            IF (SF_QTFIX) THEN
              ...
              DO K=1,Q_LEVELS
                DO I=P_BOT_ROW_START,P_BOT_ROW_START+ROW_LENGTH-1
                  QTFIX(I,K)=SP_VALUE(K,2)
                ENDDO
              ENDDO
            ENDIF
          ENDIF
! Reset the pole rows of latitude

        IF (at_top_of_LPG .AND. at_right_of_LPG) THEN
          COS_P_LATITUDE(TOP_ROW_START+LAST_ROW_PT-1)=COS_P_LAT_NP
          SEC_P_LATITUDE(TOP_ROW_START+LAST_ROW_PT-1)=SEC_P_LAT_NP
        ENDIF

        IF (at_base_of_LPG .AND. at_left_of_LPG) THEN
          COS_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)=COS_P_LAT_SP
          SEC_P_LATITUDE(P_BOT_ROW_START+FIRST_ROW_PT-1)=SEC_P_LAT_SP
        ENDIF
      ENDIF ! IF (.NOT. L_QT_POS_LOCAL)
...
CLL VERSION  DATE
!LL   4.3   14/04/97  Minor updates in line with QTPOS1A.  T Johns
CLL
...
C   If QT is negative find 8 neighbours ...
              ...
              JE=1
              JW=-1
              IF(NPT.EQ.ROW_LENGTH) JE=1-ROW_LENGTH
              IF(NPT.EQ.1) JW=ROW_LENGTH-1
...
C   ... and sum QT at neighbouring points
                ...
                DO II=IN,IS
                  DO JJ=JW,JE
                    ...
                    IF(JJ.GT.ROW_LENGTH) POINTER = POINTER-ROW_LENGTH
                    IF(JJ.LT.1) POINTER = POINTER+ROW_LENGTH
...
C
C  Loop round polar points
CL
CL---------------------------------------------------------------------
CL    SECTION 4.     STEP 2. IF GLOBAL MODEL DEAL WITH POLES IN
CL                   SIMILAR FASHION EXCEPT WORKING ROW AT A TIME.
CL---------------------------------------------------------------------
C  Set all ploar points equal
          TEMP1=0.0
          TEMP2=0.0
          DO I=1,ROW_LENGTH
            TEMP1=TEMP1+QT(I,K)
            TEMP2=TEMP2+QT(P_FIELD-ROW_LENGTH+I,K)
          END DO
          DO I=1,ROW_LENGTH
            QT(I,K)=TEMP1/ROW_LENGTH
            QT(P_FIELD-ROW_LENGTH+I,K)=TEMP2/ROW_LENGTH
          END DO
C  Check for negative QT at north pole
          IF(QT(1,K).GT.0.0) THEN
            SUM_NEIGHBOURS=0.0
            DO I=1,ROW_LENGTH
              SUM_NEIGHBOURS = SUM_NEIGHBOURS + QT(ROW_LENGTH+I,K)
            END DO
            IF(SUM_NEIGHBOURS.LT.0.0) THEN
              FACTOR(K)=1.0+QT(1,K)*ROW_LENGTH/SUM_NEIGHBOURS
              IF(FACTOR(K).GE.0.0) THEN
                DO I=1,ROW_LENGTH
                  QT(I,K)=0.0
                  QT(ROW_LENGTH+I,K)=QT(ROW_LENGTH+I,K)*FACTOR(K)
                END DO
                GOTO 400    ! Correction performed
              END IF
            END IF
C
C  No correction possible: global QT in layer is negative
            FAILURE(K)=.TRUE.

          END IF
  400     CONTINUE
C  Check for negative QT at south pole
          IF(QT(P_FIELD-1,K).GT.0.0) THEN
            SUM_NEIGHBOURS=0.0
            DO I=1,ROW_LENGTH
              SUM_NEIGHBOURS=SUM_NEIGHBOURS+QT(P_FIELD+1-ROW_LENGTH-I,K)
            END DO
            IF(SUM_NEIGHBOURS.LT.0.0) THEN
              FACTOR(K)=1.0+QT(P_FIELD-1,K)*ROW_LENGTH/SUM_NEIGHBOURS
              IF(FACTOR(K).GE.0.0) THEN
CDIR$ IVDEP
                DO I=1,ROW_LENGTH
                  QT(P_FIELD-ROW_LENGTH+I,K)=0.0
                  QT(P_FIELD+1-ROW_LENGTH-I,K) =
     *                       QT(P_FIELD+1-ROW_LENGTH-I,K)*FACTOR(K)
                END DO
