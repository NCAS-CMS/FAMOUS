...
CLL VERSION  DATE
!LL   4.2   28/11/96  New deck for HADCM2-specific section A13_1B,
!LL                   as THQDIF1A but with reintroduced 'bug' in
!LL                   value of SCALAR weight at poles.    T.Johns
CLL
...
C GLOBAL MODEL CALCULATES POLAR DEL-SQUARED USING ACROSS-POLE DIFFERENCE
C ASSUMING FLUX=0 AT HALF-GRID-LENGTH ON OTHER SIDE OF POLE


! Note - 2.0* factor is incorrect but consistent with HADCM2
      SCALAR=2.0*SEC_P_LATITUDE(TOP_ROW_START)

! Do North Pole
      IF (at_top_of_LPG) THEN
        DO I=1,ROW_LENGTH
          J=TOP_ROW_START+I-1
          FIELD3(J)=-DIFFUSION_NS(J)*FIELD1(J)*SCALAR
          NP_FLUX(I)=FIELD3(J)
          FIELD_INC(J)=0.0
        ENDDO
      ENDIF
...
! And South Pole
! Note - 2.0* factor is incorrect but consistent with HADCM2
      SCALAR=2.0*SEC_P_LATITUDE(P_BOT_ROW_START)

      IF (at_base_of_LPG) THEN
        DO I=1,ROW_LENGTH
          J=I+P_BOT_ROW_START-1
          FIELD3(J)=DIFFUSION_NS(J-ROW_LENGTH)*FIELD1(J-ROW_LENGTH)*
     &              SCALAR
          SP_FLUX(I)=FIELD3(J)
          FIELD_INC(J)=0.0
        ENDDO
...
CL    CALL POLAR TO UPDATE POLAR VALUES

      CALL POLAR(FIELD_INC,NP_FLUX,SP_FLUX,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
     &  FIRST_VALID_PT , LAST_P_VALID_PT , LAST_U_VALID_PT,
