...
C ******************************COPYRIGHT******************************
C
CLL Subroutine EXTDIAG ------------------------------------------------
CLL
CLL Purpose : To extend a set of arrays of diagnostic values to full
CLL           horizontal arrays for STASH processing. The number of
CLL           levels in each array is found from STASH control routine.
CLL Service routine
CLL version for Cray YMP
CLL
CLL CW, WI      <- programmer of some or all of previous code or changes
CLL
CLL  Model            Modification history from model version 3.0:
CLL version  Date
CLL   3.2    13/07/93 Changed CHARACTER*(*) to CHARACTER*(80) for
CLL                   portability.  Author Tracey Smith.
!LL  4.3     12/02/97 Added PPX arguments and MPP code   P.Burton
!LL  4.4     05/09/97 Initialise superpolar rows to prevent NaNs getting
!LL                   into fields. S.D.Mullerworth
CLL
CLL Programming Standard : Unified Model Documentation paper number 4
CLL                      : Version no2, dated 18/01/90
CLL
CLL System components covered : D3
CLL
CLL System task : P0
CLL
CLL Documentaton : UM Documentation Paper No P0, version 11 dated
CLL                                       version 11 dated (26/22/90)
CLLEND ------------------------------------------------------------

C*L ARGUMENTS:

      SUBROUTINE EXTDIAG (DATA,SI,SF,I1,I2,
     &                    LEN_DATA,ROW_LENGTH,STLIST,LEN_STLIST,
     &                    STINDEX,LEN_STINDEX,
...
! End of comdeck
     &                    ICODE, CMESSAGE)
      ...
      INTEGER
     &       I1,          ! Item numbers of diagnostic
     &       I2,          ! values to be extended
     &       SI(I2),      ! Address of fields in DATA array
     &       LEN_DATA,    ! Length of input data array
     &       ROW_LENGTH,  ! Number of points in row, assumed the
C                         ! same for all fields.
     &       LEN_STLIST,  !
     &       STLIST(LEN_STLIST,*), !STASH list
     &       LEN_STINDEX,
     &       STINDEX(LEN_STINDEX,I2), !STASH index
     &       LEN_STASHLEVELS, !
     &       STASH_LEVELS(LEN_STASHLEVELS,*), ! STASH levels list.
     &       NUM_STASH_PSEUDO,     ! Size of:
     &       STASH_PSEUDO_LEVELS(NUM_STASH_PSEUDO+1,*), ! STASH pseudo
C                                                       !   levels list
     &       ICODE        ! Return code =0 Normal exit
C                                       >1 Error
      INTEGER
     &  im_ident  ! IN: internal model identifier of STASH data
     &, is        ! IN: section number of STASH data

      CHARACTER*80 CMESSAGE

      REAL
     &       DATA(LEN_DATA)! Input data

      LOGICAL
     &       SF(I2)        ! Logical switch set .TRUE. if field to
C                          ! be extended.
C*
!
! Description:
...
C     Local variables
      ...
      REAL
     &       NP_MEAN(P_LEVELS_MAX)   ! NP mean value
     &,      SP_MEAN(P_LEVELS_MAX)   ! SP mean value

      INTEGER
     &       ITEM,        ! STASH item number
     &       FIELD,       ! Length of individual diagnostic field
     &       I,           !
     &       J,           ! LAM E boundary pointer
     &       K,           !
     &       INDEX,       ! Position within STASH list, refering to
C                         ! current ITEM
     &       LEVELS,      !
     &       START        ! Position of first point of field at level
C                         ! to  be processed.

      INTEGER
     &  gr         ! grid type of field being processed
     &, fld_type   ! field type (P or U) of the field being processed
     &, sp_row_start     ! start point of south pole
     &, np_row_start     ! start point of north pole
     &, lvl,lsl,lso,nv,info  ! arguments to RVECSUMR
...
CL Calculate number of levels at which each diagnostic is held

          INDEX=STINDEX(1,ITEM)

          IF(STLIST(10,INDEX).LT.0) THEN

C Levels list present

            LEVELS=STASH_LEVELS(1,-STLIST(10,INDEX))
          ELSEIF(STLIST(10,INDEX).LT.100) THEN

C Top and bottom levels

            LEVELS=STLIST(11,INDEX)-STLIST(10,INDEX)+1
            IF(LEVELS.LE.0) THEN
              ICODE=1
              CMESSAGE='EXTDIAG: Illegal levels limits.'
              WRITE(6,*)'EXTDIAG illegal levels, STLIST(10,11)=',
     &        STLIST(10,INDEX),STLIST(11,INDEX)
       WRITE(6,*)' SECTION AND ITEM NUMBER ',
     *     STLIST(2,INDEX),STLIST(1,INDEX)
              RETURN
            END IF
          ELSE
            LEVELS=1
...
C The 26th entry of STLIST indicates if the diagnostic is input to STASH
C   on pseudo-levels and, if so, which - zero for none, otherwise the
C   number of the list of pseudo-levels the data will be input to STASH
C   on, and the first element of this list is the total number in it.
C   (NUM_PSEUDO_LEVELS being the maximum value over all pseudo-levels
C   lists.)  So LEVELS, which is currently the number of physical levels
C   the data is on, must be multiplied by this to give the number of
C   combinations of physical levels and pseudo-levels.

          IF ( STLIST(26,INDEX) .GT. 0 )
     &      LEVELS = LEVELS * STASH_PSEUDO_LEVELS(1,STLIST(26,INDEX))

CL Define length of field

! Find out the gridtype of the field
          GR = EXPPXI(im_ident,IS,ITEM,ppx_grid_type,
! COMDECK ARGPPX     
! Description:
!   Argument comdeck for passing arrays which hold the
!   ppxref information.
...
! and use this to find the field type (p field or u field)
          ...
          IF (atbase) THEN
            IF (fld_type .EQ. fld_type_p) THEN
              sp_row_start=(lasize(2)-Offy-2)*ROW_LENGTH+Offx+1
            ELSEIF (fld_type .EQ. fld_type_u) THEN
              sp_row_start=(lasize(2)-Offy-3)*ROW_LENGTH+Offx+1
            ELSE
              ICODE=1
              CMESSAGE='EXTDIAG : Unrecognized field type'
              GOTO 9999
            ENDIF
          ENDIF
          IF (attop)
     &      np_row_start=(Offy+1)*ROW_LENGTH+1+Offx

          FIELD=lasize(1)*lasize(2)


CL Extend diagnostic to full field

! Calculate polar mean values
          ...
          lvl=FIELD  ! size of each field
          lsl=ROW_LENGTH-2*Offx ! size of vector to sum
          nv=LEVELS ! number of vectors

          IF (attop) THEN

            lso=np_row_start ! start address of vector

            CALL GCG_RVECSUMF(lvl,lsl,lso,nv,DATA(SI(ITEM)),
     &                        gc_proc_row_group,info,NP_MEAN)
            DO K=1,LEVELS
              NP_MEAN(K)=NP_MEAN(K)/glsize(1)
            ENDDO
          ...
          IF (atbase) THEN
            ...
            lso=sp_row_start ! start address of vector

            CALL GCG_RVECSUMF(lvl,lsl,lso,nv,DATA(SI(ITEM)),
     &                        gc_proc_row_group,info,SP_MEAN)
            DO K=1,LEVELS
              SP_MEAN(K)=SP_MEAN(K)/glsize(1)
            ENDDO

          ENDIF
          DO K=1,LEVELS
            START=FIELD*(K-1)+SI(ITEM)-1

CL Copy diagnostic information to N and S boundaries

! Update Northern boundary / Pole
            IF (attop) THEN
!L Initialise unused row
! *DIR$ CACHE_BYPASS DATA
              DO I=1,Offy*ROW_LENGTH
                DATA(START+I)=0.
              ENDDO

              DO I=Offy*ROW_LENGTH+1,(Offy+1)*ROW_LENGTH
                DATA(START+I)=NP_MEAN(K)
              ENDDO
            ENDIF

! Update Southern boundary / Pole
            IF (atbase) THEN
!L Initialise unused row
! *DIR$ CACHE_BYPASS DATA
              DO I=sp_row_start+2*ROW_LENGTH-Offx+1,
     &             lasize(1)*lasize(2)
                DATA(START+I)=0.
              ENDDO

              DO I=sp_row_start-Offx+ROW_LENGTH,
     &             sp_row_start+2*ROW_LENGTH-Offx
                DATA(START+I)=SP_MEAN(K)
              ENDDO
            ENDIF
