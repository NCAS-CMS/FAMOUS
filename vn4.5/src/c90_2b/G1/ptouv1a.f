...
CLL             points on an Arakawa B grid. Under UPDATE
CLL             identifier GLOBAL the data is assumed periodic along
CLL             rows. Otherwise, the last value on each row is set equal
CLL             to the penultimate value on each row. The output arra
CLL              contains one less row than the input array.
CLL
CLL  Not suitable for single column use.
CLL
CLL  Written 22/8/89 by A. Dickinson
CLL
CLL  Model            Modification history from model version 3.0:
CLL version  date
CLL  3.2     25/03/93   Removed one-sided differencing for first and
CLL                     last points on row if not global.
CLL                     Author: C.A.Wilson     Reviewer: A.Dickinson
!   3.5    28/03/95 MPP code: Take account of halos    P.Burton
!   4.1    02/04/96 Addition to MPP code    P.Burton
!   4.2    10/10/96 Extra brackets to ensure bit-reproducability
!                   between single/multi-processor runs. D. Robinson
CLL
...
C---------------------------------------------------------------------
      ...
      DO 200 I=1,U_POINTS-1
       U_DATA(I)=0.25*( ( P_DATA(I)+P_DATA(I+1) ) +
     &           ( P_DATA(I+ROW_LENGTH)+P_DATA(I+1+ROW_LENGTH) ) )
200   CONTINUE
