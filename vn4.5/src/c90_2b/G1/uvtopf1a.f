...
C---------------------------------------------------------------------
CL  5.Recalculate first and last rows for limited area model by
CL                    setting equal to internal rows
C---------------------------------------------------------------------
      IF (attop) THEN
        DO I=Offy*ROW_LENGTH+1,(Offy+1)*ROW_LENGTH
          P_DATA(I)=P_DATA(I+ROW_LENGTH)
        ENDDO
      ENDIF

      IF (atbase) THEN
        DO I=P_FIELD-(Offy+1)*ROW_LENGTH+1,P_FIELD-Offy*ROW_LENGTH
          P_DATA(I)=P_DATA(I-ROW_LENGTH)
        ENDDO
      ENDIF
C---------------------------------------------------------------------
CL  6.Recalculate first points in each row for the limited
CL      area model by setting equal to second points
C---------------------------------------------------------------------

      IF (atleft) THEN
        DO I=Offx+1,P_FIELD,ROW_LENGTH
          P_DATA(I)=P_DATA(I+1)
        ENDDO
      ENDIF
