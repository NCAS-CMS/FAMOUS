...
CLL   is used.
CLL
CLL       In 1991 Extensive revision bringing code into line with
CLL              vn5.0 of REPLANCA
CLL
CLL CW SI       <- programmer of some or all of previous code or changes
CLL
...
CLL  3.4  06/10/94  Add user ancillaries.                     RTHBarnes
CLL  3.4  04/08/94  Extended to allow multi-level ancillary fields. MB
CLL   3.4  16/06/94   DEF CAL360 replaced by LOGICAL LCAL360
CLL                   LCAL360 passed to TIME2SEC, SEC2TIME
CLL                                                  S.J.Swarbrick
...
!                   Author D.M. Goddard.
!LL   4.4  05/09/97   Initialise ANCIL arrays. S.D.Mullerworth
CLL
...
C                                         ! referring to data set
     ...
     &       RLOOKUP(LEN1_LOOKUP,NLOOKUPS)
      ...
      INTEGER
     &       I_AO,     ! Atmosphere_Ocean indicator = 1 Atmosphere
C                                                   = 2 Ocean
     &       ICODE,    ! Return code
...
C     Ocean fields
C     FIELD=  1 Land Sea Mask
C             2 Ocean Depth
C             3 Zonal wind stress
C             4 Meridional wind stress
C             5 Wind mixing energy
C             6 Surface pressure
C             7 Solar heat flux at sea surface
C             8 Remaining heat flux at sea surface
C             9 Precipitation minus Evaporation
C            10 River inflow
C            11 Water type
C            12 Solar radiation over ice
C            13 Snowfall
C            14 Sublimation
C            15 Reference surface temperature
C            16 Reference surface salinity
C            17 Climatological air temperature
C            18 Climatological ice depth
C            19 Anomalous heat flux
C            20 Anomalous salinity flux
C            21 Ice depth correction (heat)
C            22
C            23
C            24 Single level user ancillary 1
C            25 Single level user ancillary 2
C            26 Single level user ancillary 3
C            27 Single level user ancillary 4
C            28 Single level user ancillary 5
C            29 Single level user ancillary 6
C            30 Single level user ancillary 7
C            31 Single level user ancillary 8
C            32 Single level user ancillary 9
C            33 Single level user ancillary 10
C            34 Multi-level user ancillary 1
C            35 Multi-level user ancillary 2
C            36 Multi-level user ancillary 3
C            37 Multi-level user ancillary 4

      IOUNIT=0
      INCR_SEC = 0

! Initialise arrays - particularly for unused rows in MPP U fields
! *DIR$ CACHE_BYPASS ANCIL1,ANCIL2
      DO I = 1,IMT*JMT
        ANCIL1(I)=0.0
        ANCIL2(I)=0.0
      ENDDO
...
C         UPDATE(FIELD)=MOD(O_STEP,STEPS(FIELD)).EQ.0
          UPDATE(FIELD)=(MOD(O_STEP+OFFSET_STEPS,STEPS(FIELD)).EQ.0
     &                   .OR.O_STEP.EQ.0)
...
CL Loop over ancillary fields (ocean)
...
C     Update required for field
      ...
      IF (L_OPRINT) WRITE(6,*)'REPLANCO:UPDATE REQUIRED FOR FIELD',FIELD
...
CL Adjust time to middle of updating interval
            ...
            IF(.NOT.LGREG_MONTHLY) THEN
              SEC=SEC+STEPS(FIELD)*O_SECS_P_P/(O_STEPS_P_P*2)
...
C  & update with values for half a period before first standard update.
              IF (O_STEP.EQ.0) THEN
                DAY1 = DAY
                SEC1 = SEC
      INCR_SEC = -O_SECS_P_P*MOD(OFFSET_STEPS,STEPS(FIELD))/O_STEPS_P_P
...
CL FInd time(in seconds) of first ancillary data on file
     ...
     &                   LCAL360)
              ...
              IF(SECONDS.LT.0) THEN
                ICODE=400+FIELD
           CMESSAGE='REPLANCO: Current time precedes start time of data'
                RETURN
              END IF
...
CL   Do not allow non-standard periods
      ...
      IF (LCAL360) THEN
              ...
              IF(PERIOD.NE.8640.AND.PERIOD.NE.720.AND.PERIOD.NE.24)THEN
                ICODE=600+FIELD
           CMESSAGE='REPLANCO: Non-standard period for periodic data'
                RETURN
              ENDIF
      ELSE
...
CL Adjust YMD time back to start of updating interval
      ...
      IF (LINTERPOLATE.AND.L_OPRINT) THEN   
        WRITE(6,*)' REPLANCO - time interpolation for field ',field
...
CL Check time interpolation factors
          IF(TIME.LT.TIME1.OR.TIME.GT.TIME2) THEN
           ...
           ICODE=500+FIELD
           CMESSAGE='REPLANCO: TIME INTERPOLATION ERROR'
           RETURN
          END IF
        END IF
...
C  LEN_FLD is length of first field to be read
       LEN_FLD = IMT*JMT
        ...
        IF ( LEN_FLD .GT. IMT*JMT ) THEN
          ...
          CMESSAGE='REPLANCO : field length error'
          GO TO 900
        END IF
