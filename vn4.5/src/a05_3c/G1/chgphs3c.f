...
!LL   4.5   19/05/98  Allow treatment of precip below cloud base as
!LL                   in version 3A or 3B (HADAM3) of convection, in 
!LL                   optimised code (version 3C).     Julie Gregory
CLL
...
C                                  !    change of precip should be 
C                                  !    limited as in hadam3 physics.
C----------------------------------------------------------------------
...
C
        DO I=1,NPNTS
        BPPNWT_K = THE_K(I)*EXK(I).GT.TM
        BPPNWT_KM1 = THE_KM1(I)*EXKM1(I).GT.TM
        FACTOR = LF*G/(EXKM1(I)*CP*DELPKM1(I))
C FREEZE
        IF (.NOT.BPPNWT_KM1.AND.(BPPNWT_K.OR.RAIN(I).GT.0.0)) THEN
