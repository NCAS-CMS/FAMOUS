...
!LL                   Not bit-reproducible with CONVEC3A.
!LL                                                     Alan Dickinson
!LL  4.4   17/10/97  Loop splitting by hand for T3E optimisation
!LL                                                   D.Salmond
...
!LL  4.5   30/04/98  Loop splitting by hand for T3E optimisation
!LL  4.5   19/05/98  Pass L_PHASE_LIM down to DD_CALL to determine
!LL                  treatment of precip below cloud base. Julie Gregory
!LL  4.5   5/6/98    Updraught factor and L_CLOUD_DEEP passed into 
!LL                  convection as part of anvil scheme.   Julie Gregory
CLL   4.5    Jul. 98  Kill the IBM specific lines (JCThil)
!LL  4.5   13/02/98  Add missing code in CONVEC3C for new CCA/Anvil
!LL                  scheme. D. Robinson.
CLL
CLL
...
C
     ...
     *                   AKM12,BKM12,DELAK,DELBK,TIMESTEP,T1_SD,Q1_SD,
     ...
     &     ,l_halo
     &                  ,UD_FACTOR,L_CLOUD_DEEP,L_PHASE_LIM
...
!                                 !    with height (3D), or not (2D)
      LOGICAL L_CCW               ! IN Switch for allowing precip
                                  ...
                                  !    path.
!
      LOGICAL L_PHASE_LIM         ! IN Switch to determine if phase 
!                                 !    change of precip is limited so LH
!                                 !    does not make temp go other side 
!                                 !    of TM.
!
...
C
      PARAMETER (SAFETY_MARGIN = TINY(1.0) )
!
C
...
CL
CL UM DOCUMENTATION PAPER 27-3. SECTION 2.
CL
...
!OCL NOVREC
      ...
      IF(FLG_ENTR_UP) THEN
      DO I=1,NINIT
        ENTRAIN_UP(INDEX3(I),K)=(1.0-DELTAK(I))*
     &             (1.0-WORK(I,14))*(WORK2(I,11)+WORK2(I,12)*
     &             (1.0+WORK2(I,11)))*FLX(INDEX3(I),K)
        IF(BTERM(INDEX3(I))) ENTRAIN_UP(INDEX3(I),K+1)=0.0 
       END DO
      ENDIF
      IF(FLG_DETR_UP) THEN
       DO I=1,NINIT
        DETRAIN_UP(INDEX3(I),K)=-(WORK(I,14)+
     &                        DELTAK(I)*(1.0-WORK(I,14)))*
     &                        FLX(INDEX3(I),K)
        IF(BTERM(INDEX3(I))) DETRAIN_UP(INDEX3(I),K+1)=
     &                      -(1.0-DELTAK(I))*FLX(INDEX3(I),K)
       END DO 
      ENDIF     
      DO I=1,NINIT                                                      
...
C
C UM DOCUMENTATION PAPER 27-3. SECTION 5.
C
