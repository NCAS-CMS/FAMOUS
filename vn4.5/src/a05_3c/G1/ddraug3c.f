...
C
     ...
     &                     BDD_ON,B_DD_END,CCA,L_MOM,UDD_K,VDD_K,
     &                     UE_K,VE_K,UE_KM1,VE_KM1,DUBYDT_K,DUBYDT_KM1,
     &                     DVBYDT_K,DVBYDT_KM1,DELUD,DELVD,EFLUX_U_DD,
     &                     EFLUX_V_DD,L_TRACER,
...
C
      LOGICAL L_MOM                 ! IN SWITCH FOR INCLUSION
...
C
      REAL QDD_K(NPNTS)             ! INOUT
                                    ...
                                    !     LAYER (KG/KG)
C
      REAL UDD_K(NPNTS)             ! INOUT
                                    ! IN  DOWNDRAUGHT U IN LAYER K
                                    !     (M/S)
                                    ! OUT U RESET FOR NEXT LAYER (M/S)
C
      REAL VDD_K(NPNTS)             ! INOUT
                                    ! IN  DOWNDRAUGHT V IN LAYER K
                                    !     (M/S)
                                    ! OUT V RESET FOR NEXT LAYER (M/S)
C
...
C
      REAL UE_K(NPNTS)              ! INOUT
                                    ! IN  U OF ENVIRONMENT IN LAYER K
                                    !     (M/S)
                                    ! OUT ENVIRONMENT U RESET FOR
                                    !     NEXT LAYER (M/S)
C
      REAL VE_K(NPNTS)              ! INOUT
                                    ! IN  V OF ENVIRONMENT IN LAYER K
                                    !     (M/S)
                                    ! OUT ENVIRONMENT V RESET FOR
                                    !     NEXT LAYER (M/S)
C
...
C
      REAL QDD_KM1(NPNTS)           ! DOWNDRAUGHT MIXING RATIO OF
                                    ! LAYER K-1 (KG/KG)
C
      REAL UDD_KM1(NPNTS)           ! DOWNDRAUGHT U IN LAYER K-1 (M/S)
C
      REAL VDD_KM1(NPNTS)           ! DOWNDRAUGHT V IN LAYER K-1 (M/S)
C
...
C
      DO I=1,NPNTS
        IF (K .EQ. KCT+1 .OR. BDD_START(I)) THEN
           BDDWT_K(I) = THDD_K(I)*EXK(I) .GT. TM
        ELSE
...
C
      IF(L_MOM)THEN
        DO I=1,NPNTS
C
          UDD_KM1(I) = (UDD_K(I)+(EKM14(I)*UE_K(I)) +
     *                 (1.0+EKM14(I))*EKM34(I)*UE_KM1(I))/
     *                 ((1.0+EKM14(I))*(1.0+EKM34(I)))
          VDD_KM1(I) = (VDD_K(I)+(EKM14(I)*VE_K(I)) +
     *                 (1.0+EKM14(I))*EKM34(I)*VE_KM1(I))/
     *                 ((1.0+EKM14(I))*(1.0+EKM34(I)))
C
C----------------------------------------------------------------------
C ADD AN IN-CLOUD PRESSURE GRADIENT TERM TO THE MOMENTUM INCREMENTS
C----------------------------------------------------------------------
C
          UDD_KM1(I) = UDD_KM1(I) - (0.7*(UE_K(I)-UE_KM1(I))/
     *                              (1.0+EKM34(I)))
          VDD_KM1(I) = VDD_KM1(I) - (0.7*(VE_K(I)-VE_KM1(I))/
     *                              (1.0+EKM34(I)))
        END DO
      END IF
C
C----------------------------------------------------------------------
C  DRY DESCENT FOR TRACERS
C----------------------------------------------------------------------
C
      IF(L_TRACER)THEN
C
...
C
      DO I=1,NPNTS
        BDDWT_KM1(I) = THDDS(I)*EXKM1(I) .GT. TM
      END DO
C
C-----------------------------------------------------------------------
...
C
     ...
     &              L_MOM,UDD_K,VDD_K,UDD_KM1,VDD_KM1,UE_K,VE_K,UE_KM1,
     &              VE_KM1,DUBYDT_K,DUBYDT_KM1,DVBYDT_K,DVBYDT_KM1,
...
C
C-----------------------------------------------------------------------
C SWITCH POTENTIAL TEMPERATURE, MIXING RATIO, MOMENTUM, MASS FLUX
C AND TRACER READY FOR CALCULATION AT NEXT MODEL LAYER
C-----------------------------------------------------------------------
C
      IF (K.GT.2) THEN
        DO I=1,NPNTS
