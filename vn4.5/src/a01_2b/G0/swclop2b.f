...
CLL version  Date
CLL   4.3  18/4/97    This deck, SWCLOP2B, added to restrict the
CLL  treatment of the singular case when the e-folding depths for direct
CLL  & diffuse light are nearly equal to when there is a danger of
CLL  failure on the T3E - for use in HadCM2, which for true backwards
CLL  compatibility with the original HadCM2 on the C90 would never use
CLL  this form, but which, thanks to the T3E's far lower range of
CLL  numbers, is liable to a failure that never occurred on the C90.
CLL                                                   William Ingram
CLL
CLL
...
C     !  ("Too small" rather than "zero", now because of the risk of
C     !  failure on dividing by it.)
C     !  Note that this value is intended for the T3E only !
C     !  1.7977E+308 is the largest 64-bit REAL the T3E can cope with.
      PARAMETER ( SWCTOL = TINY(1.0) )
