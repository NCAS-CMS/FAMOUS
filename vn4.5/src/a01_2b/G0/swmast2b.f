...
CLL  This is the "HADCM2 anthropogenic sulphate aerosol" version (2B)
CLL  which allows for this by modifying the surface albedo, & diagnoses
CLL  the impact at top of atmosphere.  Otherwise it matches 1B.
CLL  It is the top-level plug-compatible routine in component P234
CLL  (interaction of shortwave radiation with the atmosphere), which is
CLL  in task P23 (radiation).
CLL  It acts as the master routine for P234, assembling the net solar
...
CLL version  Date
CLL   4.2  19/11/96   First version, based on SWMAST1B
CLL   4.3   18/3/97   Alter to do non-sulphate stuff only if needed. WJI
CLL
CLL Programming standard :
CLL  It conforms to standard A of UMDP 4 (version 3, 07/9/90), and
CLL  includes no features deprecated in 8X.
CLL  The code is standard FORTRAN 77 except for ! comments and
CLL  dynamically allocated arrays.
CLL
...
C*L
     ...
     &     CCAAFO, CCAAFB,
     &     L2, NLEVS, NCLDS,
     &     SO4_FORCE, SO4_FORCE_ON, TSANA, NAADIM,
...
!   storage for MODSANA at - this is workspace used only if SO4_FORCE_ON
...
C     ! the whole grid-box for each band for direct & then diffuse light
     &     TSANA(L1,NBANDS,2),         ! TSA with no aerosol
...
C     !    differences of their successive elements.
      LOGICAL!, INTENT(IN)
     &     SO4_FORCE_ON,               !  Is SO4_FORCE wanted ?
...
C                                      !   surface where DTSA applies
     &     , SO4_FORCE(L1)             ! Sulphate aerosol forcing
...
C                                      ! allow for multiple reflections
     &     MODSANA(NAADIM,NBANDS,2),   ! TSANA similarly modified
...
C
C        ! And similarly for no-aerosol albedos:
C
         IF ( SO4_FORCE_ON )
     &    CALL SWMSAL (TSANA, REF(1,1,FSTCLD,2), LCA, CCREF(1,1,2), CCA,
     &     CCB, LSTCLR,
     &     L2,
     &     L1, NBANDS, NCLDS,                           MODSANA)
C
...
Cfpp$      Select(CONCUR)
   ...
   92    CONTINUE
       ...
       IF ( SO4_FORCE_ON ) THEN
         DO BAND=1, NBANDS
           DO J=1, L2
             SO4_FORCE(J) = SO4_FORCE(J) + GTRANS(J,BAND) *
     & (MODSA(J,BAND,DIRDIF)-MODSANA(J,BAND,DIRDIF)) * GREY(J,BAND,BEAM)
           ENDDO
         ENDDO
       ENDIF
