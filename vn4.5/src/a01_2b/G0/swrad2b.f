...
CLL                                     initialize LUT
CLL  The code is standard FORTRAN 77 except for ! comments & dynamically
CLL  allocated arrays.
CLL
CLL  This version, 2B, is as the standard routine (1A/2A/2B) except that
CLL   extra arguments are added, and one argument (land surface albedo)
CLL   made 2-dimensional, to allow for altered surface albedos (intended
CLL   to represent the effects of anthropogenic sulphate aerosol), and
CLL   if wanted to diagnose their impact on the toa flux.
CLL  Author: William Ingram
CLL
CLL  Model            Modification history:
CLL version  Date
CLL   4.2   19/11/96  First version, based on SWRAD1A       (W Ingram)
CLL   4.3    18/3/97  Alter to use memory more efficiently. (W Ingram)
...
C*L
     ...
     &     AICE, COSZIN, LIT, LAND, LIST, TAC, SCS, LUT, PTS, SANAIN,
     ...
     &     CWPAJ, CWPAJON, MICRO, SO4_FORCE, SO4_FORCE_ON,
     &     NLALBS, NAADIM,
     &     LCA3L, LCA3ON, LCLD3,
...
C*IF -DEF,CRAY
     ...
     &     NAADIM,                     ! Number of points to assign
!   storage for SANAGI at - this is workspace used only if SO4_FORCE_ON
     &     NLALBS,                   ! Number of fields of land surface
C     !  albedo - 2 if different for direct & diffuse sunlight, 1 if not
...
C*ELSE
C      INTEGER L1, NLIT, NDO              ! Array sizes must be constant
C      PARAMETER (NLIT=1, L1=1, NDO=1)    ! Make it an SCM
C*CALL NLEVSVAL
C*CALL NCLDSVAL
C*CALL NWETVAL
C*CALL NOZONVAL
...
C     !               and, as explained above, only their sum is used.
     ...
     &     SALIIN(L1,NLALBS),            ! (True) Surface Albedo for
     &     SAOSIN(L1,2),                 !  land & ice, for & open sea
     &     SANAIN(L1,2),                 !        & for no aerosol
...
C     ! restricted to 3 layers, calculated at all points on SW timesteps
     &     , SO4_FORCE(L1)            ! Diagnostic of radiative forcing
C     ! due to the change in surface albedo from SANA to SALI/SAOS.
C*
C     !  Constants:
...
C     !  3*NDO+ NLIT*(3*NCLDS+NWET+NOZONE+4*NBANDS+8) +2*(NLEVS+1)
     ...
     &     SANAGI(NAADIM,NBANDS,2),      ! water content to path.
...
Cfpp$  Select(CONCUR)
       DO 17 J=1, NLIT
        ...
        SAGI  (J,1,DIRDIF) = SALIIN(LIST(J),MIN(DIRDIF,NLALBS))
        IF ( .NOT. LAND(LIST(J)) )
     &   SAGI(J,1,DIRDIF) = SAGI(J,1,DIRDIF) * AICE(LIST(J)) +
     &                    SAOSGI(J,1,DIRDIF) * ( 1.-AICE(LIST(J)) )
   17  CONTINUE
       IF ( SO4_FORCE_ON ) THEN
         DO J=1, NLIT
          SANAGI(J,1,DIRDIF) = SANAIN(LIST(J),DIRDIF)
         ENDDO
         DO BAND=2, NBANDS
          DO J=1, NLIT
           SANAGI(J,BAND,DIRDIF) = SANAGI(J,1,DIRDIF)
          ENDDO
         ENDDO
       ENDIF
...
CL      ~~~~~~~~~
     ...
     &     NLIT, NLEVS, NCLDS,
     &     SO4_FORCE(1+NNIGHT), SO4_FORCE_ON, SANAGI, NAADIM,
...
C
      IF ( SO4_FORCE_ON ) THEN
        ...
        ENDDO
        DO J=1, NLIT
          SO4_FORCE(LIST(J)) = SO4_FORCE(J+NNIGHT)
        ENDDO
        DO J=1, NDO
          SO4_FORCE(J) = IITOA(J) * SO4_FORCE(J)
        ENDDO
      ENDIF
