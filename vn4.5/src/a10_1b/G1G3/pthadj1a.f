...
C LOOP OVER ALL POINTS AS VALUES OF DIVERGENCE AND VERTICAL VELOCITY
C AT THE POLES WERE CALCULATED IN VERT_VEL.
C AS ETADOT AT LEVEL 1 AND LEVEL P_LEVELS+1 ARE ZERO AND ALSO ARE
C NOT STORED, SLIGHTLY DIFFERENT CODE IS REQUIRED AT THESE LEVELS.
...
C LOOP OVER ALL POINTS AS POLAR VALUES OF DIVERGENCE AND VERTICAL
C VELOCITY WERE CALCULATED IN VERT_VEL.
C ADJUST ALL PRESSURE VALUES.

! loop over all points, including valid halos
          DO 300 I=FIRST_VALID_PT,LAST_P_VALID_PT
