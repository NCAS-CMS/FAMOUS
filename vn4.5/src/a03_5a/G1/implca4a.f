...
CLL
CLL  Extensive modifications to incorporate Penman-Monteith
CLL  formulations
CLL
CLL                                                J.Lean
CLL                                                RE 31/10/94
...
C*L  Arguments :-
     ...
     +,ICE_FRACT,LYING_SNOW,PSTAR,RADNET,RESFT,RHOKPM,RHOKPM_POT
     +,U0,V0,TIMESTEP,LAND_MASK,SU10,SV10
     +,EPOT,FQW,FTL,E_SEA,H_SEA,QW
     +,GAMMA_RHOKE,GAMMA_RHOKH_1,GAMMA_RHOKM_1,TL,U,V
...
C                                  !    (+ve downwards, W per sq m)
     ...
     +,RHOKPM_POT(P_FIELD)         ! IN Surface exchange coeff. for
...
C                                  !       leads fraction (kg/m2/s).
C                                  !       Zero over land.
     +,H_SEA(P_FIELD)              ! INOUT Surface sensible heat flux ov
C                                  !       sea times leads fraction (W/m
C                                  !       Zero over land.
     +,QW(P_FIELD,BL_LEVELS)       ! INOUT Total water content (kg per
C                                  !       kg air).  From P243.
     +,GAMMA_RHOKE(P_FIELD)        ! IN    Surface exchange coeff. for
C                                  !       FQW, =GAMMA(1)*RHOKE from
C                                  !       SF_EXCH.
C                                  !   OUT =RHOKE to satisfy STASH.
...
C                                  !     surface over land or sea-ice
C                                  !     fraction of gridbox (W/m2).
...
C                                  ! rapidly mixing layer.
     +,BPM(P_FIELD)                ! Used in calculating elements of
...
C  Local scalars :-
     ...
     +,GAMMA_RKE_DQ ! Used in calculation of implicit increments to
C                   ! surface latent and sesible heat fluxes.
...
C
     ...
     &                           - AT_ATQ(I,K)*DTL_DV(I,KP1)  ! P244.???
          ...
          IF ((NRML(I) .GE. 2) .AND. (K .EQ. 1)) THEN
            GAMMA_RKE_DQ = GAMMA_RHOKE(I) *
     &                     (DQW_DU(I,1) - ALPHA1(I)*DTL_DV(I,1))
            IF (LAND_MASK(I)) THEN
              FTL(I,1) = FTL(I,1) + RHOKPM(I)*(LAT_HEAT(I)*GAMMA_RKE_DQ
     &                                 - GAMMA(1)*ASHTF(I)*DTL_DV(I,1))
              FQW(I,1) = FQW(I,1) - RHOKPM(I)*(CP*GAMMA_RKE_DQ
     &                        + GAMMA(1)*RESFT(I)*ASHTF(I)*DQW_DU(I,1))
              EPOT(I) = EPOT(I) - RHOKPM_POT(I)*(CP*GAMMA_RKE_DQ
     &                        + GAMMA(1)*ASHTF(I)*DQW_DU(I,1))
            ELSEIF (ICE_FRACT(I) .GT. 0.0) THEN
              FQW_ICE(I) = FQW(I,1) - E_SEA(I) - RHOKPM(I) *
     &                (CP*GAMMA_RKE_DQ + GAMMA(1)*ASHTF(I)*DQW_DU(I,1))
              FTL_ICE(I) = FTL(I,1) - H_SEA(I)/CP + RHOKPM(I) *
     &                (LS*GAMMA_RKE_DQ - GAMMA(1)*ASHTF(I)*DTL_DV(I,1))
              E_SEA(I) = E_SEA(I) - (1.0 - ICE_FRACT(I)) *
     &                                     GAMMA_RHOKH_1(I)*DQW_DU(I,1)
              H_SEA(I) = H_SEA(I) - (1.0 - ICE_FRACT(I)) * CP *
     &                                     GAMMA_RHOKH_1(I)*DTL_DV(I,1)
            ...
            ELSE ! ordinary sea point
              ...
              FQW(I,1) = FQW(I,1) - GAMMA_RHOKH_1(I)*DQW_DU(I,1)
              EPOT(I) = EPOT(I) - GAMMA_RHOKH_1(I)*DQW_DU(I,1)
            ENDIF
          ENDIF
  231   CONTINUE
   23 CONTINUE
...
C
      DO 33 I=P1,P1+P_POINTS-1
        IF (LAND_MASK(I)) THEN
          BPM(I) = GAMMA(1)*ASHTF(I)*RHOKPM(I)
        ELSE
          BPM(I) = (1. - ICE_FRACT(I))*GAMMA_RHOKH_1(I) +
     &                      GAMMA(1)*ASHTF(I)*RHOKPM(I)
        ENDIF
        IF ( NRML(I) .GE. 2 ) THEN
C
C-----------------------------------------------------------------------
...
C
          AQ_RML(I) = - DTRDZ_RML(I)*GAMMA_RHOKE(I)*CP*RHOKPM(I)
          CQ_RML = -DTRDZ_RML(I) * GAMMA_RHOKH_RDZ(I,NRMLP1)
          RBQ_RML = 1./(1. - AQ_RML(I) - CQ_RML*(1. + AQ_AM(I,NRMLP1))
     &                                + DTRDZ_RML(I)*RESFT(I)*BPM(I) )
...
C
          DQW_DU(I,1) = - DTRDZ(I,1)*(FQW(I,2) - FQW(I,1))
          AQ_AM(I,1) = - DTRDZ(I,1)*GAMMA_RHOKE(I)*CP*RHOKPM(I)
          CQ = - DTRDZ(I,1)*GAMMA_RHOKH_RDZ(I,2)
          RBQ = 1./( 1. - AQ_AM(I,1) - CQ*(1. + AQ_AM(I,2))
     &                               + DTRDZ(I,1)*RESFT(I)*BPM(I) )
          DQW_DU(I,1) = RBQ*(DQW_DU(I,1) - CQ*DQW_DU(I,2))
...
CL     transport problem (i.e. for ice/liquid
...
C
          ...
          CT_RML = - DTRDZ_RML(I)*GAMMA_RHOKE(I)*LAT_HEAT(I)*RHOKPM(I)
          RBT_RML = 1./(1. - AT_RML(I) - ALPHA1(I)*CT_RML*(1.+AQ_RML(I))
     &                                 + DTRDZ_RML(I)*BPM(I) )
...
C
          DTL_DV(I,1) = - DTRDZ(I,1)*(FTL(I,2) - FTL(I,1))
          CT = - DTRDZ(I,1)*GAMMA_RHOKE(I)*LAT_HEAT(I)*RHOKPM(I)
          AT_ATQ(I,1) = - DTRDZ(I,1)*GAMMA_RHOKH_RDZ(I,2)
          RBT = 1./( 1. - AT_ATQ(I,1) - ALPHA1(I)*CT*(1. + AQ_AM(I,1))
     &                                + DTRDZ(I,1)*BPM(I) )
C
          DTL_DV(I,1) = RBT*(DTL_DV(I,1) - CT*DQW_DU(I,1))
...
C
C-----------------------------------------------------------------------
CL 5.  "Downward sweep" through whole matrix.  TL, QW and
...
C-----------------------------------------------------------------------
          ...
          DQW_DU(I,1) = DQW_DU(I,1) - ALPHA1(I)*AQ_AM(I,1)*DTL_DV(I,1)
...
C
      DO 61 I=P1,P1+P_POINTS-1
        ...
        ELSE
           ...
           DQW = DQW_DU(I,1)
        ENDIF
        GAMMA_RKE_DQ = GAMMA_RHOKE(I)*(DQW - ALPHA1(I)*DTL)
        IF (LAND_MASK(I)) THEN
          FTL_ICE(I) = 0.0
          FQW_ICE(I) = 0.0
          FTL(I,1) = FTL(I,1) + RHOKPM(I)*( LAT_HEAT(I)*GAMMA_RKE_DQ
     &                                      - GAMMA(1)*ASHTF(I)*DTL )
          FQW(I,1) = FQW(I,1) - RHOKPM(I)*( CP*GAMMA_RKE_DQ
     &                             + GAMMA(1)*RESFT(I)*ASHTF(I)*DQW )
          EPOT(I) = EPOT(I) - RHOKPM_POT(I)*( CP*GAMMA_RKE_DQ
     &                             + GAMMA(1)*ASHTF(I)*DQW )
          SURF_HT_FLUX(I) = RADNET(I) - LAT_HEAT(I)*FQW(I,1)
     &                                - CP*FTL(I,1)
        ELSEIF (ICE_FRACT(I).GT.0.0) THEN
          FQW_ICE(I) = FQW(I,1) - E_SEA(I) - RHOKPM(I) *
     &                      (CP*GAMMA_RKE_DQ + GAMMA(1)*ASHTF(I)*DQW)
          FTL_ICE(I) =  FTL(I,1) - H_SEA(I)/CP + RHOKPM(I) *
     &                  (LS*GAMMA_RKE_DQ - GAMMA(1)*ASHTF(I)*DTL)
          E_SEA(I) = E_SEA(I) - (1.0 - ICE_FRACT(I)) *
     &                                        GAMMA_RHOKE(I)*DQW
          H_SEA(I) = H_SEA(I) - (1.0 - ICE_FRACT(I)) * CP *
     &                                        GAMMA_RHOKH_1(I)*DTL
          ...
          FQW(I,1) = FQW_ICE(I) + E_SEA(I)
          EPOT(I) = FQW_ICE(I) + E_SEA(I)
          SURF_HT_FLUX(I) = RADNET(I) - LS*FQW_ICE(I) - CP*FTL_ICE(I)
        ELSE  ! ordinary sea point
          FTL(I,1) = FTL(I,1) - GAMMA_RHOKH_1(I)*DTL
          FQW(I,1) = FQW(I,1) - GAMMA_RHOKH_1(I)*DQW
          EPOT(I) = EPOT(I) - GAMMA_RHOKH_1(I)*DQW
          E_SEA(I) = FQW(I,1)
          H_SEA(I) = FTL(I,1)*CP
          FTL_ICE(I) = 0.0
          FQW_ICE(I) = 0.0
        ...
        GAMMA_RHOKE(I) = GAMMA_RHOKE(I) / GAMMA(1)
   61 CONTINUE
C
C-----------------------------------------------------------------------
