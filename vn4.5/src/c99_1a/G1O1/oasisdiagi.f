...
C       1/ Add the wrap around 2 columns at the end of the global
C          array
C       2/ Scatter the array from 1 PE to the rest of them
C       3/ Copy those small arrays onto D1  while ignoring the
C       undefined values(rmdi) of the current field.
C       Copy the field over to the D1 array

C       1.Add two columns at the end identical to the colums one and
C       two.
        if (mype .eq. gather_pe) then
        if ((cyclic_ocean) .and.
     &    (FieldLocator(grd,CouplingField) .eq. 'T'))  then
          call oasis_cyclicbc(Zwork,
     &      Zwork,g_imt,g_jmt)
        elseif ((cyclic_ocean) .and.
     &      (FieldLocator(grd,CouplingField) .eq. 'U'))  then
          call oasis_cyclicbc(Zwork,
     &      Zwork,g_imt,g_jmt)
        endif
        endif ! 1 pe.
...
C       2. Scatter the array from 1 PE to the rest of them
     ...
     &      swap_levels)
          ...
          cmessage='OASIS DIAGNOSTICS IMPORT : ERROR in input list'
          icode=102
          go to 999
        endif

C       3.Copy the local arrays into D1.
