...
C declatarions for the atmosphere model :
      real
     &  xta(0:g_row_length+1)   ! atmosphere tp longitude coordinates
     &  , xua(0:g_row_length+1) ! atmosphere uv longitude coordinates
     &  , yta(0:g_p_rows+1)     ! atmosphere tp latitude coordinates
     &  , yua(0:g_u_rows+1)     ! atmosphere uv latitude coordinates
!
...
!
      logical
     &  Zmaskua(g_row_length,g_u_rows) ! atmosphere mask on u grid
     &  , Zmaskta(g_row_length,g_p_rows) ! atmosphere mask on t grid
      integer
     &  imaskua(g_row_length,g_u_rows) ! atmosphere mask on u grid
     &  , imaskta(g_row_length,g_p_rows) ! atmosphere mask on t grid
!
!     surface arrays
!
      real
     &  surfua(g_row_length,g_u_rows) ! atmosphere surface on u grid
     &  , surfta(g_row_length,g_p_rows) ! atmosphere surface on t grid
...
!
      real
     &  Zxua(g_row_length,g_u_rows) ! atm longitude on u grid
     &  , Zxta(g_row_length,g_p_rows) ! atm longitude on t grid
     &  , Zyua(g_row_length,g_u_rows) ! atm latitude on u grid
     &  , Zyta(g_row_length,g_p_rows) ! atm  latitude on t grid
...
cl--------------------------------------------------------------------
C       2. calculate size of each of the coupling fields.
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            FieldSize(ii) = g_row_length * g_u_rows
          else                  ! TP grid.
            FieldSize(ii) = g_row_length * g_p_rows
          endif
        enddo
...
c
! define the grids of the atmosphere UM submodel.
        do ii = 0, g_row_length + 1
          xta(ii)=a_realhd(4)+(ii-1)*a_realhd(1)
          xua(ii)=a_realhd(4)+(ii-0.5)*a_realhd(1)
        enddo
        do j = 0, g_p_rows + 1
          yta(j)=a_realhd(3)-(j-1)*a_realhd(2)
        enddo
        do j = 0, g_u_rows + 1
...
! layout.
        do ii = 1, g_row_length
          do j = 1, g_u_rows
            Zxua(ii,j) = xua(ii) ! longitude on u grid
            Zyua(ii,j) = yua(j)  ! latitude on u grid
          enddo
        enddo
        do ii = 1, g_row_length
          do j = 1, g_p_rows
            Zxta(ii,j) = xta(ii) ! longitude on t grid
            Zyta(ii,j) = yta(j)  ! latitude on t grid
          enddo
        enddo
! define the mask grids of the atmosphere UM submodel.
!       T grid :
        do j = 1, g_p_rows
          do ii = 1, g_row_length
            if (atmos_landmask(ii+(j-1)*g_row_length)) then
              Zmaskta(ii,j)= .true.
              imaskta(ii,j)= 1
            else
              Zmaskta(ii,j)= .false.
          ...
          enddo
        enddo
C       set up logical land/sea mask on atmosphere UV grid
        do j = 1, g_u_rows
          do ii = 1, g_row_length-1
            Zmaskua(ii,j) =
     &             Zmaskta(ii,j)   .or.  Zmaskta(ii+1,j)
     &        .or. Zmaskta(ii,j+1) .or.  Zmaskta(ii+1,j+1)
          enddo
          Zmaskua(g_row_length,j) =
     &           Zmaskta(g_row_length,j)   .or.  Zmaskta(1,j)
     &      .or. Zmaskta(g_row_length,j+1) .or.  Zmaskta(1,j+1)
        enddo
!       Generate an integer mask from the logical mask above ;
!       1 = land, 0 = ocean. (as requiered by OASIS).
        do j = 1, g_u_rows
          do ii = 1, g_row_length
            if (Zmaskua(ii,j)) then
              imaskua(ii, j) = 1
            else
              imaskua(ii, j) = 0
            endif
...
!       u grid
        do ii = 1, g_row_length
          do j = 1, g_u_rows
            Zlatitude1 = (yua(j-1)+yua(j))/2.
            Zlatitude2 = (yua(j+1)+yua(j))/2.
            ...
            surfua(ii,j) = Rearth * Rearth ! on u grid
     &        * abs(
     &        (   sin(pi_over_180*Zlatitude2)
     &          - sin(pi_over_180*Zlatitude1) )
     &        * pi_over_180
     &        * ((xua(ii+1)+xua(ii))/2. - (xua(ii-1)+xua(ii))/2.)
     &        )
          enddo
        enddo
...
!       p grid
        do ii = 1, g_row_length
          do j = 1, g_p_rows
            Zlatitude1 = (yta(j-1)+yta(j))/2.
            Zlatitude2 = (yta(j+1)+yta(j))/2.
            if (Zlatitude1 .gt.  90.0) Zlatitude1 =  90.0
            if (Zlatitude2 .gt.  90.0) Zlatitude2 =  90.0
            if (Zlatitude1 .lt. -90.0) Zlatitude1 = -90.0
            if (Zlatitude2 .lt. -90.0) Zlatitude2 = -90.0
            surfta(ii,j) = Rearth * Rearth ! on u grid
     &        * abs(
     &        (   sin(pi_over_180*Zlatitude2)
     &          - sin(pi_over_180*Zlatitude1) )
     &        * pi_over_180
     &        * ((xta(ii+1)+xta(ii))/2. - (xta(ii-1)+xta(ii))/2.)
     &        )
          enddo
        enddo

!---------------------------------------------------------------------
! Write to file the grids of the coupling
! fields.
! Note : the layout of the atmos grid is reverse of the one of oasis;
! therefore the descending j indexes for masks, surf, and grids.
        nulgr = 3
     ...
     &    FORM = 'UNFORMATTED', IOSTAT = iost)
        if (iost .ne. 0) then
          icode = 1
          cmessage = 'io error in INIT_OASIS from UM atmos model.'
        endif
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            ...
            !   longitude array:
            write(nulgr) ((Zxua(i,j),i=1,g_row_length),
     &        j=g_u_rows,1,-1)
            !   locator for the latitude array:
            write(nulgr) FieldLocator(lat,ii)
            !   latitude array:
            write(nulgr) ((Zyua(i,j),i=1,g_row_length),
     &        j=g_u_rows,1,-1)
          else                  ! T.P grids
            !   locator for the longitude array:
            write(nulgr) FieldLocator(lon,ii)
            !   longitude array:
            write(nulgr) ((Zxta(i,j),i=1,g_row_length),
     &        j=g_p_rows,1,-1)
            !   locator for the latitude array:
            write(nulgr) FieldLocator(lat,ii)
            !   latitude array:
            write(nulgr) ((Zyta(i,j),i=1,g_row_length),
     &        j=g_p_rows,1,-1)
            !    mask locator :
          endif
        enddo
     ...
     &    FORM = 'UNFORMATTED', IOSTAT = iost)
        ...
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            ...
            write(nulma)
     &        ((imaskua(i,j),i=1,g_row_length),j=g_u_rows,1,-1)
            !   surface locator :
          else                  ! T.P grids
            !    mask locator :
            write(nulma) FieldLocator(msk,ii)
            write(nulma) ((imaskta(i,j),i=1,g_row_length),
     &        j=g_p_rows,1,-1)
          endif
        enddo
     ...
     &    FORM = 'UNFORMATTED',IOSTAT = iost)
        ...
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            ...
            write(nulsu)((surfua(i,j),i=1,g_row_length),
     &        j=g_u_rows,1,-1)
          else                  ! T.P grids
            !   surface locator :
            write(nulsu) FieldLocator(srf,ii)
            !   surface field  :
            write(nulsu) ((surfta(i,j),i=1,g_row_length),
     &        j=g_p_rows,1,-1)
          endif
        enddo
