...
C******************************COPYRIGHT******************************
C
CLL   Routine : OASIS_DIAGNOSTICS ------------------------------------
CLL
CLL   Called : by OASIS_STEP.
CLL
CLL   Purpose : Some fields need to be computed as diagnostics
CLL   before they are handed over to the coupling
CLL   However, most of the coupling fields are already generated in
CLL   either OCN_STEP, or ATM_STEP.
CLL
CLL   Algorithm :
CLL   - extract diagnostics from D1, compute the new ones, and
CLL     store them Zwork.
CLL
...
C Constants for physics  routines. Array sizes are resolution dependent.
      ...
      integer internal_model    ! No of the corrent internal model.
...
! Declaration of the pointers on the ocean D1.
      integer
     &  D1_Zptr_snow_depth      ! Pointer towards the coupling field
     &  ,D1_Zptr_aice           ! in D1.
     &  ,D1_Zptr_hice
     &  ,D1_Zptr_tstar
! These need to be stored in a static area of memory (even if they
! are initialized as dummy) :
      data
     &  D1_Zptr_snow_depth     /1/ ! Pointer towards the coupling
     &  ,D1_Zptr_aice          /1/ ! field in D1.
     &  ,D1_Zptr_hice          /1/
     &  ,D1_Zptr_tstar         /1/

      real
     &  rcmpm                   ! reciprocal of cm per m
     &  ,conratio               ! ratio of conductivities (ice/snow)
     &  ,rhosnow                ! density of snow in kg/m**3
     &  ,aicemin                ! minimum ice concentration if ice
                                ! present
      parameter (conratio = 6.5656)
      parameter (rhosnow  = 300.0 )
      parameter (rcmpm    = 0.01  )
      parameter (aicemin  = 0.001 )

      integer    ptr_field

      integer    number_of_landpts_out
      integer    info

C     Atmos only variables :
      real
     &  Zworktemp1(g_p_field)   ! Temp work array for field gathering.
     &  ,Zworktemp2(g_p_field)  ! Temp work array for field gathering.
     &  ,Zworktemp3(g_p_field)  ! Temp work array for field gathering.
     &  ,Zworktemp4(g_p_field)  ! Temp work array for field gathering.
     &  ,Zworktemp5(g_p_field)  ! Temp work array for field gathering.
     &  ,Zworktemp6(g_p_field)  ! Temp work array for field gathering.
      INTEGER
     &  ocentpts(g_p_field)     ! Ocean entry points index
                                ! Intermediate field for work space
     &  ,ocentpts_local(p_field) ! local land-compressed
     &  ,ocentpts_global(g_p_field) !
c     Intermediate pre-calculated trig field for coupling:
     &  ,a_cos_p_latitude(g_p_field) ! Cos(lat) on atmos p grid


C     Ocean only variables :

      icode = 0                 ! error code set to nil at begining
                                ! of the procedure.

C---------------------------------------------------------------------
      write(nulou,*) 'entering OASIS_DIAGNOSTICS ...'
      write(nulou,*) "CouplingField number", CouplingField
C---------------------------------------------------------------------


C     I/ if the internal model is the UM_atmosphere, generate the
C     required diagnostics.
...
C
C*-- Following the field number, gather it :
C
        if ((FieldLocator(direction,CouplingField) .eq. 'E')
     &    .and. (FieldLocator(istash,CouplingField) .eq. '03228'))
     &    then
C
C*--  HEAT FLUXes
C
C         Compute the heat-flux field.
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_solar), Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
            icode=1
            go to 999
          endif
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_blue), Zworktemp2,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_longwave), Zworktemp3,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_sensible), Zworktemp4,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_evap), Zworktemp5,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         compute the field :
C         (solar - blue + longwave - (sensible + LC * evap))
C            1       2        3          4             5
          if (mype .eq. gather_pe) then
          do i = 1, FieldSize(CouplingField)
            if (   (Zworktemp1(i) .eq. rmdi)
     &        .or. (Zworktemp2(i) .eq. rmdi)
     &        .or. (Zworktemp3(i) .eq. rmdi)
     &        .or. (Zworktemp4(i) .eq. rmdi)
     &        .or. (Zworktemp5(i) .eq. rmdi) ) then
              Zwork(i) = rmdi
            else
              Zwork(i) =
     &          Zworktemp1(i)
     &          - Zworktemp2(i) + Zworktemp3(i)
     &          - (Zworktemp4(i) + LC * Zworktemp5(i) )
            endif
          enddo
          endif

        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '04203'))
     &      then
C
C*--    PRECIPITATION MINUS EVAPORATION.
C
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_snowls), Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_snowconv), Zworktemp2,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_ice), Zworktemp3,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_rainls), Zworktemp4,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_rainconv), Zworktemp5,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_evap), Zworktemp6,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Compute the precipitation-minus-evaporation field:
C         [(snowls+snowconv)*(1-aice)+rainls+rainconv-evap]
C              1        2        3      4        5     6
          if (mype .eq. gather_pe) then
          do i = 1, FieldSize(CouplingField)
            if (   (Zworktemp1(i) .eq. rmdi)
     ...
     &        .or. (Zworktemp3(i) .eq. rmdi)
     &        .or. (Zworktemp4(i) .eq. rmdi)
     &        .or. (Zworktemp5(i) .eq. rmdi)
     &        .or. (Zworktemp6(i) .eq. rmdi) ) then
              Zwork(i) = rmdi
            else
              Zwork(i) =
     &          (Zworktemp1(i)+Zworktemp2(i))
!*IF DEF,SEAICE
     &          * (1.0 - Zworktemp3(i))
!*ENDIF
     &          + Zworktemp4(i) + Zworktemp5(i)
     &          - Zworktemp6(i)
            endif
          enddo
          endif
        ...
        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '08205'))
     &      then
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_slowrunoff), Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_fastrunoff), Zworktemp2,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
c         Compute the runoff field:
c         (slow_runoff + fastrunoff)
c                1           2
          if (mype .eq. gather_pe) then
          do i = 1, FieldSize(CouplingField)
            if (   (Zworktemp1(i) .eq. rmdi)
     &        .or. (Zworktemp2(i) .eq. rmdi) ) then
              Zwork(i) = rmdi
            else
              Zwork(i) =
     &          (Zworktemp1(i)+Zworktemp2(i))
     &          / 86400         ! daily accumulated
                                !    --> instantaneous (s-1)
            endif
          enddo
          endif

! Expand river index compressed on land points for each PE domain
      call from_land_points(ocentpts_local,id1(ptr_ocentpts),
     &  atmos_landmask_local,lasize(1)*lasize(2),
     &  number_of_landpts_out)

      call gather_field(ocentpts_local,ocentpts_global,
     &  lasize(1),lasize(2),glsize(1),glsize(2),
     &  gather_pe,gc_all_proc_group,info)
      if(info.ne.0) then      ! check return code
         cmessage='oasis diag : error in gather of ocentpts'
         icode=20
         go to 999
      endif

!     Pre-calculated trig field cos(lat) on atmos p grid
      call gather_field(cos_p_latitude,a_cos_p_latitude,
     &  lasize(1),lasize(2),glsize(1),glsize(2),
     &  gather_pe,gc_all_proc_group,info)
      if(info.ne.0) then      ! check return code
         cmessage='oasis diag : error in gather of a_cos_p_latitude'
         icode=21
         go to 999
      endif

c     Compress river index on global domain onto land points
      if(mype.eq.gather_pe) then ! global data on single pe only
        call to_land_points(ocentpts_global,ocentpts,
     &  atmos_landmask,glsize(1)*glsize(2),
     &  number_of_landpts_out)

C     call a dedicated routine to compute the river outflow:
      call ComputeRiverOutflow(
     &  Zwork, g_row_length, g_p_rows,
     &  A_COS_P_LATITUDE,
     &  atmos_landmask,
     &  ocentpts)
      endif ! 1 pe.
      ...
      elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '04204'))
     &      then
C
C*--    SNOWFALL
C
...
C            1        2
     ...
     &            (Zworktemp1(i)+Zworktemp2(i))
        ...
        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '03231'))
     &      then
C
C*--    SUBLIMATION
C
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_sublimation_accumul), Zwork,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
...
C         Compute the sublimation field:
C         (day accumulation to instantaneous)
          if (mype .eq. gather_pe) then
            do i = 1, FieldSize(CouplingField)
              if (Zwork(i) .ne. rmdi) then
                Zwork(i) =
     &            (Zwork(i) / 86400 )
              endif
            enddo
...
C
C*--  Fields which do not need any particular handling :
C
        elseif ( (FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(grd,CouplingField) .eq. 'T') ) then
C         Pointer towards the coupling field in D1
          ptr_field = D1_Zptr(CouplingField)
C         Gather the field from all the PE onto the gather_pe
C         processor:
          call gather_field(D1(ptr_field), Zwork,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis diagnostic : ERROR in field gathering '
        ...
        elseif ( (FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(grd,CouplingField) .eq. 'U') ) then
C         Pointer towards the coupling field in D1
          ptr_field = D1_Zptr(CouplingField)
C         Gather the field from all the PE onto the gather_pe
C         processor:
...
C---------------------------------------------------------------------
C       CouplingField/ if the internal model is the UM_ocean,
C       generate the required diagnostics.
      else if (internal_model .eq. ocean_im) then
C---------------------------------------------------------------------
C       IIII/ if the internal model is any of the above, generate an
C       error message
      else                      !! internal_model
        icode = 1
...
C     Error trap.
 999  continue
      ...
      write(nulou,*) "exiting OASIS_DIAGNOSTICS"

      return
      end

C     This routine is to deal with the wrapped around grid of the
C     ocean; the source grid is copied into the target grid which
C     happen to be smaller ; in the process, the last 2 columns of
C     the source grid are left over.
      subroutine oasis_oce_export(source,is,js,target,it,jt)
      integer is,js
      real source(is,js)
      integer it,jt
      real target (it,jt)
...
C     RIVER OUTFLOW
c     sum the runoff for each ocean entry point (k,l) :-
c     for every land point (i,j) get the coordinates of the ocean
c     entry point (k,l) from array ocentpts and add the runoff for
c     point (i,j) to point (k,l) - multiply by the ratio of areas of
c     source to target gridbox in forming sum; this gives a mass flux
c     per unit area.
      subroutine ComputeRiverOutflow(
     &  runoffinout, icols, jrows,
     &  a_COS_P_LATITUDE,
     &  amasktp,
     &  ocentpts)
      real runoffinout(icols,jrows)
      integer icols, jrows
      logical amasktp(icols, jrows)
      integer ocentpts(icols*jrows)
      real A_COS_P_LATITUDE(ICOLS,JROWS) ! IN COSINE OF LATITUDE AT P
                                ! POINTS

C     local variables :
      real  worka(icols, jrows)
      integer landpt
      integer i,j,k,l
      ...
      do j=1,jrows
        do i=1,icols
          if (amasktp(i,j)) then
            landpt=landpt+1
            k=ocentpts(landpt)/100000
            l=mod(ocentpts(landpt),100000)
            worka(k,l)=worka(k,l)+runoffinout(i,j)*
     &        a_cos_p_latitude(i,j)/a_cos_p_latitude(k,l)
          endif
        enddo
      enddo
C     copy back the quantities obtained in worka to runoffinout.
      do j=1,jrows
        do i=1,icols
          runoffinout(i,j) = worka(i,j)
        enddo
      enddo
      return
      end
