...
CL 1. Pointers needed by SWAP_A2O and SWAP_O2A -------------------------
     ...
     *,   JA_AICE            ! Fractional ice conc. on atmos grid
C
      COMMON /AOPTR/
     &    JO_SST,JO_UCURR
     &,   JA_AICE
...
CL 2. Pointers needed by SWAP_A2O -------------------------------------
     ...
     &,   JO_co2flux
     &,   JO_SNOWFALL,       ! Snowfall rate on ocean grid
     &    JA_SUBLIM,         ! Sublimation on atmos grid
     &    JO_SUBLIM,         ! Sublimation on ocean grid
     &    JA_BOTMELT,        ! Diffusive heat thro ice on atmos grid
     &    JO_BOTMELT,        ! Diffusive heat thro ice on ocean grid
     &    JA_TOPMELT,        ! Seaice top melting flux on atmos grid
     &    JO_TOPMELT         ! Seaice top melting flux on ocean grid
C
      COMMON /A2OPTR/
...
CL 3. Pointers needed by SWAP_O2A -------------------------------------
     ...
     *    JO_VCURR,          ! Surface merid current on ocean grid
     ...
     &,   JO_AICE,           ! Fractional ice conc. on ocean grid
     &    JO_ICEDEPTH,       ! Ice depth on ocean grid
     &    JA_ICEDEPTH,       ! Ice depth on atmos grid
     &    JO_SNOWDEPTH,      ! Snow depth on ocean grid
     &    JA_SNOWDEPTH       ! Snow depth on atmos grid
C
      COMMON /O2APTR/
     &    JO_TSTAR,JA_TSTAR,JA_UCURR,JO_VCURR,JA_VCURR
     &,   JO_AICE,JO_ICEDEPTH,JA_ICEDEPTH,
     &    JO_SNOWDEPTH,JA_SNOWDEPTH
! History:
! Version  Date  Comment
...
C         Test the flags associated with each of the fields :
          if (sf(item, section)) then
            D1_Zptr(i) = si(item, section, im_index)
          else
            D1_Zptr(i) = si(item, section, im_index)
          endif
        enddo
