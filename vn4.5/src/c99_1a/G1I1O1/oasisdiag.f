...
C     Ocean only variables :
      real
     &  Zwork_Diagnos(imt*jmt)  ! Work array of local area of the
                                ! decomposition.
      real
     &  Zworktemp1(g_imtjmt) ! Temp work array for field gathering.
     &  ,Zworktemp2(g_imtjmt) ! Temp work array for field gathering.
     &  ,Zworktemp3(g_imtjmt) ! Temp work array for field gathering.
     &  ,Zworktemp4(g_imtjmt) ! Temp work array for field gathering.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C       Some fields need to be unpacked before they are used.
        if ((FieldLocator(direction,CouplingField) .eq. 'E')
     &    .and. (FieldLocator(istash,CouplingField) .eq. '00147'))
     &    then
C
C*--      Ice Depth :
c
c         Begin by converting from the grid box mean actual ice depth
c         to the equivalent ice depth averaged over thick ice.
c         this process uses the ice concentration and snow depth
c         fields.
c         Neglect sea-ice in boxes with less than the minimum
c         ice-fraction

C         Pointer towards the coupling field in D1
          D1_Zptr_hice = D1_Zptr(CouplingField)
C         Gather to 1 PE.
          CALL GATHER_FIELD(D1(D1_Zptr_tstar),Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          IF(info.NE.0) THEN    ! Check return code
            CMESSAGE='oasis : error in gather_field'
            ICODE=41
            GO TO 999
          ENDIF
C         Gather to 1 PE.
          call gather_field(D1(D1_Zptr_aice),Zworktemp2,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          IF(info.NE.0) THEN    ! Check return code
            CMESSAGE='oasis : error in gather_field'
          ...
          ENDIF
C         Gather to 1 PE.
          call gather_field(D1(D1_Zptr_hice),Zworktemp3,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          IF(info.NE.0) THEN    ! Check return code
            CMESSAGE='oasis : error in gather_field'
          ...
          ENDIF
C         Gather to 1 PE.
          call gather_field(D1(D1_Zptr_snow_depth),Zworktemp4,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          IF(info.NE.0) THEN    ! Check return code
            CMESSAGE='oasis : error in gather_field'
...
C         Compute the field on 1 pe:
C         (hice / aice + conratio * snow_depth
C           3       2                   4
          if (mype .eq. gather_pe) then
          do i = 1, g_imt*g_jmt
            if (Zworktemp1(i).ne.rmdi) then
              if (Zworktemp2(i).lt.aicemin) then
                Zwork(i)=0.
              else
                Zwork(i) =
     &            Zworktemp3(i)
     &            / Zworktemp2(i)
     &            + conratio * Zworktemp4(i)
              endif
            else
              Zwork(i) = rmdi
            endif
          enddo
C         Re-arrange the field on the unwrapped grid:
          call oasis_oce_export(Zwork,
     &      g_imt,g_jmt, Zwork,g_imt-2,g_jmt)
          endif ! 1 pe.

C
C*--      Snow depth (has to be multiplied by the snow density
C*--      rhosnow):
C
        elseif ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '00141'))
     &      then
C         Pointer towards the coupling field in D1
          D1_Zptr_snow_depth = D1_Zptr(CouplingField)
C         In Zwork :
C         Gather to 1 PE.
          write(nulou,*) "calling gather_field"
          CALL GATHER_FIELD(D1(D1_Zptr_snow_depth),Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          IF(info.NE.0) THEN    ! Check return code
            CMESSAGE='oasis : error in gather_field'
...
C         Compute the field :
C         snow_detph * rhosnow
          if (mype .eq. gather_pe) then
          write(nulou,*) "rescale zworktemp1"
          write(nulou,*) "rhosnow", rhosnow
          write(nulou,*) "rmdi", rmdi
          write(nulou,*) "g_imt, g_jmt", g_imt, g_jmt
          do i = 1, g_imt*g_jmt
ccccccc            write(nulou,*)"i,Zworktemp1(i)", i,Zworktemp1(i)
            if (Zworktemp1(i) .eq. rmdi) then
              Zwork(i) = rmdi
            else
              Zwork(i) =
     &          Zworktemp1(i) * rhosnow
            endif
          enddo
C         Re-arrange the field on the unwrapped grid:
          write(nulou,*) "calling oasis_oce_export"
          call oasis_oce_export(Zwork
     &      ,g_imt,g_jmt,
     &      Zwork,g_imt-2,g_jmt)
          endif
          write(nulou,*) "called (oasis_oce_export)"

C
C*--      Sea Ice fraction :
C
        elseif ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '00146'))
     &      then
C         Pointer towards the coupling field in D1
          D1_Zptr_aice = D1_Zptr(CouplingField)
C         In Zwork :
C         Gather to 1 PE.
          call gather_field(D1(D1_Zptr_aice),Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis : error in gather_field'
...
C
C         IMPORTANT : the sst is simply exported as is in the ocean
C         model (degrees C) and the real computations performed
C         at the import step in the atmosphere model. This is the
C         easyest way to cope with the blending between different
C         atmos timesteps I have found.
        elseif ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField) .eq. '00101'))
     &      then
C         unpack the array and let it into Zwork_Diagnos :
C         Pointer towards the coupling field in D1
          D1_Zptr_tstar = D1_Zptr(CouplingField)
!         The use of Zwork_diagnos as below implies that the
!         field will be cut in its 2 last columns : they overlap
!         the columns 1&2 and are not needed by the external model.
          if (l_ocomp) then
            CALL UNPACK(1,JMT, 1,1,JMT,KM, IMT,JMT,1,
     &        O_CFI1,O_CFI2,joc_no_segs,O_CFI3,joc_no_seapts,
     &        D1(joc_tracer(1,2)),Zwork_Diagnos,
     &        RMDI,CYCLIC_OCEAN)
          else
            do i = 1, imt*jmt
              Zwork_Diagnos(i) = D1(joc_tracer(1,2)-i+1)
            enddo
          endif
C         Gather to 1 PE.
          call gather_field(Zwork_Diagnos,Zworktemp1,
     &      lasize(1),lasize(2),glsize(1),glsize(2),
     &      gather_pe,GC_ALL_PROC_GROUP,info)
          if(info.ne.0) then    ! Check return code
            cmessage='oasis : error in gather_field'
...
C         Re-arrange the field on the unwrapped grid:
          if (mype .eq. gather_pe) then
          call oasis_oce_export(Zworktemp1
     &      ,g_imt,g_jmt,
     &      Zwork,g_imt-2,g_jmt)
          endif
C
...
C
        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField).eq.'00121'))
     &      then
...
!         the columns 1&2 and are not needed by the external model.
          if (l_ocomp) then
            CALL UNPACK(1,JMT, 1,1,JMT,KM, IMT,JMT,1,
     &        O_CFI1,O_CFI2,joc_no_segs,O_CFI3,joc_no_seapts,
     &        D1(joc_u(2)),Zwork_Diagnos,
     &        RMDI,CYCLIC_OCEAN)
          else
            do i = 1, imt*jmt
              Zwork_Diagnos(i) = D1(joc_u(2)-i+1)
            enddo
          endif
C         Gather to 1 PE.
          call gather_field(Zwork_Diagnos,Zworktemp1,
...
C         Rescale surface currents from ocean (cm/s) to atmosphere
C         (m/s) units
          if (mype .eq. gather_pe) then
          do i = 1, g_imt*(g_jmt-1)
            if (Zworktemp1(i).NE.RMDI) then
              Zwork(i) =
     &          Zworktemp1(i) * RCMPM
            else
...
C         Re-arrange the field on the unwrapped grid:
          call oasis_oce_export(Zwork
     &      ,g_imt,g_jmt-1,
     &      Zwork,g_imt-2,g_jmt-1)
          endif ! 1 PE.
C
C*--    V Surface Current :
C
        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and. (FieldLocator(istash,CouplingField).eq.'00122'))
     &      then
!         The use of Zwork_diagnos asa below implies that the
!         field will be cut in its 2 last columns : they overlap
!         the columns 1&2 and are not needed by the external model.
          if (l_ocomp) then
            CALL  UNPACK(1,JMT, 1,1,JMT,KM, IMT,JMT,1,
     &        O_CF I1, O_CFI2,joc_no_segs,O_CFI3,joc_no_seapts,
     &        D1(joc_v(2)),Zwork_Diagnos,
     &        RMDI,CYCLIC_OCEAN)
          else
            do i = 1, imt*jmt
              Zwork_Diagnos(i) = D1(joc_v(2)-i+1)
            enddo
          endif
...
C
C*--    Fields which to not need any particular handling (U grid):
C
        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and.   (FieldLocator(grd,CouplingField) .eq. 'U'))
     &      then
C         Pointer towards the coupling field in D1
...
C
C*--    Fields which to not need any particular handling (T grid):
C

        elseif  ((FieldLocator(direction,CouplingField) .eq. 'E')
     &      .and.   (FieldLocator(grd,CouplingField) .eq. 'T'))
     &      then
C         Pointer towards the coupling field in D1
          ptr_field = D1_Zptr(CouplingField)
C         Gather to 1 PE.
