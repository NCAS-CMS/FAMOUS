...
C******************************COPYRIGHT******************************
C
CLL   Routine: INIT_OASIS -------------------------------------------
CLL
CLL   Purpose: Initialises address pointers needed by OASIS_STEP when
CLL   running the UM with an external model connected by the OASIS
CLL   coupler.
CLL   Also handles the generation of the grid and mask defintions for
CLL   the coupler, plus the production of the restart fields for
CLL   OASIS when it restarts a coupled simulation. (Although those
CLL   last items might be moved outside this part of the code, into a
CLL   small executable scanning the dumps).
CLL   Takes care of opening the communication channels for each of
CLL   the fields exchanged with the coupler, plus the general one for
CLL   the coupler itself.
CLL   Subroutine FINDPTR and FINDLOOKPTR are used, in this instance
CLL   searching the STASHlist
CLL   on section/item codes and STASHmacro tagging information.
CLL
...
CLL  version  date
!LL  4.5     13/01/98 Replaced IOVARS by ATM_LSM            P.Burton
!LL   4.5    18/09/98  Corrected non-standard FORMAT statment
!LL                                                  P.Burton
CLL
CLL
CLL
...
C
      ...
      integer iost              ! return status of the open statement.
      external ini_z_ptr
C
C* ---------------------------- Include files ------------------------
C
...
C
      real Rearth
      parameter (Rearth = 6 366 198 ) ! Radius of the earth in meters
      real
     &  Zlatitude1              ! latitudes delimiting the gridboxes.
     &  , Zlatitude2            !
C
C declatarions for the atmosphere model :
C declations for the ocean model :
...
!
! Grid points definition. A virtual point is added at the begining
! and end of each of the coordinate system to allow a more regular
! computation of the surfaces.
!
      real
     &  xuo(0:g_imt+1)            ! ocean uv longitude coordinates
     &  ,xto(0:g_imt+1)           ! ocean ts longitude coordinates
     &  ,yuo(0:g_jmtm1+1)         ! ocean uv latitude coordinates
     &  ,yto(0:g_jmt+1)           ! ocean ts latitude coordinates
!
!     The masks are defined twice here, once as real,
!     once as integer. And that's because they are handled
!     as real in the UM while OASIS expects integers.
!
      integer
     &  imaskto(g_imt,g_jmt)        ! ocean mask on t grid
     &  , imaskuo(g_imt,g_jmt)    ! ocean mask on u grid
!
!     surface arrays
!
      real
     &  surfto(g_imt,g_jmt)         ! ocean surface on t grid
     &  , surfuo(g_imt,g_jmtm1)     ! ocean surface on u grid


!
!     Arrays holding the latitudes, longitudes for both sub-models,
!     and both type of grids.
!
      real
     &  Zxuo(g_imt,g_jmtm1)         ! oce longitude on u grid
     &  , Zxto(g_imt,g_jmt)         ! oce longitude on t grid
     &  , Zyuo(g_imt,g_jmtm1)       ! oce latitude on u grid
     &  , Zyto(g_imt,g_jmt)         ! oce latitude on t grid

      real
     &  fkmp(g_imt,g_jmt)       !in number of levels at ocean T points
     &  ,fkmq(g_imt,g_jmt)      !in number of levels at ocean U points


      integer ll, p             ! indexes.

c*--------------------------------------------------------------------
c
      write(nulou,*) "entering INIOASIS"
      icode = 0

C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
CCC   Read here the input values :
C     1/ number of coupling fields.
C     2/ the list of fields itself.

C     Search for end of 'coasis_in'
      ll=0
      do p=1,200
        if(coasis_in(p:p).ne.' ') then
          ll=ll+1
        endif
      enddo
c     construct filename with pe no. appended
      coasis_in(ll+1:ll+1)='.'
      write(coasis_in(ll+2:ll+5),'(i2.2)') mype

      open(unit = 3, file = coasis_in)
      read(3,*) NoCouplingField ! input the number of coupling fields.
      if (NoCouplingField .gt. MaxCouplingField) then
        icode=27
        cmessage='init_oasis: the number of coupling fields is to '
     &    //     'large : increase MaxCouplingField and recompile.'
        goto 999
      else
C       Input the description of the fields.
        do ii = 1, NoCouplingField
          read(3,'(a5,1x,a1,1x,a1,1x,a2,1x,a2)')
     &      Zinput(Zistash,ii),Zinput(Zgrd,ii),
     &      Zinput(Zdirection,ii),Zinput(Zexc_frequency,ii),
     &      Zinput(Zexc_basis,ii)
        enddo
      endif
      close(3)                  ! close oasis namefile.

c
c*--------------------------------------------------------------------
c
C     Initialise the FieldLocator array from the Input Array:
      do ii = 1, NoCouplingField
        FieldLocator(istash,ii)    = ZInput(Zistash,ii)
        FieldLocator(lon,ii)       = ZInput(Zistash,ii)(1:5) // 'lon'
        FieldLocator(lat,ii)       = ZInput(Zistash,ii)(1:5) // 'lat'
        FieldLocator(msk,ii)       = ZInput(Zistash,ii)(1:5) // 'msk'
        FieldLocator(srf,ii)       = ZInput(Zistash,ii)(1:5) // 'srf'
        FieldLocator(grd,ii)       = ZInput(Zgrd,ii)
        FieldLocator(direction,ii) = ZInput(Zdirection,ii)
        FieldLocator(exc_frequency,ii) = ZInput(Zexc_frequency,ii)
        FieldLocator(exc_basis,ii) = ZInput(Zexc_basis,ii)
      enddo
...
c*--------------------------------------------------------------------
c
c      Initialise the communication channels with the OASIS coupler :
c      Either create pipes, or pvm channels. ( pipes until now )
c
c      The pipes are only handled by one PE.
c
...
C
C*-   Most of the computation below need only be done on one PE.
C
      if (mype .eq. gather_pe) then

c*--------------------------------------------------------------------
C     Split the work according to wether the UM internal model
C     is the atmosphere or the ocean. Any other submodel is
C     not (yet) implemented and the code fails on error in such a
C     case.

      if(internal_model.eq.atmos_im) then ! atmosphere ( 1.a & 1.b )


cl--------------------------------------------------------------------
C     PART II : we now deal with the ocean in the following
C       sections.

      elseif (internal_model.eq.ocean_im) then ! ocean ( 2.a & 2.b )

C
CL    SECTION 1: No. of distinct columns in ocean.
C
...
C
cl--------------------------------------------------------------------
C       2. calculate size of each of the coupling fields using dump
C       info.
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grid
            FieldSize(ii) = (g_iru) * (g_jmtm1)
          else                  ! TS grid
            FieldSize(ii) = (g_irt) * (g_jmt)
          endif
        enddo

cl--------------------------------------------------------------------
cl    3. calculate gridline coordinates on all grids using dump
cl    information on grid spacing and position
        if (global_ocean.and..not.cyclic_ocean) then
          icode=24
          cmessage='init_oasis: a coupled global ocean must be cyclic'
          goto 999
        elseif (.not.global_ocean.and.cyclic_ocean) then
          icode=25
          cmessage='init_oasis: '
     &      //'a coupled limited-area ocean must not be cyclic'
          goto 999
        endif
!        if (a_realhd(5).ne.o_realhd(5).or.a_realhd(6).ne.o_realhd(6))
!     &    then
!          icode=26
!          cmessage='init_oasis: '
!     &    //'coupled atmosphere and ocean must have coincident poles'
!          goto 999
!        endif

! define the grids of the ocean UM submodel.
!    the global alternative can be removed when we are sure that the
!     ocean dump headers have been correctly created
        if (global_ocean) then
          xuo(1)=o_realhd(4)+0.5*o_realhd(1)
        else
          xuo(1)=o_realhd(8)
        endif
        xuo(0)=xuo(1)-o_coldepc(1)
        xto(1)=xuo(1)-0.5*o_coldepc(1)
        xto(0)=xto(1)-o_coldepc(1)
        do ii=2,g_imt
          xuo(ii)=xuo(ii-1)+o_coldepc(ii)
          xto(ii)=xto(ii-1)+0.5*(o_coldepc(ii-1)+o_coldepc(ii))
        enddo
        xuo(g_imt+1)=xuo(g_imt)+o_coldepc(g_imt)
        xto(g_imt+1)=xto(g_imt)+o_coldepc(g_imt)

        yuo(1)=o_realhd(7)
        yuo(0)=yuo(1)-o_rowdepc(1)
        yto(1)=yuo(1)-0.5*o_rowdepc(1)
        yto(0)=yto(1)-o_rowdepc(1)
        do j=2,g_jmt
          yuo(j)=yuo(j-1)+o_rowdepc(j)
          yto(j)=yto(j-1)+0.5*(o_rowdepc(j-1)+o_rowdepc(j))
        enddo
        yto(g_jmt+1) = yto(g_jmt)+o_rowdepc(g_jmt)

! Develop those 1D array onto 2D arrays in order to suit the oasis
! layout.
        do  ii = 1, g_imt
          do j = 1, g_jmtm1
            Zxuo(ii,j) = xuo(ii) ! longitude on u grid
            Zyuo(ii,j) = yuo(j) ! latitude on u grid
          enddo
        enddo

        do  ii = 1, g_imt
          do j = 1, g_jmt
            Zxto(ii,j) = xto(ii) ! longitude on t grid
            Zyto(ii,j) = yto(j) ! latitude on t grid
          enddo
        enddo

! define the mask grids of the ocean UM submodel.
!       TS grid :  (use the number of levels)

        do  j = 1, g_jmt
          do ii = 1, g_imt
            if ( o_flddepc(ii+(j-1)*g_imt) .lt. 0.1 ) then
              imaskto(ii,j) = 1
            else
              imaskto(ii,j) = 0
            endif
          enddo
        enddo
!       UV grid : (use the number of levels)
        do  j = 1, g_jmt
          do ii = 1, g_imt
            if ( (o_spcon(jocp_fkmq_global+ii-1+(j-1)*g_imt))
     &        .LT. (0.1) ) then
              imaskuo(ii,j) = 1
            else
              imaskuo(ii,j) = 0
...
!---------------------------------------------------------------------
! prepare the surface grids here :
! Note that I produce the surfaces at the centre of the gridboxes.

!       on U grid :
        do j = 1, g_jmtm1
          do  ii = 1, g_imt
            Zlatitude1 = (yuo(j-1)+yuo(j))/2.
            Zlatitude2 = (yuo(j+1)+yuo(j))/2.
            if (Zlatitude1 .gt.  90.0) Zlatitude1 =  90.0
            if (Zlatitude2 .gt.  90.0) Zlatitude2 =  90.0
            if (Zlatitude1 .lt. -90.0) Zlatitude1 = -90.0
            if (Zlatitude2 .lt. -90.0) Zlatitude2 = -90.0
            surfuo(ii,j)  = Rearth * Rearth ! on u grid
     &        * abs(
     &        (   sin(pi_over_180* Zlatitude2 )
     &          - sin(pi_over_180* Zlatitude1 ) )
     &        * pi_over_180
     &        * ((xuo(ii+1)+xuo(ii))/2. - (xuo(ii-1)+xuo(ii))/2.)
     &        )
          enddo
        enddo
...
!       on T grid
        do j = 1, g_jmt
          do  ii = 1, g_imt
            Zlatitude1 = (yto(j-1)+yto(j))/2.
            Zlatitude2 = (yto(j+1)+yto(j))/2.
            if (Zlatitude1 .gt.  90.0) Zlatitude1 =  90.0
            if (Zlatitude2 .gt.  90.0) Zlatitude2 =  90.0
            if (Zlatitude1 .lt. -90.0) Zlatitude1 = -90.0
            if (Zlatitude2 .lt. -90.0) Zlatitude2 = -90.0
            surfto(ii,j)  = Rearth * Rearth ! on t grid
     &        * abs(
     &        (   sin(pi_over_180*Zlatitude1)
     &          - sin(pi_over_180*Zlatitude2) )
     &        * pi_over_180
     &        * ((xto(ii+1)+xto(ii))/2. - (xto(ii-1)+xto(ii))/2.)
     &        )
          enddo
        enddo
...
!---------------------------------------------------------------------
! Write to file the grids, masks and surface of each of the coupling
! fields.
!      a/import of fields
!      b/export of fields
        nulgr = 3
        OPEN (UNIT = nulgr,  POSITION = 'APPEND',
     &    FILE = cficgr,STATUS = 'UNKNOWN',
     &    FORM = 'UNFORMATTED',IOSTAT = iost)
        if (iost .ne. 0) then
          icode = 1
          cmessage = 'io error in INIT_OASIS from UM ocean model.'
        endif
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            !   locator for the longitude array:
            write(nulgr) FieldLocator(lon,ii)
            !   longitude array:
            write(nulgr) ((Zxuo(i,j),i = 1,g_iru), j=1,g_jmtm1)
            !   locator for the latitude array:
            write(nulgr) FieldLocator(lat,ii)
            !   latitude array:
            write(nulgr) ((Zyuo(i,j),i = 1,g_iru), j=1,g_jmtm1)
            !   mask locator :
          else                  ! T grids
            !   locator for the longitude array:
            write(nulgr) FieldLocator(lon,ii)
            !   longitude array:
            write(nulgr) ((Zxto(i,j),i = 1,g_irt), j=1,g_jmt)
            !   locator for the latitude array:
            write(nulgr) FieldLocator(lat,ii)
            !   latitude array:
            write(nulgr) ((Zyto(i,j),i = 1,g_irt), j=1,g_jmt)
            !    mask locator :
          endif
        enddo
! Close the grids, masks and surface files.
        close(nulgr)

!---------------------------------------------------------------------
! Write to file the masks of the coupling
! fields.
!      a/import of fields
!      b/export of fields
        nulsu = 3
        OPEN (UNIT = nulsu, POSITION = 'APPEND',
     &    FILE = cficsu,STATUS = 'UNKNOWN',
     &    FORM = 'UNFORMATTED',IOSTAT = iost)
        if (iost .ne. 0) then
          icode = 1
          cmessage = 'io error in INIT_OASIS from UM ocean model.'
        ...
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            !   locator for the longitude array:
            write(nulsu) FieldLocator(srf,ii)
            !   surface filed for tx :
            write(nulsu) ((surfuo(i,j),i = 1,g_iru), j=1,g_jmtm1)
          else                  ! T grids
            !   surface locator :
            write(nulsu) FieldLocator(srf,ii)
            !   surface field  :
            write(nulsu) ((surfto(i,j),i = 1,g_irt), j=1,g_jmt)
          endif
        enddo
! Close the grids, masks and surface files.
        close(nulsu)
...
!      b/export of fields
        nulma = 3
        OPEN (UNIT = nulma, POSITION = 'APPEND',
     &    FILE = cficma,STATUS = 'UNKNOWN',
     &    FORM = 'UNFORMATTED',IOSTAT = iost)
        if (iost .ne. 0) then
          icode = 1
          cmessage = 'io error in INIT_OASIS from UM ocean model.'
        ...
        do ii = 1, NoCouplingField
          if (FieldLocator(grd,ii) .eq. 'U') then ! U grids
            !   mask locator :
            write(nulma) FieldLocator(msk,ii)
            write(nulma) ((imaskuo(i,j),i = 1,g_iru),j=1,g_jmtm1)
          else                  ! T grids
            !    mask locator :
            write(nulma) FieldLocator(msk,ii)
            write(nulma) ((imaskto(i,j),i = 1,g_irt), j=1,g_jmt)
          endif
        enddo
! Close the grids, masks and surface files.
        close(nulma)
      ...
      else        ! neither ocean or atmosphere UM have been selected.
        icode = -1
        write(nulou,*)
     ...
     &    'the atmosphere or the ocean not currently allowed.'
        goto 999
      endif  ! atmos or ocean model.

      endif                     ! mype.
!
!      Initialise pointers of the coupling fields to the D1 array :
!
...
c
c      Send OK to the coupler indicating the initialization phase has
c      been completed.
c
c      The pipes are only handled by one PE.
c
