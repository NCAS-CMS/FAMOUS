...
!                 on Fujitsu VPP700 - saves 38%.  RBarnes@ecmwf.int
!
! Description of Code:
...
!
     ...
     &    (1.0E+00+SCALE_PARAMETER(2)*(T(L, I)/T_REFERENCE-1.0E+00)   
     &            +SCALE_PARAMETER(3)*(T(L, I)/T_REFERENCE-1.0E+00)**2) 
            ENDDO
         ENDDO
...
!
         DO I=I_TOP, N_LAYER
            DO L=1, N_PROFILE
               GAS_FRAC_RESCALED(L, I)=PWK(L,I-I_TOP+1)
     &            *(1.0E+00
     &            +SCALE_PARAMETER(3)*(T(L, I)/T_REFERENCE-1.0E+00)
     &            +SCALE_PARAMETER(4)*(T(L, I)/T_REFERENCE-1.0E+00)**2)
            ENDDO
         ENDDO
      ELSE
         WRITE(IU_ERR, '(/A)')
