...
!LL                   Not bit-reproducible with COFTHQ1A.
...
C*L   ARGUMENTS:---------------------------------------------------
     ...
     7                   KD,DELTA_AK,DELTA_BK,PSTAR_UV,COS_FUNCTION_U)
     ...
     *, LEVEL_BASE         !IN BOTTOM MODEL LEVEL FOR DIFFUSION
...
! End of comdeck TYPFLDPT
      ...
      REAL
     * DIFFUSION_EW(P_FIELD,P_LEVELS)  !OUT HOLDS EAST-WEST
     *                                 !EFFECTIVE DIFFUSION COEFFICIENT.
     *,DIFFUSION_NS(P_FIELD,P_LEVELS)  !OUT HOLDS NORTH_SOUTH
     *                                 !EFFECTIVE DIFFUSION COEFFICIENT.
     ...
     *,PSTAR_UV(U_FIELD)               !IN PSTAR AT U POINTS
     *,COS_U_LATITUDE(U_FIELD)         !IN COS(LAT) AT U POINTS
     *,COS_FUNCTION_U(U_FIELD)         !IN
...
! Define local arrays
      ...
      REAL
     * DIFFUSION_COEFFICIENT(U_FIELD)  ! HOLDS EAST-WEST DIFFUSION
     *                                 ! COEFFICIENT.
     *,DIFFUSION_COEFFICIENT2(U_FIELD) ! HOLDS NORTH-SOUTH DIFFUSION
     *                                 ! COEFFICIENT.
!                           ! zero at a point
...
C SET DIFFUSION COEFFICIENT AND COPY THETAL INTO FIELD1.
     ...
     1        (DELTA_AK(LEVEL)+DELTA_BK(LEVEL)*PSTAR_UV(I))
...
C     RECALCULATE END-POINTS
      ...
      DIFFUSION_NS(START_POINT_NO_HALO-ROW_LENGTH,LEVEL)=
     &  DIFFUSION_NS(START_POINT_NO_HALO-ROW_LENGTH+1,LEVEL)
