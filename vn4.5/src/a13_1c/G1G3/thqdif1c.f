...
C ASSUMING FLUX=0 AT HALF-GRID-LENGTH ON OTHER SIDE OF POLE
      ...
      SCALAR=SEC_P_LATITUDE(TOP_ROW_START)

! Do North Pole
      IF (at_top_of_LPG) THEN
        DO I=1,ROW_LENGTH
          J=TOP_ROW_START+I-1
          FIELD3(J)=-DIFFUSION_NS(J,K)*FIELD1(J)*SCALAR
          NP_FLUX(I)=FIELD3(J)
          FIELD_INC(J)=0.0
        ENDDO
      ENDIF
...
! And South Pole
      SCALAR=SEC_P_LATITUDE(P_BOT_ROW_START)

      IF (at_base_of_LPG) THEN
        DO I=1,ROW_LENGTH
          J=I+P_BOT_ROW_START-1
          FIELD3(J)=DIFFUSION_NS(J-ROW_LENGTH,K)*FIELD1(J-ROW_LENGTH)*
     &              SCALAR
          SP_FLUX(I)=FIELD3(J)
          FIELD_INC(J)=0.0
        ENDDO
