...
CLL  as well as effective cloud top and base amount in ECTA and ECBA
...
C     ! the in-cloud value.
     ...
     &  , ECTA(L2,NCLDS,NBANDS),  ! "effective cloud top amount"
     &    ECBA(L2,NCLDS,NBANDS)   ! "effective cloud bottom amount"
C*
...
C        ! Equation 2.3.1:
     ...
     &             CECL = CECL * ( 1. - CCA(J) )
         ...
         ECTA(J,LEVEL,BAND) = CECL
         ECBA(J,LEVEL,BAND) = CECL
        end do                                                          
       end do                                                           
      end do                                                    
C
...
C
      DO BAND=1, NBANDS                                             
       DO J=1, L2                                                   
        ...
        ECBA(J,CCB(J),BAND) = ECBA(J,CCB(J),BAND) + CECC(J)
        ECTA(J,CCT(J)-1,BAND) = ECTA(J,CCT(J)-1,BAND) + CECC(J)
       end do                
