...
!LL                   as THADV1C but with reintroduced error in
!LL                   calculation of S.Pole THETAL_INCREMENT.  T.Johns
!LL   4.3   24/04/97  Updated in line with MPP optimisations.
!LL                   Complete coding non-local communication. T Johns
!LL 4.3      24/04/97 Fixes to 4th order calculations   P.Burton
!LL   4.4    03/12/97 Fix to HadCM2 deliberate bug S.D.Mullerworth
CLL
...
! Extra local arrays to allow N.polar PSTAR and P_EXNER to be sent 
! to S.polar processors, recreating the HADCM2 behaviour caused by
! an error in indexing in the original advection code.

      REAL
     &    PSTAR_NP(ROW_LENGTH),
     &    P_EXNER_NP(ROW_LENGTH,P_LEVELS+1)
...
! For HADCM2, first send (from N.pole) and receive (at S.Pole) the 
! requisite N. polar rows of PSTAR and P_EXNER via local arrays,
! but don't bother to call the communications routines if the northern
! and southern polar rows are on the same processor.
! The ids of the corresponding receiving/sending processors are computed
! based on an assumed sequential numbering of PEs as defined in PARVARS.

! send and receive PSTAR first.

       CALL GC_GSYNC(nproc,info)  ! synchronise all PEs at this point 

       IF (at_top_of_LPG) THEN  ! This PE is at the N pole
! Send PSTAR N.polar rows to the corresponding S polar PE
         DO I=1,ROW_LENGTH
           PSTAR_NP(I)=PSTAR(I+TOP_ROW_START-1)
         END DO
         IF (.NOT.at_base_of_LPG) THEN   ! only send if not at S pole
           CALL GC_RSEND(4002,ROW_LENGTH,
     &                   (mype+nproc_x*(nproc_y-1)),info,
     &                   PSTAR_NP,PSTAR_NP)

         ENDIF
       ENDIF

       CALL GC_GSYNC(nproc,info)  ! synchronise all PEs before receive 

       IF (at_base_of_LPG) THEN
! Get PSTAR  N.polar rows from the corresponding N polar PE
         IF (.NOT.at_top_of_LPG) THEN

           CALL GC_RRECV(4002,ROW_LENGTH,
     &                   (mype-nproc_x*(nproc_y-1)),info,
     &                   PSTAR_NP,PSTAR_NP)
...
! now transfer P_EXNER data

       IF (at_top_of_LPG) THEN  ! This PE is at the N pole
! Send P_EXNER N.polar rows to the corresponding S polar PE
         DO K=1,P_LEVELS+1  ! copy N.pole values of P_EXNER
           DO I=1,ROW_LENGTH
             P_EXNER_NP(I,K)=P_EXNER(I+TOP_ROW_START-1,K)
           END DO
         END DO
         IF (.NOT.at_base_of_LPG) THEN   ! only send if not at S pole 
           CALL GC_RSEND(4001,(P_LEVELS+1)*ROW_LENGTH,
     &                   (mype+nproc_x*(nproc_y-1)),info,
     &                   P_EXNER_NP,P_EXNER_NP)
       ...
       IF (at_base_of_LPG) THEN
! Get P_EXNER  N.polar rows from the corresponding N polar PE
         IF (.NOT.at_top_of_LPG) THEN
           CALL GC_RRECV(4001,(P_LEVELS+1)*ROW_LENGTH,
     &                   (mype-nproc_x*(nproc_y-1)),info,
     &                   P_EXNER_NP,P_EXNER_NP)
         ENDIF
       ENDIF

       CALL GC_GSYNC(nproc,info)  ! synchronise all PEs after transfer 
...
! Southern boundary/pole
     ...
     &            (DELTA_AK(K1)+DELTA_BK(K1)*PSTAR(I))
! Reintroduce error in indexing at S.Pole, to get N.Polar PSTAR and
! P_EXNER -- this involves additional non-local data fetches.

! Old                PK  = AKH(K)  + BKH(K) *PSTAR(I)
! Old                PK1 = AKH(K1) + BKH(K1)*PSTAR(I)  !  K1 = K-1
! Old                P_EXNER_FULL = P_EXNER_C(P_EXNER(I,K),
! Old     &                                 P_EXNER(I,K1),PK,PK1,KAPPA)

! New (HADCM2 uses received N Polar row values sent earlier)
                PK  = AKH(K)  + BKH(K) *PSTAR_NP(I-P_BOT_ROW_START+1)
                PK1 = AKH(K1) + BKH(K1)*PSTAR_NP(I-P_BOT_ROW_START+1)
                P_EXNER_FULL = P_EXNER_C(
     &                           P_EXNER_NP(I-P_BOT_ROW_START+1,K),
     &                           P_EXNER_NP(I-P_BOT_ROW_START+1,K1),
     &                           PK,PK1,KAPPA)

! End New (with HADCM2 error in indexing)
