...
! Hopefully we shouldn't need these lines if loop bounds are right
!      CALL SET_SIDES(WORK1,U_FIELD,ROW_LENGTH,1,fld_type_u)
!      CALL SET_SIDES(WORK2,U_FIELD,ROW_LENGTH,1,fld_type_u)
...
! If processor includes North or South polar row, compute a pt. on it
      IF (at_top_of_LPG) THEN
      FIRST_POINT = FIRST_POINT -Offx -1
      POINTS = POINTS +Offx +1
      END IF
...
C  Copy polar values along row
      DO K=1,P_LEVELS
      IF (at_top_of_LPG) THEN
        DO  I = FIRST_VALID_PT+Offx,START_POINT_NO_HALO-Offx-2
          THETAL(I,K) = THETAL(START_POINT_NO_HALO-Offx-1,K)
        END DO
      END IF
      IF (at_base_of_LPG) THEN
        DO  I = END_P_POINT_NO_HALO+Offx+2,LAST_P_VALID_PT-Offx
          THETAL(I,K) = THETAL(END_P_POINT_NO_HALO+Offx+1,K)   
        END DO
      END IF
      END DO
! Update the halos for the THETAL array                                 
...
C  Copy polar values along row
      ...
      IF (at_top_of_LPG) THEN
        DO  I = FIRST_VALID_PT+Offx,START_POINT_NO_HALO-Offx-2
          QT(I,K) = QT(START_POINT_NO_HALO-Offx-1,K)
        END DO
      END IF
      IF (at_base_of_LPG) THEN
        DO  I = END_P_POINT_NO_HALO+Offx+2,LAST_P_VALID_PT-Offx
          QT(I,K) = QT(END_P_POINT_NO_HALO+Offx+1,K)   
        END DO
      END IF
      END DO
! Update the halos for the QT array                                     
