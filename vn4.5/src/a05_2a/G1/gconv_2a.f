...
!            routines.
!       2A version. Standard downdraught version.
!
!   Code Description:
...
!
!   Modification History from U.M. version 4.0:
!
!   Version      Date
!
!     4.1        10/6/96  : Changed dimensions of momentum arrays
!                           to allow convection to be split into
!                           segments when momentum transports are
!                           used.
!     4.4        14/10/97 : Argument list changed to be consistent
!                           with CONV_CTL at vn4.4.    J.M.Gregory
!LL  4.4    Oct 97  Add halo mask to stop redundant calculations
!LL                 Add arguments L_XSCOMP and LSDXS to fix
!LL                 argument mismatch
!LL                                               Alan Dickinson
!
...
!  END -----------------------------------------------------------------
     ...
     &     TIMESTEP,
     &     T1_SD,Q1_SD,
     &     L_MOM,L_TRACER,L_CAPE,
     &     NTRA,TRLEV,L_XSCOMP,L_SDXS
     &    ,l_halo
     &     ,N_CCA_LEV, L_3D_CCA, L_CCW, MPARWTR
     &     , ANVIL_FACTOR, TOWER_FACTOR, UD_FACTOR
     &     , L_CLOUD_DEEP, L_PHASE_LIM  
...
C
      LOGICAL L_MOM               ! IN SWITCH FOR INCLUSION OF
                                  !    MOMENTUM TRANSPORTS
      LOGICAL L_XSCOMP            ! IN unused argument
      LOGICAL L_SDXS              ! IN unused argument
C
...
! Local variables
      ...
      REAL    MPARWTR        ! \
     &,       ANVIL_FACTOR   !  \ DUMMY variables added to match        
     &,       TOWER_FACTOR   ! /  argument list
     &,       UD_FACTOR      ! /
!
      INTEGER N_CCA_LEV      !    DUMMY
!
      LOGICAL L_3D_CCA       !\   DUMMY variables added to match arg.   
     &,       L_CCW          ! \  list passed in from CONV_CTL          
     &,       L_CLOUD_DEEP   ! /
     &,       L_PHASE_LIM    !/
!
      LOGICAL FLG_UP_FLX     ! Dummy variable to match CONV_CTL arg list
!
      LOGICAL FLG_DWN_FLX    !              " "
!
      LOGICAL FLG_ENTR_UP    !              " "
!
      LOGICAL FLG_ENTR_DWN   !              " "
!
      LOGICAL FLG_DETR_UP    !              " "
!
      LOGICAL FLG_DETR_DWN   !              " "
!                                                                       
      REAL UP_FLUX(NP_FIELD,NLEV)     !     " "
!
      REAL DWN_FLUX(NP_FIELD,NLEV)    !     " "
!
      REAL ENTRAIN_UP(NP_FIELD,NLEV)  !     " "
!
      REAL DETRAIN_UP(NP_FIELD,NLEV)  !     " "
!
      REAL ENTRAIN_DWN(NP_FIELD,NLEV) !     " "
!
      REAL DETRAIN_DWN(NP_FIELD,NLEV) !     " "
