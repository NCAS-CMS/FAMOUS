...
! Subroutine Interface:
      SUBROUTINE SOIL_HTC (
     & NPNTS,NSHYD,SOIL_PTS,SOIL_INDEX,B,DZ,HCAP,HCON,LYING_SNOW
     &,SATHH,SURF_HT_FLUX,TIMESTEP,V_SAT,W_FLUX
...
!  4.4      24/11/97   Offset avoids calculations on uninitialised
!                      MPP halos of SMCFU and SMCFF. S.D.Mullerworth
!LL   4.5   18/06/98  Changed Timer calls to indicate non-barrier
!LL                                                   P.Burton
!  4.5  22/06/98  Extra Fujitsu vectorization directive for 
!                 snow insulation loop.  R.Barnes@ecmwf.int
!
! Code Description:
...
!   Array arguments with intent(IN) :
     ...
     &,LYING_SNOW(NPNTS)    ! IN Lying snow (kg/m2).
...
!OCL NOVREC
      DO J=1,SOIL_PTS
        ...
        IF (LYING_SNOW(I) .GT. 0.0) THEN
          SNOW_DEPTH = LYING_SNOW(I) / RHO_SNOW
          IF (SNOW_DEPTH .LE. (0.5*DZ(1))) THEN
            SIFACT(I)=1.0
     &            /(1.0+2*SNOW_DEPTH/(DZ(1) + DZ(2)))
          ELSE
            SIFACT(I)=(DZ(1)+DZ(2))
     &          /(HC(I,1)/SNOW_HCON*(2*SNOW_DEPTH-DZ(1))
     &            +2*DZ(1)+DZ(2))
          ENDIF
        ELSE
          SIFACT(I)=1.0
