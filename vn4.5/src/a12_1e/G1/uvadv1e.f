...
!LL                   Not bit-reproducible with UVADV1C.
!LL 4.4      07/08/97 Removed GCG_RMIN call from loop over levels
!LL                                                      P.Burton
...
CLL                 test for section 2.1, so that it can be used later 
...
C DEFINE LOCAL ARRAYS: 35 ARE REQUIRED
     ...
     &                   !   OF P-GRID.
      ...
      REAL
     & U_SECOND_INC(U_FIELD,P_LEVELS)      ! HOLDS U INCREMENT
     &                            !RETURNED BY SECOND CALL TO ADV_U_GD
     &,U_PROV(U_FIELD,P_LEVELS)            ! HOLDS PROVISIONAL VALUE OF

      REAL
     & V_SECOND_INC(U_FIELD,P_LEVELS)      ! HOLDS V INCREMENT
     &                            !RETURNED BY SECOND CALL TO ADV_U_GD
...
C TIME AS ALL USES OF THEM IN CALCULATION ARE CONTROLLED BY *IF'S.
     ...
     &                    ! USED IN NORTH-SOUTH ADVECTION.
      ...
      REAL NUX_MIN(upd_U_ROWS,P_LEVELS),
!          minimum value of NUX along a row
     &     NUY_MIN(ROW_LENGTH-2*EW_Halo,P_LEVELS)
!          minimum value of NUY along a column
...
! Initialise RS_U prior to P_TO_UV call
      DO K=1,P_LEVELS
! cdir$ cache_bypass rs_u
        DO I=1,FIRST_VALID_PT-1
          RS_U(I,K)=0.0
        ENDDO
! cdir$ cache_bypass rs_u
        DO I=LAST_U_VALID_PT-ROW_LENGTH,U_FIELD
          RS_U(I,K)=0.0
      ENDDO
      ENDDO
...
! for each level (inefficient)
      ...
      DO K=1,P_LEVELS
! Initialise U_MEAN_P and V_MEAN_P prior to UV_P call
! cdir$ cache_bypass u_mean_p
        DO I=1,FIRST_VALID_PT-1
        U_MEAN_P(I,K)=0.0
        ENDDO
! cdir$ cache_bypass u_mean_p
        DO I=LAST_U_VALID_PT+1,U_FIELD
          U_MEAN_P(I,K)=0.0
        ENDDO
! cdir$ cache_bypass v_mean_p
        DO I=1,FIRST_VALID_PT-1
        V_MEAN_P(I,K)=0.0
      ENDDO
! cdir$ cache_bypass v_mean_p
...
C-----------------------------------------------------------------
C
C U_SECOND_INC is used for
C                          1./(RS_U(I,K)*RS_U(I,K)
C    &                    *(DELTA_AK(K)+DELTA_BK(K)*PSTAR_OLD(I)))
C-----------------------------------------------------------------
      DO K=1,P_LEVELS
        DO I=START_POINT_NO_HALO,END_U_POINT_NO_HALO
           U_SECOND_INC(I,K) = 1.0/(RS_U(I,K)*RS_U(I,K)*
     &              (DELTA_AK(K)+DELTA_BK(K)*PSTAR_OLD(I)))
        END DO
      END DO

!L If NU_BASIC not equal to zero (ie. 4th order)
...
C-----------------------------------------------------------------
          DO I=START_POINT_NO_HALO,END_U_POINT_NO_HALO
            ...
            SCALAR3 = TIMESTEP*U_SECOND_INC(I,K)
...
! Calculate minimum along this row
            ...
            ENDDO
            NUX_MIN(J-FIRST_ROW+1,K)=MAX(SCALAR1,0.0)
...
! I_end points to the end of column J
            ...
            ENDDO
            NUY_MIN(J-FIRST_ROW_PT+1,K)=MAX(SCALAR1,0.0)

          ENDDO ! J : loop over columns

      ENDDO ! K: loop over levels

! We have so far only calculated local NUX_MIN and NUY_MINs. We must
! now calculate the minimum values across all processors in the row/
! column respectively.

      CALL GCG_RMIN(upd_U_ROWS*P_LEVELS,GC_ROW_GROUP,info,NUX_MIN)

      CALL GCG_RMIN((ROW_LENGTH-2*EW_Halo)*P_LEVELS,GC_COL_GROUP,
     &              info,NUY_MIN)

! And now copy these values back to NUX and NUY arrays
      ...
      DO K=1,P_LEVELS
        ...
        DO J=FIRST_ROW,FIRST_ROW+upd_U_ROWS-1
          ...
          DO I=FIRST_ROW_PT,LAST_ROW_PT
            ...
            NUX(I+(J-1)*ROW_LENGTH,K)=NUX_MIN(J-FIRST_ROW+1,K)

            NUY(I+(J-1)*ROW_LENGTH,K)=NUY_MIN(I-FIRST_ROW_PT+1,K)
      ...
      ENDIF ! End of 4th order calculations
...
C BRSP IS CURRENTLY HELD IN OMEGA
          ...
          CALL ADV_U_GD(P_LEVELS,U,
     &                    U_MEAN_P_COPY,V_MEAN_P_COPY,
     &                    ETADOT_U,
     &                    SEC_U_LATITUDE,U_PROV,
     &                    NUX,NUY,U_FIELD,
     &                    ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     ...
     &                    OMEGA,L_SECOND,LWHITBROM,
     &                    U_COPY,extended_U_FIELD,
     &                    extended_address)

CL    CALL ADV_U_GD FOR V.
          CALL ADV_U_GD(P_LEVELS,V,
     &                    U_MEAN_P_COPY,V_MEAN_P_COPY,
     &                    ETADOT_U,
     &                    SEC_U_LATITUDE,V_PROV,
     &                    NUX,NUY,U_FIELD,
     &                    ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
...
c-----------------------------------------------------------------
          DO I=START_POINT_NO_HALO,END_U_POINT_NO_HALO-1
            ...
            SCALAR1=U_SECOND_INC(I,K)
            U_PROV(I,K) = U(I,K)- U_PROV(I,K)*SCALAR1
            V_PROV(I,K) = V(I,K)-V_PROV(I,K)*SCALAR1
          ENDDO
...
C BRSP IS CURRENTLY HELD IN OMEGA
          ...
          CALL ADV_U_GD(P_LEVELS,U_PROV,
     &                  U_MEAN_P_COPY,V_MEAN_P_COPY,
     &                  ETADOT_U,SEC_U_LATITUDE,
     &                  U_SECOND_INC,NUX,NUY,U_FIELD,
     &                  ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
...
CL    CALL ADV_U_GD FOR V.
          CALL ADV_U_GD(P_LEVELS,V_PROV,
     &                  U_MEAN_P_COPY,V_MEAN_P_COPY,
     &                  ETADOT_U,SEC_U_LATITUDE,
     &                  V_SECOND_INC,NUX,NUY,U_FIELD,
     &                  ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
...
C ---------------------------------------------------------------------
     ...
     &                           (DELTA_AK(K)+DELTA_BK(K)*PSTAR(I))
        U_SECOND_INC(I,K)=U_SECOND_INC(I,K)/SCALAR1
        V_SECOND_INC(I,K)=V_SECOND_INC(I,K)/SCALAR1
        WK(I)=WK(I)/SCALAR1
      END DO
...
CL    CORIOIS TERM AND ADDED ONTO MASS-WEIGHTED FIELD.
        ...
        DO I=START_POINT_NO_HALO,END_U_POINT_NO_HALO
          SCALAR3 = 1./RS_U(I,K)
          U(I,K)=0.5 * (U(I,K)-U_SECOND_INC(I,K)+U_PROV(I,K))
          V(I,K)=0.5 * (V(I,K)-V_SECOND_INC(I,K)+V_PROV(I,K))
        ENDDO
        IF (LWHITBROM) THEN
          DO I=START_POINT_NO_HALO,END_U_POINT_NO_HALO
