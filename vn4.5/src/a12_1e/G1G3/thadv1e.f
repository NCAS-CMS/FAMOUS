...
C ******************************COPYRIGHT******************************
CLL   SUBROUTINE TH_Q_ADV -------------------------------------------
CLL
CLL   PURPOSE:  CALCULATES MASS-WEIGHTED INCREMENTS TO THETAL & QT
...
CLL VERSION  DATE
CLL
CLL          07/12/95 New version of routine specifically for MPP
CLL                   P.Burton
!LL          16/08/96  Add TYPFLDPT arguments to FILTER subroutine
!LL                    and make the FILTER_WAVE_NUMBER arrays
!LL                    globally sized                       P.Burton
!LL          10/01/97  Initialise unprocessed points in THETAL_PROV.
!LL                    D. Robinson.
!LL          24/04/97 Fixes to 4th order calculations   P.Burton
C              Mar. 97   T3E migration : optimisation changes
C                                       D.Salmond
CLL
!LL   4.4   11/08/97   New version optimised for T3E.
!LL                    Not bit-reproducible with THADV1C
CLL   4.4    04/08/97  Optimisation for T3E  D.Salmond
CLL  4.5  19/12/97  Move calculation of 1/RS*RS*P outside 4th order IF
CLL                 test in section 2.1, so that it can be used later
CCL                 in 2nd order code too.                 RTHBarnes.
CLL
...
C*L   ARGUMENTS:---------------------------------------------------
      SUBROUTINE TH_Q_ADV
     1              (THETAL,QT,PSTAR_OLD,PSTAR,U_MEAN,V_MEAN,
...
C REAL SCALARS
     ...
     &,R_P_EXNER_FULL    ! Exner pressure at full model level
...
CL---------------------------------------------------------------------
C
C   THETAL_INCREMENT HOLDS 1.0/(RS(I,K)*RS(I,K) 
C    *                *(DELTA_AK(K)+DELTA_BK(K)*PSTAR_OLD(I)))
C-------------------------------------------------------------------
        DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO
          THETAL_INCREMENT(I,K)=1.0/(RS(I,K)*RS(I,K)
...
CL    CALCULATE NU FROM COURANT NUMBER INCLUDING TRIG AND RS TERMS.
          DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO
            ...
            SCALAR1 = TIMESTEP*THETAL_INCREMENT(I,K)
...
C ---------------------------------------------------------------------
          ...
          CALL ADV_P_GD(P_LEVELS,THETAL,
     *                  U_MEAN_COPY,V_MEAN_COPY,
     &                  ETADOT_MEAN,SEC_P_LATITUDE,
     *                  THETAL_FIRST_INC,NUX,NUY,P_FIELD,
     *                  U_FIELD,ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
     ...
     *                  BRSP,L_SECOND,LWHITBROM,
     &                  T_COPY,extended_P_FIELD,extended_U_FIELD,
     &                  extended_address)
...
C-------------------------------------------------------------------
C
C   THETAL_INCREMENT HOLDS 1.0/(RS(I,K)*
C    *                RS(I,K)*(DELTA_AK(K)+DELTA_BK(K)*PSTAR_OLD(I)))
C-------------------------------------------------------------------
          DO I=START_POINT_NO_HALO,END_P_POINT_NO_HALO
            SCALAR1 = THETAL_INCREMENT(I,K)
...
C ---------------------------------------------------------------------
          ...
          CALL ADV_P_GD(P_LEVELS,THETAL_PROV,
     *                  U_MEAN_COPY,V_MEAN_COPY,
     &                  ETADOT_MEAN,SEC_P_LATITUDE,
     *                  THETAL_INCREMENT,NUX,NUY,P_FIELD,
     *                  U_FIELD,ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
...
C ---------------------------------------------------------------------
C
CL    END OF THETAL ADVECTION
C
C ---------------------------------------------------------------------
C ---------------------------------------------------------------------
C
CL   SECTION 5.    START QT ADVECTION
C
C ---------------------------------------------------------------------
...
! Loop over entire field
     ...
     *                *BKH(K)*.25*(PSTAR(I)-PSTAR_OLD(I))
      ...
      IF (.NOT.L_SECOND) THEN  ! fourth order
        CALL COPY_FIELD(QT,T_COPY,
     &                  P_FIELD,extended_P_FIELD,
...
CL
C ---------------------------------------------------------------------
CL    SECTION 5.2    CALL ADV_P_GD TO OBTAIN FIRST INCREMENT DUE TO
CL                   ADVECTION.
C ---------------------------------------------------------------------

CL    CALL ADV_P_GD FOR QT.
          CALL ADV_P_GD(Q_LEVELS,QT,
     &                  U_MEAN_COPY,V_MEAN_COPY,
     &                  ETADOT_MEAN,SEC_P_LATITUDE,
     *                  THETAL_FIRST_INC,NUX,NUY,P_FIELD,
     *                  U_FIELD,ROW_LENGTH,
     ...
     *                  LONGITUDE_STEP_INVERSE,
     &                  SEC_U_LATITUDE,BRSP,L_SECOND,LWHITBROM,
     &                  T_COPY,extended_P_FIELD,extended_U_FIELD,
     &                  extended_address)

C ---------------------------------------------------------------------
CL    SECTION 5.3    REMOVE MASS-WEIGHTING FROM INCREMENT AND ADD ONTO
CL                   FIELD TO OBTAIN PROVISIONAL VALUE.
C ---------------------------------------------------------------------
     ...
     *            RS(I,K)*(DELTA_AK(K)+DELTA_BK(K)*PSTAR_OLD(I)))
            THETAL_FIRST_INC(I,K) = THETAL_FIRST_INC(I,K)*SCALAR1
            THETAL_PROV(I,K) = QT(I,K)-THETAL_FIRST_INC(I,K)
          ENDDO
...
CL---------------------------------------------------------------------
CL    SECTION 6.     ALL WORK IN THIS SECTION PERFORMED AT LEVEL-1.
CL                   CALCULATE SECOND INCREMENT DUE TO ADVECTION.
CL                   CALCULATE TOTAL INCREMENT TO FIELD AND FILTER
CL                   WHERE NECESSARY THEN UPDATE FIELD.
CL                   THE POLAR INCREMENTS ARE THEN CALCULATED AND ADDED
...
C ---------------------------------------------------------------------
CL    SECTION 6.1    CALL ADV_P_GD TO OBTAIN SECOND INCREMENT DUE TO
CL                   ADVECTION.
C ---------------------------------------------------------------------

          CALL ADV_P_GD(Q_LEVELS,THETAL_PROV,
     &                  U_MEAN_COPY,V_MEAN_COPY,
     &                  ETADOT_MEAN,
     *                  SEC_P_LATITUDE,
     *                  THETAL_INCREMENT,NUX,NUY,P_FIELD,
...
C ---------------------------------------------------------------------
CL    SECTION 6.2    CALCULATE TOTAL MASS-WEIGHTED INCREMENT TO FIELD.
C ---------------------------------------------------------------------
...
C ---------------------------------------------------------------------
CL    SECTION 6.3    IF GLOBAL MODEL CALCULATE POLAR INCREMENTS.
...
! Do Southern boundary/pole
     ...
     &                  (DELTA_AK(K1)+DELTA_BK(K1)*PSTAR(IK))
            THETAL_INCREMENT(IK,K1) = -.5*(THETAL_INCREMENT(IK,K1) -
     &                           THETAL_FIRST_INC(IK,K-1)*SCALAR2)
              QT(IK,K1)     = QT(IK,K1)*SCALAR2
            ENDDO
          ENDIF ! (atbase)

      ENDDO ! DO K=2,Q_LEVELS+1

CL---------------------------------------------------------------------
CL    SECTION 7      IF GLOBAL MODEL THEN FILTER INCREMENTS AND
CL                   UPDATE POLAR VALUES BY CALLING POLAR.
CL                   UPDATE ALL OTHER VALUES.
CL---------------------------------------------------------------------

C ---------------------------------------------------------------------
CL    SECTION 7.1    CALL FILTER TO DO FILTERING.
C ---------------------------------------------------------------------
...
CL    CALL FILTER FOR QT INCREMENTS
      ...
      CALL FILTER(THETAL_INCREMENT,P_FIELD,Q_LEVELS,
     &            FILTER_SPACE,ROW_LENGTH,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
...
C ---------------------------------------------------------------------
CL    SECTION 7.2    CALL POLAR TO UPDATE POLAR VALUES
C ---------------------------------------------------------------------

      CALL POLAR(QT,THETAL_INCREMENT,THETAL_INCREMENT,
     &  FIRST_ROW , TOP_ROW_START , P_LAST_ROW , U_LAST_ROW,
     &  P_BOT_ROW_START , U_BOT_ROW_START , upd_P_ROWS , upd_U_ROWS,
     &  FIRST_FLD_PT , LAST_P_FLD_PT , LAST_U_FLD_PT,
     &  FIRST_VALID_PT , LAST_P_VALID_PT , LAST_U_VALID_PT,
...
C ---------------------------------------------------------------------
CL    SECTION 7.3    UPDATE ALL OTHER POINTS.
C ---------------------------------------------------------------------
