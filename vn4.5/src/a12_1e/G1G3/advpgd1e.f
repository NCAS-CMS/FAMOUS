...
! North Pole Flux
     ...
     &              ETADOT(I,K+1) * (FIELD(I,K+1) - FIELD(I,K))
          ...
          IF (LWHITBROM)
     &    FIELD_INC(I,K) = FIELD_INC(I,K)+FIELD(I,K)*BRSP(I,K)
          WORK(I)=SCALAR1
        ENDDO
      ENDIF ! (at_top_of_LPG)
...
! North Pole Flux
     ...
     &    FIELD_INC(I,K) = FIELD_INC(I,K)+FIELD(I,K)*BRSP(I,K)
      ...
      ENDIF ! (at_top_of_LPG)
      IF (at_base_of_LPG) THEN
! North Pole Flux & South Pole Flux
        DO I=P_BOT_ROW_START,P_BOT_ROW_START+ROW_LENGTH-1
          SCALAR1 = 0.5 * ADVECTION_TIMESTEP *
     &              ETADOT(I,K+1) * (FIELD(I,K+1) - FIELD(I,K))
          FIELD_INC(I,K) =  SCALAR1
...
! South Pole Flux
     ...
     &    FIELD_INC(I,K) = FIELD_INC(I,K)+FIELD(I,K)*BRSP(I,K)
        ENDDO
      ENDIF ! (at_base_of_LPG)
      endif ! if(k.ne.1.and.k.ne.P_LEVELS)
      IF (at_top_of_LPG) THEN
        DO I=TOP_ROW_START+1,TOP_ROW_START+ROW_LENGTH-1
          FIELD_INC(I,K) = FIELD_INC(I,K) +
     *            ADVECTION_TIMESTEP * SEC_P_LATITUDE(I) *
     *            .25*LATITUDE_STEP_INVERSE*
     *            (V(I,K)+V(I-1,K))*
     *            (FIELD(I,K) - FIELD(I+ROW_LENGTH,K))
        ENDDO
      ENDIF ! (at_top_of_LPG)
      IF (at_base_of_LPG) THEN
        DO I=P_BOT_ROW_START+1,P_BOT_ROW_START+ROW_LENGTH-1
          FIELD_INC(I,K) = FIELD_INC(I,K) +
     *            ADVECTION_TIMESTEP * SEC_P_LATITUDE(I) *
     *            .25*LATITUDE_STEP_INVERSE*
     *            (V(I-ROW_LENGTH,K)+V(I-1-ROW_LENGTH,K))*
...
! Calculate V_TERM in polar row
            V_TERM(IK-ROW_LENGTH) = (1.0+NUY(IK,K))*0.5*
     &         extended_WORK(extended_index-extended_ROW_LENGTH)
     &      - NUY(IK,K)*0.5*extended_WORK(extended_index)
...
! Calculate V_TERM in polar row
            V_TERM(IJ+ROW_LENGTH) = (1.0+NUY(IJ,K))*0.5*
     &         extended_WORK(extended_index) - NUY(IJ,K)*0.5*
     &         extended_WORK(extended_index-extended_ROW_LENGTH)
...
! South Pole Flux
     ...
     &    FIELD_INC(I,K) = FIELD_INC(I,K)+FIELD(I,K)*BRSP(I,K)
      ...
      IF (at_top_of_LPG) THEN
        DO I=TOP_ROW_START,TOP_ROW_START+ROW_LENGTH-1
          FIELD_INC(I,K) = FIELD_INC(I,K) +
     *            ADVECTION_TIMESTEP * SEC_P_LATITUDE(I) * V_TERM(I)
        ENDDO
      ENDIF ! (at_top_of_LPG)
      IF (at_base_of_LPG) THEN
        DO I=P_BOT_ROW_START,P_BOT_ROW_START+ROW_LENGTH-1
